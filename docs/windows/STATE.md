...
| 2 | **金鑰裡的模組清單用哪一套 key** | **已備妥資料，見下方〈第 2 項的決策資料〉，現在是可決定的了。** | 🟠 第 4 步前要決 |
...
1. **金鑰沿用同一套 key，不另造一套。** 兩套 key 會產生「對照表」，
   而對照表是最會腐爛的東西（本專案已有 `_FILE_MODULE`／側欄／後端三方對不上的前例，
   靠 `test_module_keys_consistency_2026_09_13.py` 才擋得住）。
   **金鑰管「機器買了什麼」、旗標管「人能看什麼」，兩層都用同一組名字、各自獨立判定。**
2. **但賣的單位要粗一點。** 金鑰裡寫的是**套餐**不是 40 個開關，例如：
   `selection_db`（＝七類導覽 14 個 key 一起）、`core_sales`（報價＋案件＋簽核）、
   `finance`、`procurement`、`hr`。客戶不會想懂 40 個 key，業務也不會想報 40 個價。
   套餐→key 的展開表放在 `helpers/licensing.py` 裡，**是程式碼不是設定**（會被測試釘住）。
3. **那 3 個要補的後端檢查，在第 4 步一起做**，不要另開一輪。

> 您只要回答一句：**「套餐怎麼切」**。上面第 2 點的五個套餐是我的提案，
> 您可以直接說「就這樣」或告訴我您打算怎麼報價，我照您的報價單位切。
