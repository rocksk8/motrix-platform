...
- **輪次**：**第 4 輪** —— 細線 6 標案雷達第 1～3 步（設條件、抓得到、比對得出命中）
- **狀態**：🟥 **驗收測試寫完，28 題全紅、0 題綠** → 等 A 派 B 寫碼
- **檔案**：`backend/tests/test_tender_match_2026_09_21.py`（檔名照 §3，不是 A 訊息裡建議的那個）
- **契約來源**：`docs/windows/STATE.md` §3 的 10 條驗收條件

> ✅ **這一輪回到正常順序**：②（C 先寫紅測試）先於 ④（B 寫碼）。
> B 還沒寫任何產品碼，所以「不讀實作」是自然成立的，**全紅是正確的起點**。

### 🔴 我沒有讀實作

`helpers/tender_source.py`／`helpers/tender_match.py`／`routers/tender_radar.py`
**三個檔都還不存在**（我 `ls` 過），所以這一輪沒有可讀的實作。
...
**`28 failed` —— 全紅，沒有任何一題是綠的。** 紅的原因分四類，全部是預期的：

| 筆數 | 原因 |
|---|---|
| 13 | `helpers/tender_match.py` 不存在 |
| 9 | `helpers/tender_source.py` 不存在 |
| 5 | 三張新表不存在（`tender_watches`／`tenders`／`tender_hits`） |
| 1 | `tenders` 沒有 `budget` 欄位 |

**沒有任何一題是紅在別的地方** —— 這點我特地分群確認過，
因為「紅了」不等於「紅在你以為的那個原因上」。

### 28 題對應 §3 的 10 條驗收條件

| 條件 | 測試 | 題數 |
|---|---|---|
| 1 關鍵字命中 | `test_01`、`01b` 對照組、`01c` OR、`01d` 一標案多 watch | 4 |
| 2 排除詞整筆排除 | `test_02`、`02b` 對照組（沒中不可誤殺） | 2 |
| 3 機關沒填＝不篩 | `test_03`、`03b` 對照組 | 2 |
| 4 金額只篩填了的那一邊 | `test_04`（5 個參數，含「兩邊都沒填」） | 5 |
| 5 同案號去重 | `test_05` 三張表 ＋ `05b` 唯一鍵 ＋ `05c` hits 不重複 | 5 |
| 6 缺必要欄位就丟且計數 | `test_06`、`06b` 對照組 | 2 |
| 7 dropped 過半→疑似改版 | `test_07`、`07b` 對照組（正常頁不可被判改版） | 2 |
| **7b 有容器零筆→`recognised=True`** | `test_07b_empty_list_with_container_is_recognised` | 1 |
| **7c 沒有容器→`recognised=False`** | `test_07c_page_without_list_container_is_not_recognised` | 1 |
| **8 開關關著時連呼叫都不該發生** | `test_08`、**`08b` 對照組**、`08c` 出貨預設 False | 3 |
| 9 預算 0 元 vs 公告沒寫 | `test_09` | 1 |
| 10 題數不可少於上一輪 | （⑥的收斂條件，不是測試） | — |

**十一題是對照組**（`01b`／`02b`／`03b`／`06b`／`07b`／`08b` 等）。
理由都一樣：**單邊的斷言驗不出東西**。
「關鍵字沒中就不命中」缺席的話，一個「永遠回全部 watch」的實作會讓條件 1 全綠。

### 「綠」的定義（B 要做到的契約）

`helpers/tender_source.py`：`TENDER_RADAR_ENABLED`（**預設 `False`**）、
`fetch_raw(params) -> str`、`parse_list(html) -> (items, dropped, recognised)`、
⚠️ `run_scan()`、⚠️ `suspect_redesign(parsed_count, dropped) -> bool`

`helpers/tender_match.py`：⚠️ `match_watches(tender, watches) -> list[dict]`

⚠️ **標記的三個名字是我取的，§3 沒有指定** —— 見〈給彙整〉第 16 點。

**欄位一律 snake_case**（`case_no`／`org`／`deadline`／`budget`／`budget_min`…）。
理由：這些是**純函式參數與資料庫欄位，不是 API 回應**。
第 3 輪的教訓是 **API 命名空間 ≠ 資料庫命名空間**，所以這裡刻意不套 camelCase。
...
### 16 · 🟠 三個函式名是我取的，而且 §3 有一處自相矛盾

§3 只指定了 `fetch_raw`、`parse_list`、`TENDER_RADAR_ENABLED`。這三個是**我取的**：

| 名字 | 用途 | 為什麼需要它 |
|---|---|---|
| `run_scan()` | 每日掃描進入點 | 條件 8 要驗「開關關著時 `fetch_raw` 沒被呼叫」，**必須有一個呼叫它的東西** |
| `suspect_redesign(parsed_count, dropped)` | 疑似改版判定 | 見下面的矛盾 |
| `match_watches(tender, watches)` | 比對 | §3 描述了行為但沒給名字 |

⚠️ **§3 的矛盾**：它把 `parse_list` 明確定義成**三元組** `(items, dropped, recognised)`，
但條件 7 又要求「`dropped` 超過一半 → **回傳一個**『疑似對方改版』的旗標」。
**三元組裡沒有這個旗標的位置。**

兩條路：① `parse_list` 變四元組；② 另開一支純函式（我選的）。
我選 ② 是因為它是**可從 `(items, dropped)` 算出來的衍生判斷**，
放進 `parse_list` 會讓那支函式同時負責「解析」與「判斷健康度」兩件事。
**但這要 A 裁決，B 照著寫之前先定案，不然兩邊會各寫一種。**
...
- [x] 測試寫成紅的：**28 題全紅、0 綠**，且**分群確認過紅的原因全部是預期的那四類**
- [x] 每一條驗收條件都有對應的測試，**11 題是對照組**（單邊斷言驗不出東西）
- [x] 檔名照 §3（`test_tender_match_2026_09_21.py`），不照 A 訊息裡的建議
- [x] `--basetemp` 用 `-C-adhoc`（單檔），全量回歸才用 `-C-full`
- [ ] ⑤反向驗證（§3 四題）—— **等 B 寫完碼才做**
- [ ] ⑥全量回歸 —— **等 B 寫完碼才做**，基準 1,129
- [x] 規格疑義都寫進〈給彙整〉（第 15～17 點），沒有自己動產品碼
