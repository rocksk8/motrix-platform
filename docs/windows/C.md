...
- **狀態**：🟡 **②⑤⑥全部做完。46 題全綠、⑤六個突變全部發火、⑥ 1,175 題**
  ⚠️ **但⑥有 1 題紅，是 B 的新表沒登記備份**（見〈給彙整〉第 21 點，**不是我能改的**）
...
```
1 failed, 1119 passed, 55 skipped in 1140.45s (0:19:00)
```
...
| 本輪實跑（1+1119+55） | **1,175** |

＝ 1,129 ＋ 本輪新檔 46，**收集數 ＝ 實跑數，沒有題目被靜默跳過**。

**🔴 1 題紅：`test_every_table_is_either_backed_up_or_explicitly_excluded`** ——
見〈給彙整〉第 21 點。**那是 B 的產品決定，我沒有動它。**

### 握手（A 於 `4edc02d` 定案的形狀）

```
B 宣告停手 SHA        : a18adf9（35ccd60）
我跑⑥的 HEAD（前／後）: 35ccd60 → ff55ecb
工作樹（跑後）        : 乾淨
git log a18adf9..HEAD -- backend frontend  →  (空)  ✅
```

⚠️ **判準是「往前跑的都不是產品碼」，不是「HEAD 沒動」** ——
A 與 C 都會在這 19 分鐘裡 commit 文件，用「HEAD 沒動」當判準會誤報。

⚠️ **`--basetemp` 用 `motrix-pytest-C-full`**。A 一度說「用 `-adhoc`，不要跟 B 的 `-full` 撞」
是**把規則壓扁了**：協定 §5 第 3 條是**分視窗**且**分用途**兩個維度，
`-full` 是「全量回歸」不是「B 的」。照那句做，⑥ 會跟我自己的單檔跑互刪
—— **正好是那條規則存在的原因**。
...
**第 4 輪（2026-09-21）** —— ②⑤⑥全部完成

- [x] 只動了 `backend/tests/**`
- [x] **我沒有讀解析器**（接縫用 `dir`／`signature`／黑箱探測找的，沒用 `getsource`）
- [x] ② 46 題，B 實作後全綠
- [x] ⑤ 六個突變**全部發火**；形狀驗證那個我寫錯三次，過程已記
- [x] ⑥ **1,175 題**（＝基準 1,129 ＋ 本輪 46），收集數 ＝ 實跑數
- [x] 握手：`git log a18adf9..HEAD -- backend frontend` 為空，工作樹乾淨
- [x] `--basetemp` 用 `motrix-pytest-C-full`（全量）／`-C-adhoc`（單檔）
- [ ] 🔴 **⑥ 有 1 題紅**：B 的四張新表沒登記備份 —— **待 A 裁決、B 施作**（〈給彙整〉21）
- [x] 發現的問題都寫進〈給彙整〉，**沒有自己動產品碼、也沒有把新表塞進排除清單**
