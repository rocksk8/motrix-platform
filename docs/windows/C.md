...
- **狀態**：🟥 **驗收測試寫完（照 §3 第二版），35 題全紅、0 題綠** → 等 A 派 B 寫碼
- **檔案**：`backend/tests/test_tender_match_2026_09_21.py`（檔名照 §3）
- **契約來源**：`docs/windows/STATE.md` §3 **第二版**（`862d48c`）的 **17 條**驗收條件

> ⚠️ **我寫完第一版於 13:49，A 於 13:50:53 把 §3 改成第二版** —— 晚一分鐘。
> 不是 A 故意中途改單：B 開工前審了那張單，八項成立、**三處規格是錯的**。
> A 主動來告知（他自己立的規則第 ③ 條），我已全部改照第二版。
>
> **改了什麼**：`fetch_raw` 簽名加 `error`（條件 7d）／唯一鍵 `案號` → **`(機關, 案號)`**
> （條件 5）／截止日移出必要欄位（條件 6、新增 6b）／新增 6c、8b、9b、9c。
> 條件 1～4、7、7b、7c、9 沒動。**28 題 → 35 題。**

### 🔴 我沒有讀實作

`helpers/tender_source.py`／`helpers/tender_match.py`／`routers/tender_radar.py`
**三個檔都還不存在**，所以這一輪沒有可讀的實作。
...
**`35 failed` —— 全紅，沒有任何一題是綠的。** 分群確認紅的原因全部是預期的：

| 筆數 | 原因 |
|---|---|
| 14 | `helpers/tender_source.py` 不存在 |
| 13 | `helpers/tender_match.py` 不存在 |
| 7 | 四張新表不存在（含 `tender_fetch_log`） |
| 1 | `tenders` 沒有 `budget` 欄位 |

**沒有任何一題紅在別的地方** —— 特地分群確認過，因為**「紅了」不等於
「紅在你以為的那個原因上」**。

### 35 題對應 §3 v2 的 17 條驗收條件

| 條件 | 題數 | 備註 |
|---|---|---|
| 1 關鍵字命中 | 4 | 含對照組、OR、一標案多 watch |
| 2 排除詞整筆排除 | 2 | 含「沒中不可誤殺」對照組 |
| 3 機關沒填＝不篩 | 2 | 含對照組 |
| 4 金額只篩填了的那一邊 | 5 | 含「兩邊都沒填」 |
| **5 `(機關, 案號)` 去重** | 7 | 四張表存在 ＋ 同機關同案號擋下 ＋ **同案號不同機關要留兩筆** ＋ hits 不重複 |
| 6 必要欄位（案號／名稱／機關）缺一就丟 | 2 | 含「完整頁不可丟」對照組 |
| **6b 沒截止日仍收下、`deadline` 是 `None`** | 1 | |
| **6c 欄序對調要被丟掉** | 1 | |
| 7 dropped 過半→疑似改版 | 2 | 含「正常頁不可被判改版」對照組 |
| 7b 有容器零筆→`recognised=True` | 1 | |
| 7c 沒有容器→`recognised=False` | 1 | |
| **7d 抓不到 ≠ 不認得** | 1 | |
| 8 開關關著→呼叫次數 == 0 | 1 | |
| **8b 開關打開→同一計數器 ≥ 1** | 1 | **沒有它第 8 題是假綠** |
| 8c 出貨預設 False | 1 | |
| 9 預算 0 元 vs 公告沒寫 | 1 | |
| **9b `tender_fetch_log` 查得到** | 1 | |
| **9c 同日第二次不發外部請求** | 1 | |
| 10 題數不可少於 **1,129** | — | ⑥的收斂條件，不是測試 |

**十一題是對照組。** 理由都一樣：**單邊的斷言驗不出東西。**

### 這一輪的核心：三種（其實四種）壞法不能互相蓋掉
...
| **欄序被對調** | 該筆丟掉並計入 `dropped` | ⚠️ **三個訊號全綠而每一筆資料都是錯的** |
...
**第 4 輪（2026-09-21）** —— ②已完成，等 B 寫碼

- [x] 只動了 `backend/tests/**`
- [x] **我沒有讀實作**（三個檔都還不存在，自然成立）
- [x] 測試寫成紅的：**35 題全紅、0 綠**，且**分群確認紅的原因全是預期的四類**
- [x] 照 §3 **第二版**（`862d48c`）寫，17 條驗收條件全覆蓋
- [x] 11 題對照組（單邊斷言驗不出東西）
- [x] `--basetemp` 用 `-C-adhoc`
- [ ] ⑤反向驗證（§3 v2 **六題**）—— 等 B 寫完碼
- [ ] ⑥全量回歸 —— 等 B 寫完碼，基準 **1,129**（§3 寫死）
- [x] 規格疑義寫進〈給彙整〉（第 15～18 點），沒有自己動產品碼
