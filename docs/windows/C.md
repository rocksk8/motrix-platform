...
- **狀態**：✅ **②⑤⑥全部完成**。46 題全綠、⑤六個突變全部發火、**⑥ 1,175 題 0 失敗**
  （⑥第一次有 1 題紅 —— B 的新表沒登記備份，守門精準命中；補工後重跑全綠）
- **契約來源**：`docs/windows/STATE.md` §3 **`5a866e4`**
...
**第 4 輪（2026-09-21）** —— ✅ ②⑤⑥全部完成，等 A 走⑦結案

- [x] 只動了 `backend/tests/**`
- [x] **我沒有讀解析器**（接縫用 `dir`／`signature`／黑箱探測，沒用 `getsource`）
- [x] ② 46 題，B 實作後全綠
- [x] ⑤ 六個突變**全部發火**；形狀驗證那個我寫錯三次，過程已記
- [x] ⑥ **1,175 題 0 失敗**（＝基準 1,129 ＋ 本輪 46），收集數 ＝ 實跑數
- [x] ⑥ **整支重跑**，沒有只跑受影響的那一支
- [x] 握手兩個檢查都過（甲排除 `backend/tests`、乙不排除，工作樹乾淨）
- [x] `--basetemp` 用 `motrix-pytest-C-full`（全量）／`-C-adhoc`（單檔）
- [x] 補工那一筆只加 `tender_fetch_log`，**沒有把另外三張塞進排除清單**
- [x] 問題都寫進〈給彙整〉（第 15～23 點），沒有自己動產品碼
