...
- **狀態**：🟥 **46 題，43 紅 3 綠** → 等 A 派 B 寫解析器
  （3 綠是**樣本守門**，它們不碰解析器；其餘 43 題全紅）
- **契約來源**：`docs/windows/STATE.md` §3 **`5a866e4`** 的 **34 條**驗收條件
  （照 §5l 記 SHA —— 沒有版本號的「我照單寫了」，等於沒說照的是哪一張）

### 🔴 我沒有讀解析器

三個產品碼檔案都還不存在。**我讀的是 `fixtures/tender_list_20260921.html`
（測試資料，我的領域），不是解析器。**

### 🔴 所有 HTML 樣本改成從真實 fixture 切出來，不再手寫

第一版我手寫了假定結構的 HTML。真實樣本到位後全部改成**對 fixture 動外科手術**
（挖掉資料列、清空某一格、兩格對調…）。

⚠️ **手寫樣本的問題不是「不像」，是它會把我對結構的誤解變成測試的前提** ——
然後解析器照著我的誤解寫，兩邊一致而且全綠，直到上正式機。

### 我掃真實樣本發現三件事，全部已改 §3（A 複驗屬實）

| 發現 | 後果 |
|---|---|
| **`截止投標`／`預算金額` 在原始 HTML 出現 0 次** | §3 要 B 用這四個字樣判 `recognised`。照字面寫 **`recognised` 永遠是 `False`** |
| **標案名稱是 `<script>` 裡 `pageCode2Img("…")` 的引數** | naive strip tags 得到一行 JS，**而真名是它的子字串** ⇒ 關鍵字照樣命中、條件 1 照樣綠、**資料庫每一筆名稱都是 JavaScript** |
| 資料列是 **5** 筆不是 6（6 個 `<tr>` 含表頭），全頁 **29** 個 `<tr>` | 掃全部 `<tr>` 會把查詢表單的列當成標案 |

⚠️ 第一項的成因值得記：**A 看到的表頭是工具正規化後的輸出，他把它當成原始碼寫進規格。**

> **上一次（`cells[:7]`）是「工具只給了你一部分」；這一次是「工具給了你一個更好看的版本」。
> 後者更難發現，因為它沒有缺東西。**
...
```
cd backend
python -m pytest tests/test_tender_match_2026_09_21.py -q --basetemp=C:/Users/hichan/AppData/Local/Temp/motrix-pytest-C-adhoc
```

`43 failed, 3 passed`。分群確認紅的原因全部是預期的：

| 原因 | |
|---|---|
| `tender_source.py` 不存在 | 22 |
| `tender_match.py` 不存在 | 13 |
| 四張表不存在／缺 `budget` 欄位 | 8 |

**3 綠是刻意的**：`test_00`（fixture 位元組）／`test_00b`（手術真的改到東西）／
`test_00c`（表單頁確實有 `<table>`）—— 它們保護的是**樣本本身**，不碰解析器。
...
**第 4 輪（2026-09-21）** —— ②已完成，等 B 寫解析器

- [x] 只動了 `backend/tests/**`
- [x] **我沒有讀解析器**（三個檔都還不存在）
- [x] 樣本全部從**真實 fixture** 切出來，每一刀都有 `assert` 守門
- [x] 測試跑過：**46 題，43 紅 3 綠**，分群確認紅的原因全是預期的
- [x] 照 §3 **`5a866e4`** 寫（34 條驗收條件）
- [x] `--basetemp` 用 `-C-adhoc`
- [ ] ⑤反向驗證（§3 六題）—— 等 B 寫完碼
- [ ] ⑥全量回歸 —— 等 B 寫完碼，基準 **1,129**
- [x] 規格疑義寫進〈給彙整〉（第 15～20 點），沒有自己動產品碼
