...
✗ ⚠️ **沒查前端簽核設定頁能不能「編輯」出三層**（只查了後端）
  ⇒ 那一格會擋住使用者自己設三層 ⇒ 建議列進 `AS1` 的頁面驗收
✅ **三層已補查（A-2 2026-09-23，`8ed14f4`）—— 成立，`AS2` 不必擴範圍**
  結構：`setting_to_active_tiers()` 是迴圈無上限；推進是
        `currentTier = ct + 1 + len(cascaded)` 與 `>= len(tiers)` 的泛型比較；
        `tiered_approval.py` 裡 `len(tiers)`／`order` 與數字的比較 **0 處**
  實跑：`test_org_chain_approval_2026_09_15.py:336-338` **真的建了三層具名**並核准；
        `:309` 另一題斷言 `currentTier == 2`
  ⚠️ 而那一題走的是 **cascade 自簽**這條路
     ⇒ **「三個不同的人依序簽」沒有被單獨驗過** ⇒ `§5⑥` 就用三個不同的人
