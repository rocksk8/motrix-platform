...
```
❌ status not in ("已核准","已過帳")  => **會把作廢單一起擋掉**
✅ not approval_done(voucher)          => 而作廢單另外放行（見 §5）
```
☠️ 前者牴觸一條**已經存在的裁定**：`voucher_pdf.py:363` docstring 逐字
「**已作廢的傳票也要印得出來**」。
📌 A 2026-09-23 要求把這一句**寫死在規格裡**，因為那個錯法寫起來很自然。
...
## §5b 🔴 浮水印在傳票那種短版面上會擠在一起（A-2 補查，`8e68088`）

### 結構事實（這一格不依賴我的像素估算）

```
.pdf-page      width: 800px  **而沒有設 height** => 高度由內容決定
.pdf-watermark position:absolute; inset:0  => 它填滿那個**由內容決定**的高度
               grid-template-rows: repeat(4, 1fr)  => **永遠 4 列**
```
🔑 ⇒ **列距 = 頁高 ÷ 4，而頁高隨內容縮。**
報價單是滿頁 ⇒ 列距寬；傳票內容只佔 A4 的 **31.8%** ⇒ **列距約只有三分之一**。

### 粗估（⚠️ **算出來的，不是渲染出來的**）

```
傳票內容 268pt，頁寬 800px 對 A4 595pt => 比例 1.344
=> 內容高 ≈ 360px ＋ padding 32×2 => 頁高 ≈ **424px**
=> 每列 ≈ **106px**
而一個 .pdf-wm-item：b 19px ＋ small，旋轉 -28°
=> 外接高度 ≈ 130×sin28 + 47×cos28 ≈ **102px**
⇒ 102 / 106 => **幾乎貼在一起**
（橫向沒問題：3 欄 × 267px，外接寬 ≈ 137px）
```
...
```
📌 而這正是 `§1`「**本輪複製不抽共用 css**」的價值：
**傳票改列數不會動到報價單。**

⚠️ **它的失敗方式是「看起來像壞掉」，不是「壞掉」** ⇒ **不會有任何測試會紅**
⇒ 驗收只能是**人看一眼**（`§7⑬`），而那一題要留下截圖或描述。
...
頁面 ⑪ 預覽內容與匯出的 PDF **同一份模板**
     ...
     ⑫ 浮水印在預覽上看得到，且 `voided_at != ''` 時顯示的是**作廢**那一種
        （作廢 **優先於** 未簽核 —— 兩者同時成立時只顯示作廢）
     ⑬ ⚠️ **人看一眼**：浮水印在傳票的短版面上**沒有擠成一團**（§5b）
        ☠️ 這一題自動化不了 —— 它的失敗方式是「看起來像壞掉」
     ⑭ 預覽的附件清單**逐筆標出可否併入 PDF**，且與匯出後的實際結果一致
```
...
### ✅ 已裁（使用者 2026-09-23，`STATE.md §229`）
```
② 浮水印優先序  **作廢優先於未簽核** —— 照 A-2 建議，理由也照原話：
   「一張作廢的傳票可能同時未簽核完成，而**使用者要知道的第一件事是它已經作廢**」
③ 預覽附件清單  **要列，並標出哪些併得進 PDF**
   ⚙️ 「併得進」＝ 實體檔存在且型別是 jpg/png/pdf
      => 與 §2 的 attachment_count 同一次查詢回來，不要前端自己判
