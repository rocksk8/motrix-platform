# `SPEC-EM2` · 角色名稱不可硬寫在訊息裡

> 座標：`d59f47f`（工作樹：本檔為新增，其餘未動）
> 作者：視窗 A-2 ／ 2026-09-23
> 裁定：A **甲案**（動態解析）。`STATE.md §265`。

---

## §1 這一題的判準翻過一次面，**錯的那一列留著**

### ❌ 原判準（A 初裁，已作廢）

```
「11 條訊息與畫面不一致 => 改成『超級管理員』」
⚙️ 正對照 11 條會亮 ／ 負對照 那 10 條『超級管理員』不可以亮
```

### 🔴 它為什麼不成立

```
backend/routers/system.py:2000  _DEFAULT_ROLE_LABELS = {"superadmin": "超級管理員", ...}
backend/routers/system.py:2039  GET  /api/settings/role-labels   _require_user
backend/routers/system.py:2054  PUT  /api/settings/role-labels   require_superadmin=True
frontend/pages/users.html:266   <input x-model="rlForm.superadmin" placeholder="超級管理員">
```

🔑 **「超級管理員」是預設值，不是畫面的固定用詞。**
客戶按一次 PUT 把 superadmin 改名成「總經理」，**29 句訊息一起錯**——
**包含原判準指定當負對照的那一批**。

☠️ 絕對化藏在「畫面」兩個字裡：**畫面上那個詞本身是資料。**

📌 而「與畫面一致」問的是**哪一個畫面**——`sidebar.js` 與 `users.html` 今天就對不上（見 §2b）。

---

## §2 母體：**87 條後端 ＋ 5 份前端對照表**

> 🔴 **2026-09-23 A 裁定擴到 87**（原 29）。理由：
> 判準相同（硬寫角色名）／修法相同（`role_label()`）／守門相同 ⇒ **分開只是讓它做兩次**。

### §2a 後端 `HTTPException` 的 `detail` 裡硬寫角色名稱 = **87 條**

（AST 取 `detail` 字面值，掃 `backend/**/*.py`，排除 `rollback_snapshots/` 與 `tests/`；全體 `detail` 字面值 716 條）

#### superadmin = 29 條

（AST 取 `HTTPException` 的 `detail` 字面值，掃 `backend/**/*.py`，排除 `rollback_snapshots/` 與 `tests/`）

```
最高管理者  9   daily_tasks.py:419,472,552
                dev_crm.py:463,660
                quotations.py:1865,2420,2463,3372
最高管理員  3   approval_delegates.py:66,116
                quotations.py:477
超級管理員 17   auth.py:228,230,1550,1584,1643,1692
                quotations.py:1401,1571,1905,3462,4308,4465
                completion_notes.py:486 ／ contractor_vouchers.py:494
                invoice_vouchers.py:532 ／ payment_requests.py:689
                shipping_notes.py:391
```

#### admin = **58 條**

```
「管理員」（⚠️ 先把三個 superadmin 詞切掉再找，否則上面那 29 條會被重複計）
accounting_export 206,460 ／ auth 1002,1105
bonus 207,271,395,513   ／ case_action_items 176
case_extra_expenses 270,305,334,737,772,799,827,856
cashier 39 ／ completion_notes 112,763 ／ contractor_vouchers 120,701
dashboard 699 ／ dev_crm 395,429,581,624,714,979,1000
inventory 44 ／ invoice_vouchers 106 ／ material_orders 95
module_versions 19 ／ payment_requests 134
quotations 1084,1108,1858,1901,2159,3018,3311,3343,4759,5700
reports 61,2792 ／ shipping_notes 53,733 ／ system 265
vendor_contractors 25,223,251,292,331,364,559,731
```

#### sales = **1 條**（4 條命中，3 條是誤報）

```
✅ quotations.py:38         「無權限存取**其他業務**的報價單」 <= A 裁定納入
❌ dev_crm.py:92            「無**業務開發**模組權限」          => 模組名
❌ case_action_items.py:206 「…或為該案件**業務員**所屬部門主管」=> 職務描述
❌ case_action_items.py:211 「需要**業務**確認（project_approve_biz…）」 => 權限名
```
📌 D 在 `PX1` 第二輪對 `quotations.py:38` 另有一個觀察
（**它會讓缺模組的人去找業務歸屬**）⇒ 改文字時不要把它弄得更精確而更誤導。

#### engineer = **0 條** ／ viewer = **0 條**

### 🔑 「不要從 29 條推它的規模」——**方向兩邊都會錯**

```
admin  少報 58   （子字串被 superadmin 吃掉）
sales  多報 3    （「業務」在模組名與職務描述裡）
=> 推出來的不是偏大就是偏小，**沒有一次是對的**
```

### ⚠️ 上一份回報寫「11 條」，那是錯的母體——**這一列留著**

```
我上一則報 8／3／10 = 21，那是 `EM1` 的 **11~25 字視窗**撈到的，
而我**沒有講這個限定**。全長重掃才是 29。
🔑 〈判準的寬窄都會騙人〉：太窄的判準給的是**假陰性**，
   而我拿它去做了一個「只有 11 條」的裁定建議。
```

### §2b 前端 **5 份各自硬寫**的對照表（**沒有一份讀 `/api/settings/role-labels`**）

```
frontend/pages/approval-queue.html   :1402  superadmin: '最高管理者'
frontend/pages/online-stats.html     :368   superadmin: '最高管理者'
frontend/static/sidebar.js           :296   superadmin: '最高管理者'
frontend/pages/daily-tasks.html      :2999  superadmin: '超管'
frontend/pages/approval-settings.html:900   superadmin: '超級管理員'
──
frontend/pages/users.html            :771 / :773   （Alpine 初值＋表單初值，
                                                    是合理的 fallback，不在母體）
```

☠️ **第四個詞「超管」**——最初的三詞掃描（`最高管理者`／`最高管理員`／`超級管理員`）
**完全沒有看到它**。
🔑 它證明「有三種說法」這個數字本身**是沒查完的**。

### §2c 使用者自己也混用 ⇒ **這一題沒有裁示可依**

```
STATE.md 使用者原話   最高管理者 23 ／ 最高管理員 5 ／ 超級管理員 20
```
📌 而**甲案不需要那個裁示**——它讀設定，而設定是客戶自己填的。

---

## §3 處置（甲）

### §3a 後端：新增 `backend/helpers/role_labels.py`

```python
"""角色顯示名稱：**唯一的解析點**。

☠️ 不要在訊息裡寫死角色名稱 —— 它是客戶可改的設定
   （PUT /api/settings/role-labels），寫死的那天客戶改名就全錯。
"""
from helpers.settings import _get_setting

DEFAULTS = {
    "superadmin": "超級管理員",
    "admin":      "管理員",
    "sales":      "業務",
    "engineer":   "工程師",
    "viewer":     "檢視者",
}

_CACHE: dict = {"v": None}


def role_labels() -> dict:
    """五個角色的顯示名稱。行程內快取，PUT 之後由 invalidate() 清掉。"""
    if _CACHE["v"] is None:
        _CACHE["v"] = {**DEFAULTS, **(_get_setting("role_labels") or {})}
    return _CACHE["v"]


def role_label(key: str) -> str:
    return role_labels().get(key, key)


def invalidate() -> None:
    """PUT /api/settings/role-labels 成功之後**必須**呼叫。"""
    _CACHE["v"] = None
```

#### ⚠️ 為什麼是**新檔**而不是改 `helpers/settings.py`（與 A 的界線 ④ 不同）

```
A 的界線 ④：helpers/settings.py 是共用檔 => 動前宣告
我的建議  ：開新檔 helpers/role_labels.py，**settings.py 一行都不動**
理由      ：省掉共用檔的宣告與碰撞，而能力一樣
📌 若 A 仍要放進 settings.py，本節照搬即可，其餘不變。
```

#### ⚠️ 快取失效的**邊界條件**（寫下來，因為它不會自己提醒你）

```
今天成立：三支啟動指令都**沒有 --workers**
          backend/start.bat:27 ／ restart.bat:40 ／ autostart.bat:31
          => 單一 uvicorn process => 行程內快取清得掉
🔴 日後若加 --workers N，**另外 N-1 個 process 的快取不會失效**
   => 改完名字，有些請求還是舊的，而**它與「沒生效」長得一樣**
   ⇒ 那一天要改成不快取，或改成帶版本號的 settings 讀取
```

### §3b 後端：87 條分**三個形狀**（互斥，加總 = 87，已自檢）

```
甲 單純代換            **66 條**
乙 「X **以上**」      **15 條**   <= 見下，要特別標
丙 混用模組詞彙        **6 條**    <= 已分出去 `EM13`，本規格**不動它們**
```

#### 🔴 乙：「管理員**以上**」15 條 —— 代換後通順，**而語意靠一個沒寫下來的排序**

```
accounting_export.py:206,460 ／ bonus.py:207,271,395,513
completion_notes.py:763 ／ dev_crm.py:979 ／ module_versions.py:19
quotations.py:1858,1901,4759,5700 ／ shipping_notes.py:733 ／ system.py:265
```

```
f"僅{role_label('admin')}以上可查閱獎金基數。"     ✅ 文法通順
⚠️ 而「以上」這個字隱含   viewer < engineer < sales < admin < superadmin
🔑 **那個排序沒有寫在任何地方，也不隨改名而變。**
   客戶把 sales 改名成「營運總監」，「管理員以上」照樣把他排在下面。
📌 本規格**不解決排序**（那是另一題）—— 但要寫下來，
   否則下一個人會以為 EM2 全綠代表這句話正確。
```

#### 🔴 丙：6 條混用角色與模組詞彙 => **`EM13`，本規格不碰**

```
cashier.py:39              「僅管理員、**出納**或**財務**可查閱」
contractor_vouchers.py:701 「需要管理員或**出納**權限」
material_orders.py:95      「只有管理員或**專案經理**可以修改叫料」
quotations.py:3018         「僅管理員或**出納**可標記收款狀態」
quotations.py:2159         「款項收款狀態需由管理員或**出納**標記」
reports.py:2792            「僅管理員或**出納**可查閱」
```
☠️ 「出納」「財務」「專案經理」**不在五個角色裡**——它們是模組權限。
⇒ `EM2` 若動它們，會變成「**一半動態、一半寫死**」：
   **看起來比現在更一致，其實沒有。**

##### ⚙️ 而這 6 條是自檢抓出來的，不是我看出來的

```
第一版 MODULE_WORDS 含「財務」=> 撈到 accounting_export.py:460「**財務報告**」
=> 三類加總 88 != 母體 87 => **當場亮紅**
🔑 分類器要配一道「三類互斥且加總等於母體」的自檢，
   否則一個報表名會被當成一個模組角色，而清單看起來完全正常。
```

### §3b-2 代換本身

```python
# 改前
raise HTTPException(403, "僅超級管理員可執行此操作")
# 改後
raise HTTPException(403, f"僅{role_label('superadmin')}可執行此操作")
```

⚠️ **逐條改，不要 sed** ——81 條的句型不只一種，其中三條不是「僅 X 可…」：

```
approval_delegates.py:66   「…如需代替他人設定請聯絡最高管理員」
quotations.py:477          「…或聯繫最高管理員」
quotations.py:1905         「案件已結案，僅超級管理員可變更案件進度」
```

### §3c `put_role_labels` 收尾要清快取

```
backend/routers/system.py:2061  _set_setting("role_labels", labels)
                                + invalidate()          <= 新增這一行
```
🔑 驗收：**改完名字重整頁面就生效，不必重啟 server**。

### §3d 前端 5 份對照表改讀端點（**這一包的範圍，不是後續**）

```
approval-queue.html:1402 ／ online-stats.html:368 ／ sidebar.js:296
daily-tasks.html:2999    ／ approval-settings.html:900
```

☠️ **只改後端的話「看起來像修好了」而畫面還是不一致。**

建議做法：`sidebar.js` 本來就每頁載入 ⇒ 由它取一次放進 `window.__roleLabels`，
其餘四處改讀它；取不到時回落 `DEFAULTS`。
⚠️ 回落**不可以是空字串**——取不到要顯示預設中文，不是顯示 `superadmin`。

---

## §4 `EM2` **不使 `PX1` 變得不必要**

```
EM2   訊息**組出**角色名   => 與**設定**一致
PX1   訊息說的角色  vs  **程式實際檢查的**   => 與**程式**一致
```

⚠️ 一句組出來的「需要超級管理員」，若程式檢查的是 `role in ("superadmin","admin")`，
**它仍然是說謊，只是說得比較一致。**
📌 `EM2` 全綠**不構成** `PX1` 的任何證據。

---

## §5 守門

### ✅ 要釘的不變量

> **後端 `HTTPException` 的 `detail` 字面值裡，不出現任何硬寫的角色名稱。**

### 🔴 不可以釘的

```
❌ 「超級管理員是對的」          <= 把預設值當成不變量，正是本題的錯
❌ 「87 條」這個數字             <= 計數會過期；新寫的第 88 條擋不到
```

### ⚙️ 對照組（缺一不可）

```
正對照  §2a 的 **甲 66 ＋ 乙 15 = 81 條** —— **改之前全部要亮**
        🔑 先跑一次看它亮 81 個，再去改；不亮就是掃描器壞了
        ⚠️ 丙那 6 條（`EM13`）**這一包不動**，但守門**仍然會報它們**
           => 要明著放進排除清單並註明「屬 EM13」，
              **不要讓它們看起來像已經修好了**
負對照  backend/helpers/role_labels.py 的 DEFAULTS 那一支定義 **不可以亮**
        （它是名字的來源，不是訊息）
        ⚠️ 排除方式要釘在**檔名＋變數名**，不要用「含有 DEFAULTS 的行」
           —— 後者會把別人的字典一起放行
誘餌    刻意留一條測試用的假訊息（合成的，不要用真缺陷當誘餌），
        確認守門會抓到「新寫的」而不只是「清單裡的」
```

⚠️ **反向控制**：守門若有排除清單，要有一題驗「清單長度沒有變長」——
否則可以靠把 81 條全寫進排除清單變綠。
🔴 而這一包的排除清單**本來就有 6 筆**（`EM13` 那 6 條）
   ⇒ 反向控制要釘的是「**除了那 6 筆以外沒有變長**」，不是「長度為 0」。

---

## §6 驗收（`AC1`：後端＋前端＋頁面三者皆備）

```
① 後端  甲 66 ＋ 乙 15 = **81 條**全部組出（丙 6 條屬 `EM13`，不動）；
        role_labels.py 存在；put 之後 invalidate()
② 前端  5 份對照表改讀端點；回落是預設中文不是 role key
③ 頁面  users.html 改名 superadmin -> 「總經理」後：
        ⓐ 側邊欄在線成員顯示「總經理」
        ⓑ 簽核佇列顯示「總經理」
        ⓒ 打一支 403 端點，detail 回「僅總經理可執行此操作」
        🔑 ⓒ **不重啟 server** —— 那是 invalidate() 的驗收
```

### 💰 已量過的成本

```
現有測試會紅的 = **0 支**（superadmin 與 admin **分兩次量過**）
  superadmin  254 支測試的 Assert 節點裡角色名稱 13 處，**全是失敗訊息**
  admin       同樣掃法 6 處：5 處失敗訊息
                            1 處是**被斷言的值**
                            （test_approval_reminder_ladder_2026_09_22.py:465）
                            🔑 而那一句在**信件範本**裡，不在這 87 條之內
```
⚠️ 分兩次量是刻意的 —— 第一次的「0 支」**只對 29 條成立**。
🔑 〈證據的適用範圍〉：**一個結論的適用範圍，跟它當初是對哪一批量的一樣大。**

---

## §7 我沒查什麼

```
① ~~其餘四個角色只查了 superadmin~~ => ✅ **已補量（2026-09-23）**

```
admin    「管理員」  = **58 條**   🔴 是 superadmin 的兩倍
sales    「業務」    = 4 條，其中 **3 條是誤報**：
                      dev_crm.py:92「無**業務開發**模組權限」=> 模組名
                      case_action_items.py:206/211           => 職務／權限名
                      ⚠️ quotations.py:38「其他業務的報價單」待裁
engineer 「工程師」  = **0 條**
viewer   「檢視者」  = **0 條**
──
⇒ 真實母體 **29 + 58 = 87 條**（範圍要不要擴到 87，等 A 裁）
```

🔑 **兩邊都錯，而且方向相反**：admin 少報 58、sales 多報 3。
⇒ 這就是為什麼「不要從 29 條推它的規模」——推出來的**不是偏大就是偏小，沒有一次是對的**。

⚙️ 子字串：先把三個 superadmin 詞切掉再找「管理員」，否則那 29 條會被重複計。

💰 **58 條的測試成本也補量了 = 0 支會紅**
```
254 支測試裡，「管理員」（已切 superadmin）出現在 Assert 節點內 6 處
  5 處是 assert 的**失敗訊息**
  1 處是**被斷言的值** —— test_approval_reminder_ladder_2026_09_22.py:465
     assert "目前已同步通知系統管理員協助處理" not in html
  🔑 而那一句在**信件範本**裡，**不在這 58 條（HTTPException detail）之內**
  ⇒ 不受 EM2 影響
```

### ⚠️ 而那一支測試指出了一條我沒查的線

```
信件範本裡有角色名稱，而且已經有一道守門釘著「那句話不可以出現」
⇒ §7 ③（PDF／信件範本）**不是理論上的可能，是已經證實存在**
⚠️ 而它的守門釘的是「不出現」 —— 若 EM2 把信件也改成動態組出，
   那道守門**不會翻面**，它會用一個很有道理的訊息擋住正確實作
```

### 🔴 另一類：同一句話裡混用**角色**與**模組**詞彙

```
cashier.py:39              「僅管理員、**出納**或**財務**可查閱」
contractor_vouchers.py:701 「需要管理員或**出納**權限」
quotations.py:3018         「僅管理員或**出納**可標記收款狀態」
reports.py:2792            「僅管理員或**出納**可查閱」
quotations.py:2159         「款項收款狀態需由管理員或**出納**標記」
```
⚠️ 「出納」「財務」**不在五個角色裡**，它們是模組權限。
⇒ `EM2` 改完這幾句會變成「一半動態、一半寫死」，
   **看起來比現在更一致，其實沒有** —— 建議另開編號。

### ⚠️ admin 那 58 條有兩個形狀

```
甲 「僅管理員可…」「需要管理員權限」        => 直接代換
乙 「僅管理員**以上**可…」（約 15 條）      => 是**層級**表達，不是單一角色
   代換後仍然通順，但它隱含一個排序
   （viewer < engineer < sales < admin < superadmin），
   ⚠️ 而那個排序**沒有寫在任何地方**，也不隨改名而變
```
② 前端**非對照表**形式的硬寫（例如 bonus.html:150 的
   「新增獎金項目只有<strong>最高管理員（superadmin）</strong>做得到」）
   —— 那是散文不是對照表，這一份規格**沒有涵蓋它**
③ PDF／信件範本裡的角色名稱
```
