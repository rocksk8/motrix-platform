...
### 🔴 `requirements.txt` 的改動步驟（`JV3` 的一個明確步驟）

```
1. 在 requirements.txt 末尾加一行：  pypdf>=6.0.0
   （用範圍宣告，不要釘死版本 —— 〈版本適配：不可變成孤兒〉）
2. ☠️ **註解必須是純 ASCII**
   檔內 HC2a 逐字：pip_audit 用系統編碼（此機 cp932）讀它，
   **一個 CJK 註解就讓整個弱點掃描從來沒跑過，而且是靜默的**
   ⇒ 這一行要加的說明也寫英文，或**不寫註解**
3. 正式機要 pip install -r ⇒ 部署說明（DEPLOY.md）要提到多一個套件
```

⚠️ **相依在 `JV3` 加、而消費者在 `JV5`（合併輸出）** ——
中間這段期間 `pypdf` 是一個沒有人 import 的套件。
⇒ 若 `JV5` 延後，**這一行要一起延後**，不要留一個沒人用的相依。
