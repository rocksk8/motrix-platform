...
### ⚠️ 九類的查證強度**分兩級** —— `#3/#4/#5` 沒有結構層證據

```
✅ PRAGMA 實查過（是表的欄位）
   #1 #2 #6 #7 #8 #9
⚠️ PRAGMA **看不到**（是 `quotations.data_json` **裡面的陣列鍵**，不是欄位）
   #3 payment items 的 "invoiceFiles"   （quotations.py:3131/3157）
   #4 materials 的     "files"          （:3208/:3234）
   #5 materials 的     "invoiceFiles"   （:3247 註解逐字：與 files 是**兩個各自獨立的清單**）
```

☠️ ⇒ 對 `#3/#4/#5`，「`PRAGMA` 是權威、原始碼不是」那條**用不上**：
**JSON 內部沒有 schema ⇒ 原始碼就是唯一的權威。**

🔴 而 `#4` 與 `#5` 住在**同一個陣列元素**的兩個鍵（`files` 與 `invoiceFiles`）
⇒ **拿錯一個，帶進來的是另一種單據的附件，而筆數與格式都正常。**
🔑 那比「清單永遠是空的」難發現得多 —— **它有東西，只是錯的東西。**
⇒ 驗收 `#3/#4/#5` 各一題，觀測點釘在
**「帶進來那一筆的 `source_file_id` == 來源陣列裡那一筆的 id」**，不要只驗筆數。
