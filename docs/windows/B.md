...
| 2026-09-22<br>BK20 | **`backend/main.py`** | `BK20`：`main.py:517` 的 `_ensure_archive_dirs()` 在**模組層**執行 ⇒ 任何在 conftest patch 生效前 `import main` 的路徑，會在**真實雲端硬碟**上建目錄，而 `BK19` 的守門裝在 fixture 裡、跑的時候損害已經造成。**只搬一行**：把它移進下面那個 `if os.getenv("MOTRIX_DISABLE_SCHEDULERS") != "1":` 區塊的第一行。⚠️ 正式機與開發機都不會設那個變數 ⇒ **行為逐字相同**；停用排程時（只有測試會停）不建目錄，而那正是要的。🔑 不新增 `@app.on_event`，不動 middleware，不動 import。 | ⏳ 等 A 回覆（C 的 `test_bk20_main_does_not_create_directories_at_import_time` 現在是紅的） |
