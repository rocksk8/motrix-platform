...
```
☠️ **不知道這一條的話，你會看到 10 個紅燈並去追缺陷 —— 那會花掉整整一輪。**
✅ `AC1` 那一支 B 已用離線 worktree 複驗過「**`BN9` 之前就是紅的**」。

### ⚙️ 量基準的工具（今天用了四次，而它不在任何文件裡）
```
...
python -m pytest … --basetemp=<路徑>-adhoc  # ⚠️ conftest **強制**要 --basetemp，
                                           #    不給會直接 ERROR
```
🔑 不知道這個的話，「要不要宣告凍結」會變成**每一次量基準的前置談判**。

### ⚠️ 兩個「**不要修**」的已知狀態（避免你去「修好」它）
```
TermsIn（payment_requests）**只有四欄**  刻意 —— docstring 明寫「比照報價單的四個條件欄位」，
                                        afterSales 是報價單的條款，**請款單不帶**
A-2 的 16 組重複清單（faf8df7）         是**母體**不是缺陷數，
                                        **逐組是否刻意，沒有人查過**
```

### 🔴 `BN9` 留下的形狀：**一把旗標兼兩用**
```
is_manager 原本同時決定「**看得到什麼**」與「**按得到什麼**」
=> 收緊的那一刻，admin 的「產生獎金單」入口會跟著消失，**而沒有人要求過那件事**
⇒ 已拆成兩個：is_manager（可見範圍）／can_create_award（入口）
```
☠️ **`BN8` 會踩到同一個**：獎金單成為第九個 doc type ⇒ 需要**第三個**旗標（能不能簽核）。
**不可以再借用 `is_manager` 或 `can_create_award` 其中之一** —— 那正是 `BN9` 修掉的形狀。

### 📌 `AS3` 的字串（三個東西都叫 voucher，別搞混）
```
doc type = `voucher`
label    = 「**傳票（會計）**」**不是**「傳票」
=> 與後端 APPROVAL_DOC_TYPE_LABELS 一致，前端 docTypeMeta 也已對齊
```
...
| 2026-09-22<br>BK20 | **`backend/main.py`** | `BK20`：`main.py:517` 的 `_ensure_archive_dirs()` 在**模組層**執行 ⇒ 任何在 conftest patch 生效前 `import main` 的路徑，會在**真實雲端硬碟**上建目錄，而 `BK19` 的守門裝在 fixture 裡、跑的時候損害已經造成。**只搬一行**：把它移進下面那個 `if os.getenv("MOTRIX_DISABLE_SCHEDULERS") != "1":` 區塊的第一行。⚠️ 正式機與開發機都不會設那個變數 ⇒ **行為逐字相同**；停用排程時（只有測試會停）不建目錄，而那正是要的。🔑 不新增 `@app.on_event`，不動 middleware，不動 import。 | ✅ **准**（A 2026-09-22：「只搬那一行，不新增 on_event、不動 middleware、不動 import」）。已動 `3cae048`，測試轉綠 |
| 2026-09-22<br>GB1 | **`backend/db.py`** | `GB1`：計數器要有落點 ⇒ 新增 `geocode_usage` 表（`month`／`source`／`count`／`updated_at`，`UNIQUE(month, source)`）。**migration 編號講死：`_m091_geocode_usage`，`CURRENT_VERSION` 90→91**（目前最後一支是 `_m090_quotation_location`）。⚠️ 協定 §3：兩人同時加 migration，git 不會衝突、只會在執行時撞版本號 —— 所以編號寫在宣告裡，不是靠 diff。只新增一張表，不動任何既有表、不動 `_get_version`。 | ✅ **准**（A 2026-09-22：「編號歸你，這一輪沒有第二支 migration」；A-2 另複核 `CURRENT_VERSION = 90`、`_m090` 是最後一支）。已動 `10d7397` |
| 2026-09-22<br>MN3 | **`frontend/static/sidebar.js`** | `MN1`／`MN2`／`MN4`：把 `簽核佇列`／`簽核代理人`／`簽核歷史` 三行從 `sec('業務')` 底下移到 `sec('工作內容')` 底下，並把該組名稱改為 **「我的工作」**（`MN2`，使用者已裁示，A 2026-09-22 轉達）。⚠️ **只搬位置與改組名，三行的顯示條件 `cQ`、`pg()` 路由、badge 的 `id`、圖示鍵一個字都不動**——搬家順手改權限的話，一批人會忽然看不到簽核佇列，而那個症狀離「選單重整」非常遠。 | ⏳ 等 A 回覆 |
| 2026-09-22<br>頂欄 | **`frontend/static/sidebar.js`** | 使用者回報「地圖模組最上方的 logo 搜尋列這些通知整個欄位都無顯示」。成因是 `buildTopbar()` 的 `if (!el) return` —— 掛載點不在就**安靜地什麼都不做**，而 `map.html` 從來沒有那個掛載點（`6e4dcc9` 建頁時漏的）。**只改那一個 early return**：找不到 `#app-topbar` 時，若 `<body>` 有 `data-no-topbar` 就照舊安靜返回；**沒有那個宣告就 `console.error` 一行**（含頁面路徑）。⚠️ 不丟例外（login 頁是合法的沒有），不動 `buildTopbar()` 其餘任何一行。 | ✅ **A 19:00 直接派工**（本則即指派，非事後追認） |
...
# `P0-00` 前置 · `apply_update.ps1` 出口盤點

A 於 `STATE.md §30a` 裁示：B 先做不需要紅燈的那一半 —— 出口盤點，
**C 要寫紅燈本來就需要這張表**。本節唯讀量測產出，未修改任何程式碼。

量測基準：`8387479`。守門的正則**從碼裡抓出來**不是手抄
（`deploy_dashboard.py:186`　`r"更新失敗|已自動回滾|\[FAIL\]"`）。

## 🔴 一、「出口」有 14 條，而 `grep exit` 只看得到 5 條

```
字面 exit N      5
Fail "..." 呼叫點 8   ← 都走同一行 exit 1，**而條件與印出的字各不相同**
掉出尾端（隱含 0） 1   ← 成功走這一條
                ──
                14
```
☠️ **只 grep `exit` 會少算 9 條。** 對守門而言，`Fail` 的每一個呼叫點是
不同的出口：它們印不同的訊息，而守門判的正是訊息。
🔑 〈判準的寬窄都會騙人〉：太窄的判準給你一個乾淨而錯的數字。

## 二、盤點結果（守門看得見 10／看不見 4）

| 行 | 種類 | exit | 條件 | 守門 |
|---|---|---|---|---|
| 116 | `Fail()` 本體 | 1 | —— | ✅ `[FAIL]` |
| 131 | `Fail` | 1 | 不是在正式機上跑 | ✅ `[FAIL]` |
| **140** | `exit 0` | **0** | `-CheckOnly` 且 ping 成功 | 🔴 看不見 |
| **143** | `exit 1` | 1 | `-CheckOnly` 且 ping 失敗 | 🔴 看不見 |
| 148 | `Fail` | 1 | 缺 `-PackagePath` | ✅ `[FAIL]` |
| 151 | `Fail` | 1 | 包路徑不存在 | ✅ `[FAIL]` |
| 155 | `Fail` | 1 | 包裡沒有 `deploy_manifest.json` | ✅ `[FAIL]` |
| 172 | `Fail` | 1 | 與正式機目前版本相同（重複套用） | ✅ `[FAIL]` |
| 228 | `Fail` | 1 | 升級前 db 備份失敗（**正式庫未被觸碰**） | ✅ `[FAIL]` |
| 276 | `Fail` | 1 | migration 乾跑失敗（**正式庫未被觸碰**） | ✅ `[FAIL]` |
| 311 | `Fail` | 1 | 使用者取消 | ✅ `[FAIL]` |
| **545** | `exit 0` | **0** | 🔴 **健康檢查異常＋`-SkipAutoRollback`** | 🔴 看不見 |
| 621 | `exit 1` | 1 | 健康檢查失敗→**已自動回滾** | ✅ `更新失敗` |
| **646** | 掉出尾端 | **0** | 正常成功 | 🔴 看不見 |
...
L545  exit 0  **健康檢查異常而沒有回滾** ⇒ 🔴 **這一條是 P0-00**
```
📌 `deploy_dashboard.py:191` `if action in ("deploy", "rollback")` ⇒
`check-only` 那兩條**從來不會產生歷史列** ⇒ 它們的「看不見」不造成後果。
⇒ **14 條出口裡，會被錯記成「成功」的只有 `L545` 一條。**

⚠️ `L545` 的行號：A 在 `§30a` 寫 `:546`，**實際是 `:545`**
（`sed -n '543,547p'` ⇒ 543 是那行黃字 Write-Host、544 是 `}`、**545 是 `exit 0`**、546 是 `} else {`）。
差一行，而 C 要照這張表釘題 ⇒ 先更正再用。

## 🔴 四、而我的盤點工具**第一版自己出了一個假陽性**，留著

```
v1 報      L621（健康檢查失敗→自動回滾）「守門看不見」
實際       它在 :597 印「更新失敗，已自動回滾到套用前版本」⇒ 守門中得很漂亮
成因       v1 的抽取**往回走到第一個非輸出行就停**，而 L621 之前隔著
           一段 `Get-NetTCPConnection` 的診斷碼 ⇒ 撈不到 :597 那句
```
☠️ **一條真的被守住的出口被報成盲點** —— 方向與〈盤點工具的正對照〉平常
擔心的相反：**假陽性，而它會讓人去修一個不存在的洞。**
🔑 而 v1 **有正對照而且過了** —— 因為我餵給它的是一個**手寫字串**：
```
v1 的正對照   驗的是「正則會不會動」        ✅ 過了
沒有人驗的    「抽取有沒有把該條出口真正印的字撈出來」
```
📌 ⇒ 判準升級：**管線有幾個階段，就要有幾個對照。**
v2 對抽取也釘了一個：`L621` 抽出來的字**必須**含「更新失敗」，撈不到就整份作廢。
⚠️ 而 v2 的抽取**刻意偏寬**（往回掃到上一個出口為止）：偏寬會把守門說成
「看得見」⇒ 錯的方向是**低報盲點** ⇒ 表裡標 ✅ 的可以信，標 🔴 的我已逐條看過。

## 五、C 寫紅燈時可以直接用的三件

```
① 反向控制的落點    L545 是唯一「exit 0 ＋ 守門看不見 ＋ 會寫 history」的出口
② 正對照的落點      L621（exit 1 ＋ 守門看得見『更新失敗』）——
                    ⚠️ 釘它不要釘「守門有沒有抓到」，要釘**不變量**：
                    「健康檢查沒過 ⇒ 歷史那一列不可以是 succeeded」
③ 不要釘的          `[FAIL]` 那 8 條 Fail 呼叫點的字面訊息 ——
                    那是散文，它們會被改，而改了不代表行為變了
```

## 六、`P0-00` 的修法（A 已裁，等 C 紅燈）

```
1 apply_update.ps1  **每一條出口（含成功）**印一行 `::RESULT:: status=... rolled_back=... exit=...`
2 deploy_dashboard.py 改讀那一行做判定
3 🔴 **讀不到那一行＝失敗（fail-closed）**
  現在是「沒比對到＝成功」＝ fail-open，而「新增出口時忘記印」是**預設會發生**的事
4 舊的關鍵字比對**留著**當第二道，不要拿掉
```
### 🔴 生效時點 —— **上面那段曾經是錯的，更正在下面，錯的那一列留著**

```
❌ 曾經寫（A 20:2x，我照收並寫進這份檔）
   「正式機跑的是正式機上那一份 apply_update.ps1
    ⇒ ps1 的改動要到『下一包部署完成之後的那一次部署』才生效」
✅ 實際（A 20:4x 自己更正，我逐行查證屬實）
   **ps1 的改動在下一次部署就生效，不用等兩次。**
```
查證（`_dashboard_remote.ps1`，我自己讀的行）：
```
:99-107  呼叫 apply_update.ps1 **之前**，先把**套件裡的** backend\tools\*
         整個 Copy-Item 覆蓋到正式機 $Root\backend\tools
:131     才執行 $Root\backend\tools\apply_update.ps1
⇒ 執行到的就是**套件裡那一份**
```
📌 A 錯在哪（它自己指出的）：引用 2026-09-08「舊版正式機沒有 `-SkipAutoRollback`
這個參數」當證據，**而那次事件的修復就是把這段預先複製加上去的**
（`:92-97` 的註解逐字寫著）⇒ 拿一個症狀當證據，而那個症狀的成因早就被移掉了。
🔑 〈證據的適用範圍〉：拿到證據要先問**它是什麼時候、對哪個版本量的**。

⇒ **對實作是好消息，範圍變小**：
```
✅ fail-closed 直接上，不需要為「舊 ps1 不會印結果行」做退路
✅ 交付說明**不用**寫「這半要等下一次才生效」
⚠️ 兩邊仍然都要改（ps1 印、dashboard 讀），只做一半沒有意義
```

### ⚠️ 而我查證時另外看到一件，A 的更正裡沒有

`:80-91` 的註解逐字寫著：**一次失敗並自動回滾的部署，會把 Step 3 複製進去的
新工具腳本用套用前快照「蓋回去」。**
```
⇒ 失敗回滾之後，正式機的 apply_update.ps1 會退回舊版
⇒ 而**下一次部署開頭的預先複製會再把它覆蓋成新的** ⇒ 自己會好
```
📌 所以它**不是缺陷**，但要寫在這裡：**不要有人針對「回滾後 ps1 是舊的」
去設計補償邏輯** —— 那是一個會自己消失的狀態。

## 七、A 併進 `P0-00` 的兩件（20:4x，原本不在裡面）

### 🔴 ① `Test-Path $srcTools` 為 false 時要 `Fail`，不可以 skip

`_dashboard_remote.ps1:102-105`：
```powershell
if (Test-Path $srcTools) {
    Copy-Item -Path (Join-Path $srcTools "*") -Destination $dstTools -Recurse -Force
}
```
☠️ **沒有 `else`** ⇒ 包裡沒有 `backend\tools\` 時這段**安靜跳過**，
正式機就用舊的 ps1 跑，**而畫面上什麼都不會講**。
📌 與 `rg6.py` 第 ⑥ 項**完全同一個形狀**：一個缺檔讓一道關卡安靜消失。
⇒ 包裡沒有工具腳本**不是一種正常情況** ⇒ `Fail`。

### ⚠️ ② `::PROTOCOL:: v=2` 握手 —— **做，但理由換了**

```
❌ 原本的理由  相容舊版正式機上的 ps1   ← 已不成立（見上面的更正）
✅ 現在的理由  **讓上面那個「安靜跳過」變成看得見的**
```
`apply_update.ps1` 開始套用時印一行 `::PROTOCOL:: v=2`；
dashboard 收不到 ⇒ 表示「跑的不是我送過去那一份」⇒ **畫面要明著標出來**。
🔑 沒有那一句「明著標」，退化又是靜默的 —— **而靜默正是要修的東西。**
