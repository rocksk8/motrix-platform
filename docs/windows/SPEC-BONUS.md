...
- **上線即啟用**：使用者原文「獎金分潤模組上傳到正式機就自動啟用」⇒ `BONUS_MODULE_ENABLED` 預設改為開（仍可用環境變數明確關閉）；相依的模組旗標題翻面並記原話。
- **C1（出納可見性）**：使用者原文「待發放／已發放的整張，不含淨利與比率」⇒ 出納模組持有者在待發放、已發放可見每人金額、合計、尾差；**後端**濾掉淨利、比率、三類比例；草稿、待審核不可見。
- **M1（頁面入口）**：使用者原文「任何登入者都能打開，內容照規則過濾」⇒ `bonus.html` 不受頁面層模組守門、選單入口對所有登入者顯示；資料範圍完全由 API 過濾（§11.4、C1）；無資料者顯示空狀態。

## 11.8 入帳傳票（AC3，2026-09-24，依法規預設）
- 進入**待發放**時自動產生一張**轉帳傳票草稿**：借 6111 薪資支出（合計發放額）／貸 2191 應付薪資；摘要帶案號；傳票走傳票自己的簽核（JV30），不自動過帳。
