...
## 🟢 BN22 獎金分潤調整（原暫稱 BN20，與 09-23 SPEC-BN20 撞號，改名）（2026-09-25 02:4x 使用者：「獎金分潤改%數要能即時同步，並且已經帶入的業務跟專案，需要能調整人員」；執行 hichan-8d）
- **即時同步**：案件獎金比例（預設 10%）與業務／專案／後勤分配比例（50/30/20）一改，獎金總額、各組金額、每人金額立即重算顯示（不必先存檔）；存檔時後端以同一算式重算為準（前後端一致有題）。
- **調整人員**：已帶入的業務、專案（執行負責）可增、刪、換人；**只改這張獎金單，不回寫案件**；每次變更留紀錄（誰、何時、前後名單）。
- **可調狀態（使用者選「核准前都能改」）**：草稿與簽核中都可改；**簽核中改了人員或比例 ⇒ 已簽的簽核作廢、需重新簽**（畫面明確提示）；已核准／待發放／已發放不可改。
- 規則沿用 §11.7：同類平均下限、尾差留公司；比例合計≠100% 擋存並說明。
- 先交計畫（改哪些端點與畫面、簽核重置怎麼做）再動手。
- BN22 裁示（hichan-0a）：即時重算走後端 preview 端點（同一 allocate()）；簽核中改動 ⇒ 維持待審核、清空簽核重簽；沒人簽過直接存不跳確認但記 edit_log。
...
## 🟢 PERF 測試與建包 CPU／硬碟優化（2026-09-25 04:0x；使用者：「先處理 CPU 跟硬碟的問題，最後弄完再匯出升級檔」「你決定即可，最後告訴我優化那些」）
使用者選：1 範本庫、5 共用瀏覽器＋token 登入、3 e2e -n 4＋建包獨佔。hichan-0a 加：6 擋圖片／固定等待、7 同 tree 沿用全綠。不做：8 選題器（漏驗風險最高）。
| 項 | 內容 | 執行 |
|---|---|---|
| 0 | 瀏覽器端連外守門（mp0/1/8 真的連 OSM） | hichan-a3 |
| 2 | 建包 e2e 逾時改擋下 | hichan-a3 |
| 1 | 測試 DB 用範本複製（每輪少寫約 22 GB） | hichan-bf（最先） |
| 4 | 建包時獨佔兩格測試名額 | hichan-bf |
| 7 | 同 tree 已全綠沿用、不重跑 | hichan-bf |
| 5 | e2e 共用瀏覽器與伺服器＋token 登入 | hichan-a3 |
...
## 🟢 JV-LAYOUT 傳票帶入來源版型（2026-09-25 14:1x 使用者示意圖選擇；取代 4963170 右側並排；執行 hichan-bf；本包最後一項）
- 使用者原話：「傳票的部分目前案件名稱全部在右邊，摘要來源也在右邊，改到頁面科目的下方左邊顯示案件名稱可點選、右邊顯示已上傳檔案，點選已上傳檔案的時候跳出預覽這張照片的視窗做確認是否帶入，在附件的時候也顯示預覽圖」。
- 選定版型 A：分錄下方「帶入來源」左案件清單、右已上傳檔案縮圖；附件區顯示縮圖。
- 預覽視窗：大圖置中、◀▶ 切換同案檔案、顯示單號與上傳日、「取消／帶入附件」、✕；附件區點開同視窗只看不帶入。
- 使用者：「等傳票這項一起出」⇒ 下一包等這項完成（連同共用瀏覽器 event loop 修正）。

## ✅ 升級檔 20260925_151234_2220aedb（2026-09-25 15:12）——備妥，未部署
- 非 e2e 3206 passed（396 s）；e2e -n 4 361 passed（244 s）；整包 662 s（≈11 分，原 35 分）。包內無 *.db／docs/windows／tests。
- 內容（相對正式機 3a66611）：傳票帶入來源版型 A、BN22 獎金分潤、EX-SIGN、簽核設定、lost update ~30 處＋鎖釋放（W-6 成因之一）、存檔被吞、登入 TOTP Enter、提醒橫幅、地圖穿模、測試與建包優化全部。
- ⚠ 自動清理只留 2 份：3a66611（正式機現版）的升級檔已被刪（回滾靠正式機自己的 rollback_snapshots）。部署後更新 _prod_baseline.py。
