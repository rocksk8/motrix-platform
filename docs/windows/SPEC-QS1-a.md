...
### 🔴🔴 更正（2026-09-23 同日）：**「印一筆」要印到哪裡 —— 見 `SPEC-MG4`**

```
⚙️ B 已照本節實作 `_m103`（`db.py:4305`），而它把 N／M／K 全部寫進 `logger.warning`
🔴 而 `EM7 §6` 同一天量到：`logs/server.log` 的**消費端 = 0**
☠️ ⇒ **本節要求的回報，沒有人會看到。**
```
> ### 🔑 **一個判準在 `EM7` 確立，而我沒有回頭套用到這一份。**

```
⇒ 落點改由 `SPEC-MG4` 定義（`migration_log` 表 ＋ superadmin 端點）
⚠️ 而本節下面那段**仍然成立** —— 要印什麼沒有變，變的是印到哪裡
🔴 而驗收要跟著改：從「log 裡有」改成「**`migration_log` 裡有**」
```
...
② ✅ **已查：0 筆不一致 ⇒ 以 `sales_person_id` 為準**（不是選項）
   ⚙️ 26 張：兩者都有且一致 **14** ／**不一致 0** ／只有 FK **9** ／都沒有 **3**
   🔑 而理由**不是「它們今天一致」**（那是這份資料的性質，不是機制的性質）——
      是 **FK 是本體，`data_json` 是反正規化的副本**，而副本不該贏過本體
      ☠️ 而「有參照完整性」那個說法**在這個 repo 站不住**：
         `db.py:169` 的 `PRAGMA foreign_keys=ON` 包在 `try/except` 裡（失敗靜默），
         demo 重置路徑**明著關掉它**（`db.py:4952` 的註解逐字寫著）
   ⚠️ 而「兩者都有而不一致時要說出來」是**一道還沒做的守門**（A 另排）——
      ☠️ 它上線那天一定是綠的（今天 0 筆）⇒ **誘餌是必要的不是加分項**
   ~~原本的待確認：~~ **`data_json.salesPersonUsername` 與 `sales_person_id` 誰為準**
