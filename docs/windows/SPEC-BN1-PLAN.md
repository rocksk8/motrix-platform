...
沒有任何一處擋這兩件。**而兩件的症狀相反**（2026-09-23 A-2 `import` 產品碼實跑，
`base = 123456`）：

```
Σperson_pct > 10000   兩人各 10000
  pool=61728  兩人各拿 61728（各拿走整池）  remainder_of() = **-61728**
  => **負數，看得見**

total_pct > 10000     total_pct=20000（200%）
  pool=**246912**（= base × 2）  一人拿 246912  remainder_of() = **0**
  => **獎金是淨利的兩倍，而每一個既有檢查都說它是健康的**
```

📌 而 `split_award()` 的 docstring 寫著這個不變量：
> 「不變量：`Σamount <= pool`，且 `pool - Σamount < 人數`。
> **大於人數表示那不是捨入誤差，是算式錯了。**」

☠️ **實跑：`total_pct=20000` 時這兩條都成立**（`246912 <= 246912`、餘 `0 < 1`）。
🔑 **因為 `pool` 本身已經被撐大了 —— 不變量是拿 `pool` 當基準，
而出問題的正是 `pool`。** 一個以受污染的值為基準的檢查，永遠不會發現污染。

⇒ **上界必須釘在 `base` 上，不是釘在 `pool` 上**：`total_pct <= 10000`。

⚠️ **而負的 `total_pct` 也沒人擋**：`pool_for(base, -5000) = -61728`。
（相對地 `base` 那一側**有擋**：`base_amount_for` 對淨利 `-5000` 與 `0`
