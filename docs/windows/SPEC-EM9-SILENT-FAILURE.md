...
```
frontend/**/*.{html,js}（排除 rollback）  fetch( 呼叫點 **606** 個，81 個檔
  ⚠️ 初版寫 608，那是用 `\bfetch\(` 數的 ——
     `module-versions.html` 有 **3 處 `this.fetch()`**（頁面自己的重載方法），
     而 `.` 是 `\b` 認得的字界 ⇒ 被當成瀏覽器 API 收進來
     🔑 正確的 pattern 是 `(?<![.\w])fetch\(`
  ✅ 而它**沒有污染下面的 69／42** —— 自訂 `fetch` 不帶 `method:`，
     全被歸成 GET 排掉了（實際查過，不是推的）
...
## §7b 🔴 射程宣告：`EM9` **不涵蓋「寫入成功而什麼都沒寫到」**

> 加入時機：2026-09-23，`QS1` 追查時由 D 指出、A 轉達。
> ☠️ **不擴母體** —— 這一節只寫清楚 `EM9` 全綠**證明不了什麼**。

```
`EM9` 的母體是「**寫入失敗，而沒有失敗分支**」
=> 它找的是「**有一個失敗訊號，而沒有人接**」
```
...
```
UPDATE quotations SET sales_person_id = (SELECT id FROM users WHERE display_name = …)
WHERE …
=> 子查詢比對不到 => **SQL 正常執行完、不拋例外、回報成功、0 筆受影響**
☠️ **它沒有失敗** ⇒ 任何一個「找失敗」的掃描都看不到它
```

> ### 🔑 **後者比較危險，因為它連一個可以被吞掉的訊號都沒有產生。**

```
兩者的母體判準完全不同：
   EM9      「有沒有失敗分支」        （掃 try/except、.ok、.catch）
   另一族   「**有沒有人讀 rowcount**」（掃 cur.rowcount / changes()）
🔴 ⇒ 它們是**兩個母體**，一個全綠不代表另一個有人看過
```

📌 **寫在這裡的理由**：
☠️ **否則下一個人會把「`EM9` 全綠」讀成「沒有靜默寫入問題」** ——
而那正是我自己在 `QS1` 裡犯的錯（把兩種合併成一句話）。

⚙️ 實例與處置在 `SPEC-QS1 §2`（`_m010` 回填 0 筆匹配）。
