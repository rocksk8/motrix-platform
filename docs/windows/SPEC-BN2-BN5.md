...
### ☠️ 那一筆差額正是 A 警告的形狀 —— 而它**不是**淨利 ≤ 0

```
已結案且已精算 9 筆，而 netProfit > 0 的只有 **8 筆**
```

🔴 **A-2 初版把那一筆寫成「淨利 ≤ 0」，那是錯的。** 逐筆打開看：

```
MQ-202607-023  10117    MQ-202608-003   6044    MQ-202608-006     326
MQ-202607-028  11242    MQ-202608-004   1041    MQ-202608-007  483813
MQ-202607-126   2342    MQ-202608-005   1484
MQ-EXPFILE-001  **netProfit = None**   ← 差的那一筆
               而它的 settlement.summary 是 **空的 `{}`**，status 卻是 finalized
```
☠️ 成因：我用 `CAST(netProfit AS REAL) > 0` 數，而 `CAST(NULL AS REAL) > 0` 是 NULL
⇒ **數字是對的（8），而我把「沒有值」讀成了「值是 0 或負數」**。
🔑 〈null 不等於 0〉—— 我自己記憶裡的那條，寫在這份規格裡時違反了。

**⇒ 所以下拉要分三種狀態，不是兩種：**
```
① 有 summary 且 netProfit > 0     -> 可選                （今天 8 筆）
② 有 summary 且 netProfit <= 0    -> 標「淨利 0 或負數，不發放」，不可選（今天 **0 筆**）
③ **summary 是空的／缺 netProfit** -> 標「精算未完成」，不可選（今天 **1 筆**）
```
🔴 ②③ **不可以合併成一句**，因為**下一步完全不同**：
```
② 這個案件真的沒賺錢   -> 沒有出路，就是不發
③ 精算資料不完整       -> **有出路**：base_amount_for 已有明確訊息
   「這個案件的精算是舊格式（沒有淨利欄位），無法產生獎金單。
     請重新開啟並儲存一次該案的精算，系統會自動補算淨利後即可發放。」
   ⇒ 下拉直接用那句話，**不要自己寫一句**（規則只有一份）
```
📌 而「列出來不要濾掉」那一條仍然成立，與 `BN1` 的 `ok=false` 同一條規則。
...
標示（不要濾掉），**四種狀態各一句**
  ① netProfit > 0        -> 可選
  ② netProfit <= 0       -> 標「淨利 0 或負數，不發放」，不可選   （今天 0 筆）
  ③ summary 空／缺欄位   -> 標「精算未完成」＋ base_amount_for 那句可操作的話，
                            不可選                                （今天 1 筆）
  ④ 已有有效獎金單       -> 標「已產生（#id）」，**不可選**，而**看得到**
     ⚠️ 作廢之後要能再產一次 ⇒ 判斷用 `voided_at = ''`，不是「有沒有紀錄」
  ☠️ ②③ 不可以合併：一個沒有出路、一個有出路（見上）
空狀態要說得出原因
