...
| 購料發票 | `case_extra_expenses` WHERE `category='材料'` | 3 | ✅ `total_cost`（合計 6,525） |

### ✅ ①「購料發票」= `case_extra_expenses` 的材料費（使用者 2026-09-23 裁）

```
⚙️ 他的原話：「**購料發票指的就是叫料／材料費**」
⚙️ 而「叫料」(`caseRecord.materials[]`) **沒有任何金額欄位** ——
   它是物流追蹤（訂了沒、到了沒、序號），不是支出記錄（`SPEC-JV21 §2b` 實測）
⇒ 兩者裡只有「材料費」有金額 => 對應 case_extra_expenses.category='材料'
```

> ### ~~而不要自己把「材料」當成「購料發票」~~
> ### 📌 **上面那句是我寫的，而使用者推翻了它。留著是因為它的理由仍然成立** ——
> ### 🔑 我當時的理由是「`case_extra_expenses` 是 `JV21` 的來源」，
> ### ☠️ 而那個事實沒有變，**變的是「同一個來源可以餵兩個產出」這件事被裁定了**（見 §3③）。

### 🔴 ②而範圍有一個差額，`JV21` 是全部而本件只有材料

```
case_extra_expenses 的 category 實際值：
   材料 3 筆 6,525   <= **本件只帶這一類**
   運費 2 筆 2,750   ┐
   工時 1 筆 1,200   ├ JV21 帶（它的範圍是「額外支出」全部）
   其他 1 筆    15   ┘
```
⚠️ 依據是他的原話只說了「購料」—— **運費與工時不是購料**。
⇒ 而「其餘三類要不要也能帶入分錄」進 §7①（**不擋動工**，多一類只是多一個 `category`）。

### 🔴 ③ 而 `JV6` 與 `JV21` **讀同一批資料，那是設計不是重複**

```
同一筆 case_extra_expenses 的材料費：
   JV21  ->  **摘要文字**（voucher 的摘要欄，人可再編輯）
   JV6   ->  **分錄列**（voucher_lines：金額 ＋ 科目留空）
```
> ### 🔑 **同一個來源餵兩個產出，是設計不是重複。**
> ### ☠️ 而看到「兩個編號都讀 `case_extra_expenses`」時，
> ### **最省力的反應是把其中一個做成另一個的呼叫** —— ⛔ 不要那樣做。

```
⚠️ 那樣做的後果：兩種產出被綁死
   => 使用者改了摘要文字，分錄的金額跟著變（或反過來）
   => 而 JV7 的裁定是「帶入是**起點不是終點**」，摘要本來就該能改
🔑 判準（本規格 §1 那一條）：**產出物落在哪張表的哪幾欄。**
   兩者落在不同的表／不同的欄 => 它們是兩件事，各自實作。
```
...
### ① ✅ **已裁：「購料發票」= 叫料／材料費**（使用者 2026-09-23）

⇒ 見 §3①。而**留下一格沒問**：

```
❓ case_extra_expenses 的另外三類（運費 2 筆／工時 1 筆／其他 1 筆）
   要不要也能帶入分錄？
我裁（暫行）：**只做「材料」** —— 他的原話只說了購料
依據         運費與工時不是購料；而多帶一類會讓分錄出現他沒預期的列
他若要全部   §5① 的查詢端點拿掉 category 篩選，§6 多一題（帶入運費那一筆）
             —— 結構不用改
```

### ② ✅ **已裁：科目留空，由使用者自己選**（使用者 2026-09-23，與我的暫行裁示相同）

~~原本的待確認：~~ **帶入的分錄，科目代號從哪裡來**
