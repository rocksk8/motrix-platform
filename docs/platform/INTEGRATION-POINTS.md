...
| 欄位 | 內容 |
|---|---|
| 提供方 | M05 `modules/arap/api/invoice_vouchers.py`（`invoice_voucher`）、`modules/arap/api/payment_requests.py`（`payment_request`）；M03 `routers/shipping_notes.py`（`shipping_note`）；M01 `routers/quotations.py`（`quotation`、`case_stage`） |
