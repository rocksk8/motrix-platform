...
| 單位 | 層 | 用途 | 介面 | 使用者 | 契約題 |
|---|---|---|---:|---:|---|
| `plat:catalog` | L0 | 能力目錄（CUSTOMIZATION-SPEC P1；CORE-SPEC §7 端點登錄表的擴充）。 | 12 | 2 | `tests/platform/test_platform_catalog.py` |
...
| `plat:registry` | L0 | L0 模組登錄表（docs/platform/CORE-SPEC.md §4、§5）。 | 21 | 31 | `tests/platform/test_core_loader.py`、`tests/platform/test_module_selection.py` |
...
| `helper:auth` | L1 | Password hashing, session validation, weak-password detection.（無單位卡） | 16 | 50 | — |
...
| `helper:dates` | L1 | Date arithmetic utilities.（無單位卡） | 4 | 8 | — |
...
| `helper:legal_params` | L1 | L1 法規參數服務（R1；規格 CUSTOMIZATION-SPEC §9.1）。（無單位卡） | 27 | 17 | — |
...
| `helper:tax_calc` | L1 | 稅額純函式（L1；2026-09-26 自 M01 `helpers/quotations.py` 下沉，主持核准「T」）。（無單位卡） | 9 | 6 | — |
| `helper:tiered_approval` | L1 | 共用的 tiers 依序簽核純邏輯（2026-08-22）。（無單位卡） | 25 | 15 | — |
