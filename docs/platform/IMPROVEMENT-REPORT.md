...
| IP-21 attachments.for_document（A，wip/a-attachments afcfb513）＋analytics 派工題略過（A，wip/a-analytics-dispatch ef3b9f60，抽查） | D | `AUDIT-D-A-attachments.md`、`AUDIT-D-A-analytics-dispatch.md` | 完成（2026-09-26 17:34）：attachments **必修 1 待裁示**（AT-M1 附件權限只看傳票模組、新契約無 user）、建議 2（§5 b 列已達成未劃、case_update 壞 JSON 無題）；§B-11 通過；已帶入附件 D 探針驗證不受影響；analytics-dispatch 通過 |
| O5-S2-2 遮蔽（B，fd5af159）＋O9 jv7 終點與看門狗（B，wip/b-o9 79b89eef） | D | `AUDIT-D-B-o5-s2.md` §4、`AUDIT-D-B-o9.md` | 完成（2026-09-26 17:46）：S2-S1 關閉（真 Playwright 失敗經完整 pytest 回報、-n 0／-n 2：已知形狀 0 外洩）；O5S2-O2 更正為 D 探針自身錯誤；o9 通過（突變 2/2 紅），觀察：看門狗不含題目本體、-n 0 時 _exit 結束主行程 |
| approval.queue_items／reassign／detail（C，wip/c-approval-2 49dcb781） | D | `AUDIT-D-C-approval.md` | 稽核完成（2026-09-26 18:12）：**必修 2**（AP-M1 覆蓋檢查在 M05 不在時紅——§B-11 多 1 紅、本包造成；AP-M2 存簿不外洩無題驗，突變存活）；轉簽權限未放寬、角標與清單同源、突變 6/7 紅 |
| a-attachments-2 46f0e804 複核（A） | D | `AUDIT-D-A-attachments.md` §4 | 複核（2026-09-26 18:18）：AT-S1、AT-S2 關閉；**AT-M1 未關 ⇒ AT-M1b**（case_documents_readable 對 extra_expense、invoice_voucher 比原單據規則寬，D 探針實測：自己端點 403、經傳票 200；開票 files 權限突變存活）；**A 回覆（2026-09-26 18:38）**：`wip/a-attachments-3` 9973d10b——extra_expense 用 `case_owner_readable`（與額外支出 `_guard_case` 同一支）、invoice_voucher 用 `_voucher_readable`（與 `_guard_voucher` 同一支，含金額層）；探針情境與 V3 已成正式題，突變 5/5 紅；待 D 複核 |
...
| O11 建立端逾時時間線（B，wip/b-o11 55df4ed8，抽查） | D | `AUDIT-D-B-o11.md` | 抽查完成（2026-09-26 18:58）：通過、建議 O11-S1（回應本文前 120 字與 console 原樣印出，基底尚無 redact；去 query 無題守，突變存活）；O11a 突變紅 |
...
| 差異題預設 -n（B，wip/b-modtest-workers 8eb264da） | D | `AUDIT-D-B-modtest-workers.md` | 完成（2026-09-27 01:55）：**必修 2**——WK-M1 只設 E2E=3 時差異題仍 -n 2（partial_cap 取 min，PARTIAL 預設 2）；WK-M2 列車 run_train ① 不經 default_workers（TR11 差異題實測無 -n、串行）；建議 1（呼叫端守門是字串比對，補完丟掉的突變存活）。函式與 no:xdist／已帶 -n 行為正確、突變 4/5 紅 |
| WK 複核（B，wip/b-modtest-workers-2 1cb53e52） | D | `AUDIT-D-B-modtest-workers.md` §3 | 完成（2026-09-27 02:07）：**WK-M1、WK-S1 關閉**（突變 WM1、WS1～3、WG1 紅）；新守門以 TR11 真實 run_train 驗證：-3 未改 ⇒ 紅、改成 partial_pytest_args ⇒ 綠；觀察 2（守門只認位置參數 picked；PARTIAL<2 時的等價突變）；WK-M2 待 -3 |
...
| M01 ④（C，wip/c-m01-4 221adaa0） | D | `AUDIT-D-C-m01-4.md` | 完成（2026-09-27 03:07）：**必修 3**——M4-M1 SO e2e 只驗模型（提示藏掉、選單不停用存活）且正對照在真刪樹必紅；M4-M2 §2-C 只做 SO（網路規劃頁案件選單同型靜默空清單）；M4-M3 tests/platform 以外未處置：非 e2e 134 檔 701 項（modules 36／87、tests 98／614）、e2e 78 檔 166 項，M01 在時全過（1256／206）——主持已裁示 ④ 內處置、併 -5。CA3-M1 關閉；tests/platform 真刪只剩允許 5；(a)(b) 突變紅；建議 2、觀察 4（approval-queue 屬 M01 ⇒ 無 M01 即無待簽彙整） |
