...
| S-CC07 更正 2（稽核確認 N-1，2026-09-26） | ⚠ 上一列的②只抓「今天以外全部過期」⇒ 往前跳但**小於**保留天數時不觸發，真實快照照刪、沒有告警（稽核實測 30 份，跳 3／10／29 天 ⇒ 刪 2／9／23 份）。主持裁示（CORE-SPEC）：③**最新一份（今天以外）距今天超過 2 天** ⇒ 同樣暫停、寫標記、告警。每日層（本機、雲端每日、個資每日）＝2 天；週層＝14 天、月層＝62 天（正常間隔本來就超過 2 天，用 2 個週期）。代價：正常停機超過 2 天也會暫停並每輪告警，要人刪標記才恢復（寧可多告警，不靜默刪） | `test_state_cc07_small_clock_jump_holds_pruning_every_day[3/10/29]`（連跑 10 天：真實快照一份不刪、每天 ERROR）、`…_two_day_gap_is_still_normal`（邊界） | 拿掉③ ⇒ 3 紅；本機層沒帶門檻 ⇒ 3 紅；邊界改 ≥2 ⇒ 1 紅 | V9 另派代理修（主持） |
