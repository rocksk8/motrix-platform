...
| # | 項目 | 依據 | 負責 | 狀態 |
|---|---|---|---|---|
| R1 | 法規參數依**生效日**版本化（L1 法規參數服務）：勞報單扣繳、補充保費門檻、最低工資；舊單沿用建立時的版本；跨年前提示「新年度規則未設定」；加守門「兼職補充保費門檻＝當年最低工資」 | 扣繳率標準、健保署簡表、勞動部 | R | ✅ CUSTOMIZATION-SPEC §7.1 |
| R2 | 零稅率、免稅必填依據或附件 | 營業稅法 §7、§8 | R | ✅ §7.2（依據＝下拉＋說明；附件未做） |
| R3 | 個資蒐集告知：公司設定頁提供告知文字範本，承攬商與勞報單表單列印或勾選 | 個資法 §8 | R | ✅ §7.3（列印告知書＋「已告知」紀錄） |
