...
| ④ SO＋文件＋補題 | M01 不在 ⇒ 每日工作頁明說「需要案件模組」；module.json／README／CHANGELOG／SPEC；modules.json 的 M01 成員改成 `mod:case/...`；**補題（主持 2026-09-26 19:42，D 觀察）**：(a) 詳情遮蔽的「看 dataUrl」那一層：id 不是 passbook 但帶 dataUrl 的檔案也要被拿掉；(b) M05 不在時 L1 簽核鏈讀不出來會擋下的那條路 | e2e 驗提示；突變拿掉判斷 ⇒ 紅；(a)(b) 各一題＋突變 ；**(c) sparse 真刪 M01 抽樣（2026-09-26 21:35，93b90c0b，只跑 tests/platform）**：1145 過、71 紅／錯＝允許 5（產生檔 3＋unit_index＋modules_json 列不存在）＋66 題分在 20 檔（test_case_stage_connectors 14 錯、tax_calc 9、approval_providers 9、case_recognition 8、supply_connectors 5、case_summary_locations 4、case_access_l1 3、sink2 2、m01_l1_providers 2、其餘各 1）⇒ **主持裁示 21:3x**：優先搬進 `modules/case/tests/`；只有題目本身就是「驗 M01 的行為」才用 `module_installed` 略過，略過要附理由並列清單（④ 交 D 的稽核重點）；屬於「L1 在 M01 不在時要照常」的題**不可以略過**，要改成驗 M01 不在時的行為 |
