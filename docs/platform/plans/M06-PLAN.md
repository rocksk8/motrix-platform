...
| a' | `modules/accounting/api/vouchers.py` | `contractor_dispatches`、`vendor_contractors`（M04） | C 在 IP-15 新增「成本檢視」（到期能力暫記 `dispatch.cost_for_case`，C 定名後同步改守門） | 〔主持裁示（2026-09-26 19:09）：不同意「403 就不列」（〈降級之後它還是會動〉：會計會悄悄少列支出）。成本檢視放行 finance／cashier，只回金額、日期、案件、廠商名稱，不回派工細節與個資，只新增不改既有回應；**做好之前 M06 保留直讀，登記為已知例外**（守門 KNOWN_FOREIGN_READS 已恢復 a'，wip/a-m06-2 f72d51dd）。另：任何來源因權限沒列出都要在畫面或回應明說〕 |
| | 〔到期（2026-09-26 23:13，A，wip/a-m06-4）：B 的 b-ip15-cost 提供 `dispatch.cost_for_case`，本包兩處讀取改走它，a' 自到期守門刪除；同時 IP-96 `case.summary` 已在（第十班），a 列的 quotations 也到期、案件清單改走它；a 列只剩 case_extra_expenses〕 | | |
| | 〔主持裁示（2026-09-26 19:37）：JV21 第三層採選項二——成本檢視回**品項描述與金額**，**不回人名**；第三層原本列人名的位置改成「外包人員 N 人」（會計知道有這一項、看不到姓名）。**使用者看得到的行為改變** ⇒ M06 改走成本檢視的那個 commit 同時在 `backend/version_manifest.json` 加一筆 module「傳票」的條目（版號依當時最大號遞增），草稿：「傳票摘要帶入承攬商派工支出時，外包人員改顯示為『外包人員 N 人』，不再列出姓名；品項描述與金額照常。」；條目隨實作一起加，不先加（manifest 已出貨條目不可改寫）〕 | | |
