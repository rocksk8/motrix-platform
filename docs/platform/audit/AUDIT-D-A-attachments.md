...
〔20:04 主持裁示 AT6-O1：維持 JV7 現狀、不過濾，登記為**設計界線**，不算缺陷。
- 理由：cashier／finance 做傳票要能挑任何案件（U13，會計已確認照現行）；客戶名依 PN-M1 視為公司，不是個資。
- 界線：「看不到＝不存在」保護的是沒有傳票權限的角色，不保證對 finance 隱藏案件是否存在。c-case404 的規格會寫明，守門不把該頁籤當違規。
⇒ AT6-O1 結案。〕
