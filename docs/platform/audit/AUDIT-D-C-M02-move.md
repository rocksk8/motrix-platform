...
## 5. M02-S2 複核（D，2026-09-26 08:21）

- 對象：`wip/c-m02-s2` `bcd4288c`（第五班；複核時尚未合回 origin）。新題 `modules/crm/tests/test_dev_case_write_access_2026_09_26.py`：外人 PUT／狀態／轉建／新增記錄皆 403 且 DB 不變，刪除與重新連結申請 403；正對照建立者四項成功、DB 驗改名／轉建／記錄數。
- D 突變（`modules/crm/tests`，35 題）：S2a 拿掉 PUT 的 `row_access` 403（即先前存活的 MC2）⇒ **紅**；S2b 拿掉新增記錄的 403 ⇒ 紅 ⇒ **M02-S2 關閉**。
- 觀察：題目說明寫「刪除／重新連結申請與核准只准管理員」，但題只打了兩個**申請**端點，核准端點沒有外人題；且業務角色對申請端點回 403 的原因分不出是角色還是列權限（正對照沒有管理員成功的那一半）。不擋。
- 〔08:41 補〕上述觀察已由 `wip/c-m02-s2b` `6e2e15a7` 補題（`test_request_and_approve_are_gated_by_role_not_row_access`）：核准端點外人／建立者／管理員 403 且 `dev_cases` 不變、最高管理者成功；建立者申請 403、管理員（不在名單）申請 200 ⇒ 角色與列權限分得開。D 基準 2 passed；自加突變 S2c（申請刪除收緊成只限最高管理者）⇒ 紅 ⇒ **觀察結案**。
