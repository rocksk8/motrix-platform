...
|---|---|---|---|
| A-1 | 修正（與 V9 a1cc2871 同一作法，主持 2026-09-25 指定）：①有份數日期晚於今天，或②有東西要刪且今天以外全部過期 ⇒ 該層寫 `.prune_hold`、這一輪一份都不刪；**標記在就每輪都 ERROR 且不刪**，直到有人確認後手動刪標記；讀不到標記當成暫停。六層（本機、雲端每日／週／月、個資每日／月）同一判定；新版保留「最新 7 份」底線。X 的多日模擬改寫成題目（30 份、跳 40 天、連跑 40 天，第 3 天起只靠標記撐住）。突變：標記不持續（每輪重新推算）⇒ 2 紅；讀不到標記當成沒有、未來日期不觸發、暫停但不告警 ⇒ 各 1 紅；②不觸發 ⇒ 2 紅。兩支舊的日期規則題照 V9 補一份昨天的資料夾 | C `wip/c-fix-x1` | |
| A-2 | 修正：改用 AST 找模組層級的 `_startup_integrity_check()` 呼叫，且在 `init_db(DEMO_DB_PATH)` 之後。突變：刪掉呼叫 ⇒ 紅；搬到前面 ⇒ 紅（第一次的搬移突變寫成「多加一個」、沒刪原本的，綠燈不算數，已重做） | C `wip/c-fix-x1` | |
| A-3 | 修正：假的 Google 每次回不同 id；case_stage 兩欄各驗自己的 id；invoice／payment／shipping 參數化執行，驗只寫自己那一列、其他欄位不動，並加建立事件期間被改過不蓋回的 lost-update 題。突變：due／done 對調、三支各寫錯值、shipping 整包蓋掉 ⇒ 皆紅。INTEGRATION-POINTS IP-6 守門欄已更新 | C `wip/c-fix-x1` | |
| B-1 | 不修（本輪）：M12 仍在 `routers/`，拿不掉；列為「M12 搬進 `modules/` 前必修」，畫面顯示 `notice`＋取消勾選／刪除各一句正確訊息 | C `wip/c-fix-x1` | |
...
| B-3 | 修正：兩份拆法同改——email 用嚴格的位址樣式，電話只收以數字為主的片段 `[+(]?\d[\d\-\s()#轉]{5,}\d`，找不到就留空。補 5 種寫法（含 X 的 3 種）。突變：回到舊拆法 ⇒ 紅。`db._m106` 是凍結的 migration，不改 | C `wip/c-fix-x1` | |
| B-4 | 修正：`backup.alert_email_failed` audit 同「告警原因＋寄不出去原因」每日一筆（`.email_failed_<日期>`）；寄信照樣每輪重試；`SEND_SKIPPED` 寫明「這台機器設定成不寄信」。BACKUP_ALERT.txt 每次整檔覆寫，註記不節流、不會變長。突變：audit 不節流 ⇒ 紅 | C `wip/c-fix-x1` | |
| C-1 | 同意，不改：推送搬離擁有模組後這題才有實質意義；L1 直接讀 5 張表的相依已記在 IP-6 末段 | C `wip/c-fix-x1` | |
| C-2 | INTEGRATION-POINTS IP-5「對方不在時」補一句「提供者在但失敗」的行為 | C `wip/c-fix-x1` | |
| C-3 | 同意，列入：INTEGRATION-POINTS 與程式一致性守門（capability 名稱與登記對照）排進 D6 覆蓋缺口；本輪不做 | C `wip/c-fix-x1` | |
| C-4 | 修正：users.html 的「預設 miac@60575481」與兩個本公司 placeholder 已改；守門題掃頁面不可含 `_LEGACY_WEAK_PASSWORDS`（含英數的）與本公司電話／網域。突變：放回 ⇒ 紅。⚠ 查證時發現 C 先前在 ROADMAP A8d 寫的「`auth.py:29` 預設解鎖密碼含統編」是錯的（那是弱密碼黑名單），已更正並保留原文 | C `wip/c-fix-x1` | |
| C-5 | 修正：無效跳脫 `"\/"` 改成 `"\\/"`（行為相同，消除 3.12+ 的 SyntaxWarning） | C `wip/c-fix-x1` | |
| C-6 | 同意，不改：據點優先讀，不影響輸出；待 B-2（X9B）一併決定「已有值」的定義 | C `wip/c-fix-x1` | |
| C-7 | UPGRADE-RUNBOOK §1b 補「換版後第一次啟動較慢」；正式機耗時列待查（U10 健康檢查時一起量）。順帶修了 §1b 兩處被吃成控制字元的路徑（`backend\tools`） | C `wip/c-fix-x1` | |
