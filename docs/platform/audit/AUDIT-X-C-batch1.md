...
|---|---|---|---|
| A-1 | 修正（與 V9 a1cc2871 同一作法，主持 2026-09-25 指定）：①有份數日期晚於今天，或②有東西要刪且今天以外全部過期 ⇒ 該層寫 `.prune_hold`、這一輪一份都不刪；**標記在就每輪都 ERROR 且不刪**，直到有人確認後手動刪標記；讀不到標記當成暫停。六層（本機、雲端每日／週／月、個資每日／月）同一判定；新版保留「最新 7 份」底線。X 的多日模擬改寫成題目（30 份、跳 40 天、連跑 40 天，第 3 天起只靠標記撐住）。突變：標記不持續（每輪重新推算）⇒ 2 紅；讀不到標記當成沒有、未來日期不觸發、暫停但不告警 ⇒ 各 1 紅；②不觸發 ⇒ 2 紅。兩支舊的日期規則題照 V9 補一份昨天的資料夾 | 972fbb5b | ✅ 關閉（實際 commit `972fbb5b`）。多日模擬題重跑綠。稽核者突變：①整段暫停判定拿掉（＝原本「保留 7 份、其餘照刪」）⇒ **5 紅**；②標記不持續、每輪重新推算 ⇒ **3 紅**（含多日題）。假綠燈檢查：用真的 `_prune_local_db_backups`，只換掉 `date.today`，斷言的是檔案系統剩下哪些資料夾。⚠ 另開新發現 N-1（跳動小於保留天數） |
| A-2 | 修正：改用 AST 找模組層級的 `_startup_integrity_check()` 呼叫，且在 `init_db(DEMO_DB_PATH)` 之後。突變：刪掉呼叫 ⇒ 紅；搬到前面 ⇒ 紅（第一次的搬移突變寫成「多加一個」、沒刪原本的，綠燈不算數，已重做） | 972fbb5b | ✅ 關閉。稽核者突變：刪掉 main.py:568 的呼叫、`def` 保留 ⇒ 紅（修正前同一個突變是綠）。AST 只看模組層級：呼叫搬進 `if` 區塊會變成假紅，不會變成假綠，接受 |
| A-3 | 修正：假的 Google 每次回不同 id；case_stage 兩欄各驗自己的 id；invoice／payment／shipping 參數化執行，驗只寫自己那一列、其他欄位不動，並加建立事件期間被改過不蓋回的 lost-update 題。突變：due／done 對調、三支各寫錯值、shipping 整包蓋掉 ⇒ 皆紅。INTEGRATION-POINTS IP-6 守門欄已更新 | 972fbb5b | ✅ 關閉。稽核者突變：Q 的 `_STAGE_EVENT_COLUMNS` due／done 對調 ⇒ `test_calendar_writeback_case_stage_slots_are_separate` 紅；invoice 回寫改寫 `"WRONG"` ⇒ 2 紅（own_row、lost-update）。假綠燈檢查：假 Google 每次回不同 id，斷言讀的是 DB |
| B-1 | 不修（本輪）：M12 仍在 `routers/`，拿不掉；列為「M12 搬進 `modules/` 前必修」，畫面顯示 `notice`＋取消勾選／刪除各一句正確訊息 | 972fbb5b | ⛔ 維持開啟（建議）。延後的理由成立（M12 拿不掉），但「M12 搬進 `modules/` 前必修」只寫在這張表：ROADMAP 查不到對應項（`grep notice ROADMAP.md` 沒有結果）。補進 ROADMAP 後關閉 |
| B-2 | 修正（由 X-9b 修正者處理，併 X-9b M-1）：判準改成與 `fill_company_profile_blanks` 相同——原值是空值（None／空字串／只有空白）而新值等於補值 ⇒ 接受；新鍵也必須等於補值；刪鍵、改有值的欄位、補成非補值的值 ⇒ 仍算改寫。轉換後與啟動後共用 `U.settings_changes()`。補題：空字串／只有空白被補 ⇒ 通過；反向控制 4 題 ⇒ 紅。突變（空值不算可補）⇒ 紅 | eda4d3cb | ✅ 關閉（見 AUDIT-X-9b M-1：拿掉補空值例外 ⇒ `test_b2_blank_string_fill_passes_verify` 等 4 紅） |
| B-3 | 修正：兩份拆法同改——email 用嚴格的位址樣式，電話只收以數字為主的片段 `[+(]?\d[\d\-\s()#轉]{5,}\d`，找不到就留空。補 5 種寫法（含 X 的 3 種）。突變：回到舊拆法 ⇒ 紅。`db._m106` 是凍結的 migration，不改 | 972fbb5b | ✅ 關閉。稽核者重跑三種寫法：純地址 ⇒ phone `''`；`info@x.com Tel: 04-1234` ⇒ `04-1234`／`info@x.com`；`04-1234-5678, info@x.com` ⇒ `04-1234-5678` |
| B-4 | 修正：`backup.alert_email_failed` audit 同「告警原因＋寄不出去原因」每日一筆（`.email_failed_<日期>`）；寄信照樣每輪重試；`SEND_SKIPPED` 寫明「這台機器設定成不寄信」。BACKUP_ALERT.txt 每次整檔覆寫，註記不節流、不會變長。突變：audit 不節流 ⇒ 紅 | 972fbb5b | ✅ 關閉（抽查：讀過程式碼、本次基準綠；沒有另外做突變） |
| C-1 | 同意，不改：推送搬離擁有模組後這題才有實質意義；L1 直接讀 5 張表的相依已記在 IP-6 末段 | 972fbb5b | ✅ 同意 |
| C-2 | INTEGRATION-POINTS IP-5「對方不在時」補一句「提供者在但失敗」的行為 | 972fbb5b | ✅ 關閉（INTEGRATION-POINTS 已補） |
| C-3 | 同意，列入：INTEGRATION-POINTS 與程式一致性守門（capability 名稱與登記對照）排進 D6 覆蓋缺口；本輪不做 | 972fbb5b | ✅ 同意延到 D6 |
| C-4 | 修正：users.html 的「預設 miac@60575481」與兩個本公司 placeholder 已改；守門題掃頁面不可含 `_LEGACY_WEAK_PASSWORDS`（含英數的）與本公司電話／網域。突變：放回 ⇒ 紅。⚠ 查證時發現 C 先前在 ROADMAP A8d 寫的「`auth.py:29` 預設解鎖密碼含統編」是錯的（那是弱密碼黑名單），已更正並保留原文 | 972fbb5b | ✅ 關閉（抽查：`users.html` 已不含舊密碼；守門題在本次基準內綠） |
| C-5 | 修正：無效跳脫 `"\/"` 改成 `"\\/"`（行為相同，消除 3.12+ 的 SyntaxWarning） | 972fbb5b | ✅ 關閉 |
| C-6 | 同意，不改：據點優先讀，不影響輸出；待 B-2（X9B）一併決定「已有值」的定義 | 972fbb5b | ✅ 同意 |
| C-7 | UPGRADE-RUNBOOK §1b 補「換版後第一次啟動較慢」；正式機耗時列待查（U10 健康檢查時一起量）。順帶修了 §1b 兩處被吃成控制字元的路徑（`backend\tools`） | 972fbb5b | ✅ 同意（正式機耗時待 U10 量） |
| N-1 | 修正（主持 2026-09-26 裁示）：最新一份（今天以外）距今天超過 2 天 ⇒ 暫停清理、寫 `.prune_hold`、每輪告警；每日層 2 天，週／月層用 2 個週期（14／62 天）。多日模擬題涵蓋跳 3、10、29 天（各連跑 10 天，真實快照一份不刪、每天告警）＋邊界 2 天。突變：拿掉規則 ⇒ 3 紅、本機層沒帶門檻 ⇒ 3 紅、邊界改 ≥2 ⇒ 1 紅。舊的日期規則題補「昨天／本週」各一份 | wip/c-p8-gaps | |
...
- 基準：TSD＋TCS＋TA8＋`test_case_stage_done_calendar`＋9b 相關，**230 passed**。
- 稽核者自做突變 5 項（A-1 ×2、A-2 ×1、A-3 ×2），全部轉紅、已還原。
- 結果：必修 A-1～A-3 關閉；建議 B-1 維持開啟（延後的前提沒有落到 ROADMAP）。
- **新發現 N-1（需主持裁示）**：時鐘跳動**小於**保留天數時不會觸發暫停，真實快照照樣被刪，而且沒有告警、也不寫標記。
  - 稽核者裸呼叫 `_prune_select`：30 份真實快照、保留 30 天。
  - 往前跳 3／10／20／29 天 ⇒ 分別刪掉 2／9／19／23 份；跳 31、40 天才暫停。
  - 例如跳 29 天時，本機時間點還原能力從 30 天安靜地縮成 7 天。
  - STATES-DATA-OPS S-CC07 的「應有行為」舉的例子是「晚 >2 天而中間沒有快照」，實作（與 V9 `a1cc2871` 相同）只抓「除了今天以外全部過期」。
  - 這和「停機很久」的形狀相同，要不要把「最新一份非今天的快照距今 >N 天」也列為異常（只暫停、告警，不影響正常停機後的補清理），請主持裁示。
