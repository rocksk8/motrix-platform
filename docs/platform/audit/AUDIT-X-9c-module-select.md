...
|---|---|---|---|
| A-1 | 不在這次範圍（主持裁示）：P-SW-05 已在 `wip/a-platform-states`，由另一個代理合回。⚠ 該分支與本修正衝突，見表下「合併注意」 | — | **待合回**。origin/platform `af3d5ef7`（9c／batch1 的題目在 `7eb162f0` 跑；兩者之間沒有動到受稽核的檔） 的 `read_disabled_at_startup` 讀不到時仍回 `frozenset()`（MS:44-52）；`wip/a-platform-states`（`0dffe55d`）只在主樹的本機分支，**origin 上沒有**。合回後由稽核者用 WAL 觸發方式（`locking_mode=EXCLUSIVE`、檔頭損毀）重跑 R5 才關 |
...
| C-1 | 不在範圍：STATES-PLATFORM 與 P-FE-02／03、P-DT-01、P-LD-07 由 `wip/a-platform-states` 合回 | — | **待合回**（同 A-1） |
| C-2 | 不在範圍（屬 A-1）。提醒合回 A-1 的代理：該分支 `test_locked_db_uses_last_good_list_then_all_disabled` 仍用一般 journal 庫＋`BEGIN EXCLUSIVE`，要補 WAL 觸發方式 | — | **待合回**：隨 A-1 確認反向控制改用 WAL 觸發方式 |
