...
|---|---|---|---|
| R-M1 | §C-11 與 §C-4 本文的「重跑全量／就跑全量」劃掉加〔更正〕 | wip/b-rebasecheck-2 16150027 | ✅ 10:55 D：range-diff 第一個 commit 與審過的 e0137ddc 相同，新增 16150027 只改 PLAYBOOK 兩列 ⇒ **關閉（16150027）**。觀察 R-O1：PLAYBOOK :119「全量照舊——fixture 層改動…」與表格 ③（:188）「動到 fixture 層時 ⇒ 全量」沒寫由誰跑，仍可讀成各線自己跑；建議兩處補「（由列車跑，§G3）」 |
