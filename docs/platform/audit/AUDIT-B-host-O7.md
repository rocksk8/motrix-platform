...
## 5. 複核 wip/h-o7-3 461ef857（S-3、O-2、O-3）

| # | 修法 | 稽核確認 |
|---|---|---|
| S-3 | 四個動作本體改 `_xxx`，外包 `_guard`（try／catch／finally）：任何路徑 `finally` 解鎖；例外時 `state=error`、訊息說明；發布／還原說「結果不明：請關閉再打開看版本清單確認」。e2e：publish 路由 abort ⇒ 「發布結果不明」、發布鈕解鎖 | ✅ 關閉。`_publish` 裡呼叫的是包過的 `saveDraft()`——它的 finally 先解鎖、`_publish` 下一行同步再上鎖，中間沒有 await，無空窗 |
| O-2 | 範圍下拉 `:disabled="busy"`；e2e：攔住 publish ⇒ 下拉停用、放行後恢復 | ✅ 關閉（見 O-4） |
| O-3 | 載入失敗顯示「重試」鈕（在表頭，不受 inert 影響）；訊息改「按『重試』再載入一次」；e2e：500 後重試成功、重試鈕消失 | ✅ 關閉 |

**O-4（觀察）　O-2 那一題最後的 `assert "公司預設" in ed.inner_text()` 永遠成立**
- 範圍下拉的選項文字就是「公司預設」，`inner_text()` 含 select 的選項 ⇒ 這一行驗不到「訊息標對範圍」。建議改讀 `ml-msg`：`已發布第 N 版（公司預設）`。

本輪 M／S 全部關閉；只剩 O-4（不擋上車）。
