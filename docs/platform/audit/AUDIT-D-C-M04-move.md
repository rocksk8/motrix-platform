...
| M04-S1～S3 | **修正**。S1：提供方路徑補 `api/`，並在 test_integration_points_registered 加守門 `missing_provider_files`：模組在時提供方檔案要存在，有正對照與反向控制。S2：README 的 M01 不在判準改為 `case.access`（IP-12）。S3：SPEC.md 改指 `modules/subcontract/tests`，並更正「本模組沒有專屬編號」（EM12） | 27441eac、330b56c2、dd7aecf0 | |
| O-1～O-3 | O-1：主持 07:53 裁示 `privacy_notice_acks` **維持 L1 共用**（個資告知跨多個模組）。README 與 ROADMAP 劃掉並加〔更正〕。O-2：不改，讀取連接器另外開題。O-3：同意。本輪反向控制改用 `git worktree add --no-checkout`＋sparse-checkout 排除模組，從頭不取出，所以沒有 pyc 殘留。另外實測踩到：在同一棵樹上跑過題之後再排除 netplan，只剩 `__pycache__` 時資料夾還在，module_installed 判定為「在」。A 的 O-4（68f16342）合回後可解 | 27441eac | |
