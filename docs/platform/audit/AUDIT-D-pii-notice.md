...
|---|---|---|---|
| PN-M1 | 〔主持裁示 2026-09-26，CORE-SPEC 7eb5198f〕告知以**欄位與當事人**為單位決定，不以頁為單位；出貨單收件人與送貨地址可手動輸入 ⇒ 要告知（用途同聯絡人）。交給 A（出貨模組擁有者），排在 M03 之後。**A 修正（wip/a-pn-m1）**：①守門逐欄：`notice` 也要列 `fields`；一頁兩種以上當事人用 `notices`（每個對象 `subject`、端點、`data-privacy-subject` 區塊），聯集＝掃描結果、一欄只屬一個對象；`notices` 內可逐欄 covered_by／not_natural_person。②掃描字尾加 `recipient`（原本收件人掃不到）。③出貨單收件人：`/api/shipping-notes/{note_no}/privacy-notice(/ack)`（只接受已存檔的收件人、鍵含姓名、權限同出貨單清單），案件頁出貨單視窗加區塊、存檔時記錄。④pii_forms.json：case-management 拆 case_site_contact／shipping_recipient；quotation-form 業務欄位 covered_by users.html；客戶／供應商主檔 not_natural_person（使用者表單 (a)「視為公司」）。你要的反向控制（notice 頁多一個 `foo.contactPhone` ⇒ 紅）已是一題；突變 G1～G6、B1～B3、F1～F3 皆紅 | 7eb5198f（裁示）；7668d895、0c277984、7c11091b | ✅ 14:48 D 複核（wip/a-pn-m1 7c11091b）：①逐欄——守門要求 notice／notices 列出 fields、聯集＝掃描結果；case-management 拆成 case_site_contact 與 shipping_recipient；客戶／供應商主檔依 CORE-SPEC 裁示（13:47 使用者表單「視為公司」）逐欄 not_natural_person；④突變 PNF（出貨單塞一個 `shippingForm.recipient_phone`）⇒ `test_every_pii_form_has_a_decision_and_it_still_holds` 紅；③延遲載入沒有新空窗：勾選框只看有無收件人、不看告知文字是否載入，雜湊由伺服器算，存檔本來就不擋（設計），新單存檔後以單號記錄、失敗留在視窗；基準 79＋5（e2e）passed ⇒ **關閉（7c11091b）** |
| PN-S1～S3 | S1 修正：非成員 GET／POST 403 兩題，PN8、PN9 轉紅。S2 修正：e2e「甲已告知 → 改乙存檔 → 重新打開顯示尚未記錄 → 勾選存檔記錄乙」，PNJS 轉紅（出貨單收件人；PNJS 是共用元件，報價單同一段程式）。S3 修正：`_not_typeable` 只認屬性名稱（先拿掉引號內的值、標籤按引號走，不在 `=>` 截斷），你的兩個例子與 `=>` 一例加進參數化題；突變「退回舊正則」紅 | 7668d895、0c277984 || ✅ 14:48 D：PN8、PN9 重跑 ⇒ 各紅 2（`test_quotation_privacy_endpoints_refuse_non_members`）；PNJS 重跑 ⇒ `modules/supply/tests/test_e2e_shipping_recipient_privacy_2026_09_26.py` 紅（D 第一次只跑了舊的 docs e2e 檔、誤判存活，是 D 選題錯誤）⇒ **S-1～S-3 關閉（7c11091b）** |
| O-1～O-4 | O-1：第四班列車長已把兩支告知端點搬進 modules/netplan/api.py ⇒ 不需動作。O-2：不改（同意）。O-3：端點已走 api_module、頁面位置走 page_files()；「清單有、頁面不在」併入 B 的 core-only 反向控制。O-4：不改（與完工單明細一致） | — || ✅ 14:48 同意 A 的處置 ⇒ **關閉** |
