...
- **列車月台**（PLAYBOOK §G3；各線登記：分支｜HEAD｜差異題結果｜是否動 fixture 層／main）：
  ...
  - C｜`wip/c-m02-2`（取代 wip/c-m02：rebase 到 origin、含稽核 D M02-M1／M02-S1 修正；D1 階段 B：M02 業務開發搬進 `modules/crm`）｜9840974b｜M02 在：tests/platform＋modules＋原 7 個混合檔＋em1＋row_access＋spec_coverage 1371 過 1 紅（UNIT-INDEX，已重產）；**反向控制**（刪掉 modules/crm，同一範圍）：1333 過 4 紅＝`modules_json_lists_only_existing_units`、`test_unit_index_is_current`（§B-11 允許）＋`test_registry_matches_code`、`test_em1`（A 的 a-m10 已修，合回後綠）；突變 5 項皆紅｜🔴 動 main.py；M01 刪報價單改走 IP-11；`quotations.html` 顯示 notice
  - C｜`wip/c-m02-s2`（稽核 D M02-S2：業務開發案「改不了」的題；**依賴 c-m02-2**，基底＝origin/wip/c-m02-2 9840974b）｜bcd4288c｜modules/crm/tests＋模組 CHANGELOG／package 守門：58 過；突變 4 項皆紅｜只加題＋模組 1.0.2；產品碼不動
  - C｜`wip/c-m02-s2b`（主持轉 D 的 M02-S2 觀察：核准端點（刪除／重新連結）補外人題，並分開角色與列權限——建立者（有列權限）申請 403、管理員（不在名單）申請成功；外人／建立者／管理員核准 403 且資料不變、最高管理者核准成功；舊題說明加〔更正〕；**疊在 c-m02-s2 bcd4288c 之上**）｜6e2e15a7｜modules/crm/tests＋CHANGELOG／unit_cards 守門：67 過；突變 4 項（兩個核准放寬成管理員、兩個申請不擋角色）皆紅｜只加題＋模組 1.0.3；產品碼不動〔C 登記〕
  - C｜`wip/c-m04`（D1 階段 B：M04 外包工班搬進 `modules/subcontract`，router 放 `api/`；**疊在 c-case-access-2 之上**，含 dep_scan／module_files 同一改動（與 c-module-files 重複，列車上以先合回者為準））｜dd6f1550｜合回閘門（tests/platform＋modules＋45 檔含 e2e，-n 2）：1578 過 3 紅（origin 新守門：module.json customization、X-VAT 捨入清單路徑，已修）；修後 tests/platform＋modules 1099 過；**反向控制**（刪掉 modules/subcontract）：啟動 ping 200、四個前綴 404、出納／T100／整包明說；tests/platform＋modules 1082 過 6 紅＝§B-11 允許 2（modules_json、UNIT-INDEX）＋已分派 A 的 3（IP 登記 2、case_read_scope）＋B 的 1（G1 快照）；突變 8＋e2e 1 項皆紅｜🔴 動 main.py（拿掉三支 router）；M01 `routers/quotations.py`（IP-12 取用、IP-13 提供）、M05 `cashier.py`／M06 `accounting_export.py`（IP-14）；L1 `helpers/dates.normalize_date`；出納頁顯示 notice；CORE 暫取 1.22
  - H｜`wip/h-hist`（〔更正 2026-09-26 07:59：第一版 f4c14dc5 只鎖寫入端，D 實測 Windows 上儀表板輪詢時 os.replace 丟 PermissionError、300 次掉 145 筆（比修正前更糟）⇒ 改帶 `wip/h-hist-2`（f4c14dc5＋288e3a78：讀取端也拿鎖、占用時重試後直接覆寫、讀不到時警告明說；讀寫並發題＋突變紅；儀表板 119 題綠）〕部署儀表板歷史寫入加鎖＋原子取代：B 反向控制抓到的 history.json 殘字＝解除鎖定端點與 job 收尾同時讀改寫）｜f4c14dc5｜tests/platform -k deploy_dashboard/insights 117 過；新並發題＋突變（還原舊寫法 ⇒ 紅）｜fixture 層／main：無〔2026-09-26 07:44 H〕
  - H｜`wip/h-u14`（U14 前端：自訂模組草稿只有建立者與超管看得到編輯／送出，依伺服器 canEdit，缺欄位不當 false；其他人看到說明）｜48dab2a3｜會開 custom-records 的 4 檔 34 題綠（-n 2）；新 e2e 3 題；突變（還原前端 ⇒ 紅）；版本紀錄 80 題綠｜fixture 層／main：無；VR3 併入系統 26a｜等 D 稽核〔2026-09-26 07:53 H〕〔2026-09-26 08:04：D 稽核必修 0（AUDIT-D-host-U14-hist）；補 U-S1 題（存檔後不重整仍可送出），突變 U1 轉紅；可上車〕
  - H｜`wip/h-hist-3`（D 稽核 H-S1：寫入端把「讀不到」當空清單覆寫——被占用重試後仍不行就另存 pending、不碰主檔並警告；讀不懂先封存壞檔再開新檔；疊在 h-hist-2 上，第五班之後上車）｜9e55dfa1｜儀表板＋gitignore 122 題綠；兩題＋行為突變紅（第一次突變是換回舊檔、夾具 setattr 出錯＝假陽性，改只動行為重做）｜fixture 層／main：無；.gitignore 加兩行〔2026-09-26 08:04 H〕〔2026-09-26 08:10：D 關閉 H-S1；H-S2 pending 沒有落點（只寫不收，「尚未併回」警告永遠在且蓋掉 15 分鐘失敗警告）⇒ 9e55dfa1 下一次寫入併回並刪檔、讀取端看合併後紀錄；行為突變 2 項各紅；待 D 複核〕
  ...
  - H｜`wip/h-corered`（D 稽核 AUDIT-D-B-G1 G-M1 的 A 那 2 題：case_read_scope 反向控制題、IP 登記表 present_still_red 的「模組在」正對照改用合成模組，不綁真實 L2；A 無回應由主持做）｜3710f462｜sparse 樹（不含 backend/modules）：舊版 2 紅、新版 11 綠；一般樹 11 綠；突變 2 項（在的模組也被豁免）各紅｜只改測試；fixture 層／main：無〔2026-09-26 09:01 H〕
  - B｜`wip/b-m08`（〔2026-09-26 08:20 稽核 ⑰（AUDIT-X-B-M08-move）：必修 4——M-1 連線守門只包 db.get_db（綁名 import 看不到＝假綠，與 O8 同一成因，改觀測最底層）、M-2 憑證守門只掃 routers/、M-3 product_drill 拿前綴當端點、M-4 首頁 M08 不在時顯示 0 與「沒有…」；§B-11 真刪被權限擋、B 的 1670／3 紅未獨立核對 ⇒ B 修後自己重做並附輸出；D 複核〕〔主持 2026-09-26 07:44：c-module-files 已合回；**上車前要 D／子代理稽核 ⑰**〕D1 階段 B：M08 營運分析搬進 `modules/analytics`，router 放 `api/`；**疊在 wip/c-module-files 之上，須排在它之後**；含 L1 新增 `helpers/receivables.py`（A8b 中繼，M05 搬遷時收回）、`routers/company_lookup.py`（GCIS／`/api/now`）、`/api/sales-orders` 移 M01、IP-1 dispatch.row 改走提供者；選單四項移入 module.json；tests/platform 四道守門改模組感知：選單對等、跨組邊基線、final_drill 冒煙清單（新增 `smoke_plan`）、IP-1／獎金發放串接點題）｜9930923f｜閘門（M08 相關＋tests/platform＋側欄／五頁 e2e＋modules/analytics/tests，-n 2、低優先權）：1866 過，另 1 題 e2e 冒煙 45s 存檔逾時（負載；單跑過、第一次閘門過）；**反向控制**（刪 modules/analytics，--continue-on-collection-errors，同一範圍）：1670 過 3 紅＝§B-11 允許兩題＋`test_deploy_dashboard_jobs` 1 題（history.json 尾端多出殘字、單跑過，疑非原子寫入的產品競態，非 M08，已報主持）；探針：伺服器起、M08 端點 404、出納／T100（同 /api/reports 前綴）／公司查詢／銷售訂單／選單 200，正對照模組在時全紅；**D1b**：改 reports.py 66.8%→23.3%（3265→1141 題）、dashboard.py 25.1%→22.2%、reports.js 22.0%→21.8%（契約 838 題固定）；**追加 9930923f**：tests/platform/test_l1_connections.py 補回 GCIS／/api/now／sales-orders 連線守門（8 過、突變紅）｜fixture 層：無；**main.py：有**（拿掉兩支 router 與月報排程，改 ModuleSpec）；L1 新增（CORE 暫取 1.22，列車上 core_bump 重定）〔07:09 B 登記〕
  - B｜`wip/b-g1`（主持派工：G1 L1 公開介面改由 L1 自己宣告 `__l1_public__`，不再依「L2 有沒有在用」——拿掉 M04 時被判刪除、要升主版號那一類；首次導入把當時跨模組在用的 42 個底線名稱逐檔宣告（11 個 L1 檔只新增行），介面 0／0／0 不升版；新守門 test_l2_uses_only_declared_l1_underscore_names（合成樹反向控制、突變拿掉 _require_user 宣告 ⇒ 4 題紅）；**core-only 反向控制工具** `tools/platform/core_only_rc.py`（拋棄式工作樹拿掉全部 L2 模組跑 tests/platform，§B-11 兩題以外必須全綠；PLAYBOOK §G3 每班跑一次）；它第一輪抓到的三道守門修正（選單 API 綁 tender_radar、信件登記正對照寫死 tender_*、頁面路徑基線把「沒裝」當「降低」）；PLAYBOOK §B-11 補「判斷路由在不在用 tests._routes.all_routes」）｜6b0c8fdd｜閘門（modtest 差異題＋tests/platform，-n 2、低優先權）：1609 過、3 xfail；rebase 到 427c8be9 後 tests/platform 1009 過；core-only 實跑（拿掉 tender_radar）：965 過、4 紅＝§B-11 允許 2＋選單對等 2（已在 wip/b-m08-2 修，兩包合回後應歸零）｜fixture 層：無；main.py：無；**L1 檔 11 個只新增 `__l1_public__` 宣告**（不升 CORE_VERSION）〔08:33 B 登記〕
