...
- **列車月台**（PLAYBOOK §G3；各線登記：分支｜HEAD｜差異題結果｜是否動 fixture 層／main）：
  ...
  - C｜`wip/c-m01-s3`（M01-PLAN §3-3＋§3-4：§3-3 在 origin 上已成立、不需改碼；§3-4 `case.summary`／`case.locations`（主持四點裁示；IP-96／97 暫定）；系統身分哨兵 `helpers.case_access.SYSTEM`（user=None 拒絕，只准 L1＋守門）；地圖改走 case.locations、KNOWN_L1 刪 map_points（到期題紅→綠）；IP-12 summary 轉呼叫並標淘汰；基底 origin/platform）｜6d091294｜tests/platform＋地圖／案件存取相關檔 1425 過；突變 5 項皆紅｜L1 新增（CORE 暫 1.35）；⚠ 與 c-m05b-2 交會：deprecations 反掃會要求登記 IP-12 summary（兩包都上車後補）；ATT 那一列待第八班合回後補〔C 登記〕
