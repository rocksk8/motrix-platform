...
- **列車月台**（PLAYBOOK §G3；各線登記：分支｜HEAD｜差異題結果｜是否動 fixture 層／main）：
  ...
  - B｜`wip/b-g1`（G1 L1 公開介面改由 L1 自己宣告 `__l1_public__`＋守門 test_l2_uses_only_declared_l1_underscore_names；**core-only 反向控制工具** `tools/platform/core_only_rc.py`（sparse-checkout 排除全部 L2、不刪檔；判定＝紅 ⊆ §B-11 允許 ∪ `core_only_known_red.json`，清單轉綠未刪／一題都沒跑 ⇒ 不過；清單新增要 RUN-PLAN 帶錨點裁示）；core-only 第一輪抓到的三道守門修正；PLAYBOOK §G3／§B-11；含 AUDIT-D-B-G1 G-M1／G-S1 修正；rebase 到 25b81a70）｜6e7ba250｜閘門：modtest 差異題＋tests/platform 1609 過（rebase 前）；rebase 後 tests/platform 1009 過；工具題 12 過；**core-only 實跑 6e7ba250 前一版 3f6e0242**（排除 daily_tasks、netplan、tender_radar；1014 題）：8 紅＝允許 2＋6 題，6 題都已有修正在月台（case_read_scope、IP 登記表 ⇒ h-corered；dep_scan ⇒ c-coreonly-depscan；pii netplan ⇒ c-m07；menu_parity 2 ⇒ b-m08-2），清單依裁示從空開始 ⇒ **本包要和這四包同一班或在其後**｜fixture 層：無；main.py：無；L1 檔 11 個只新增 `__l1_public__` 宣告（不升 CORE_VERSION）〔09:35 B 更新〕
