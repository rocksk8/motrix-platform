...
- **列車月台**（PLAYBOOK §G3；各線登記：分支｜HEAD｜差異題結果｜是否動 fixture 層／main）：
  - C｜`wip/c-probes`（主持派工，擋正式 D7：crm／subcontract／payroll 宣告 `provides.probes`（3／4／4 支純讀 GET）；新守門 `test_probe_side_effects.py`（主持補充條件①②③：暖機後逐支比對每張表列數＋內容雜湊、smtplib、Thread.start、caplog／capsys 不含回應裡長度 ≥4 的字串值；product_drill／final_drill 只記狀態碼（AST）；合成路由反向控制）；MODULE-GUIDE probes 段加規則；另夾：M01／M05／M06／PROBES 步驟表移入 docs/platform/plans/（稽核 D CA-S3，內容不改）；基底 origin/platform 73d8ba63）｜**b2ef6439**（324b1504 之後加：操作軌跡 user_request_log 精確認出中介層那一列＋對照組 /api/system/version，不排除整張表；突變共 6 項皆紅）｜tests/platform＋三個模組 tests：1473 過（唯一紅＝CHANGELOG 未提交，提交後 51 過）；test_product_drill_probes 3 過；突變 4 項皆紅（probe 寫表、起背景工作、log 回應值、product_drill 保留內文）｜只動三個 module.json／CHANGELOG（crm 1.0.6、subcontract 1.0.5、payroll 1.0.3）＋一個新題檔＋文件；產品碼、fixture、main、sidebar 都不動〔C 登記〕
  ...
  - H｜`wip/h-o7-4`（取代 h-o7-3：rebase 到第六班 a3263993；O7 第三輪 S-3／O-2／O-3＋O-4，B 已全數關閉 AUDIT-B-host-O7 §5）｜0e1f89f3｜P9 e2e＋test_p9_layout 22 過｜只動 L1 前端＋模組測試〔2026-09-26 12:28 H〕
  - H｜`wip/h-o6s1-2`（取代 h-o6s1：rebase 到 a3263993；D 稽核 O6-S1 掃描那一半＋小建議，D 已關閉）｜f0eeca26｜local_only 11 過｜只動測試〔2026-09-26 12:28 H〕
  - H｜`wip/h-probes`（正式 D7 前置：tender_radar／daily_tasks／netplan 宣告 provides.probes；版號 1.3.1／1.0.2／1.0.3）｜d56c5fc6｜probes／package／changelog／catalog 守門 117 過；突變（probe 指到不存在路由）紅 2｜只改 module.json＋CHANGELOG；**待 D 稽核**（前哨之後）〔2026-09-26 12:28 H〕
  - B｜`wip/b-o6-3`（取代 wip/b-o6-2：rebase 到第六班 a3263993；無衝突；hard_cap 5 過；取代 wip/b-o6：稽核 D O6-M1——探針改放 backend/tests/.hardcap_probe_<id>（backend 根會被產品碼掃描看到，D 實測 20 次 3 次 FileNotFoundError）；刪不掉記錄並重試一次、仍不行發 warning；靜態守門：core.source_tree 的產品碼清單找不到探針（實建一個驗）、突變改回 backend 根紅）｜fb556e42｜（＋O6-S1：靜態守門說明寫明自己寫 rglob 的掃描靠各自排除 tests/）hard_cap 5 題過、探針目錄用完已刪｜fixture 層：無；main.py：無〔10:59 B 更新：取代 b-o6〕〔11:04 B 更新 HEAD；排第七班〕〔12:41 B 更新：wip/b-o6-3〕
  - B｜`wip/b-rebasecheck-3`（取代 wip/b-rebasecheck-2：rebase 到第六班 a3263993；無衝突；rebase_check／bookkeeping 28 過；取代 wip/b-rebasecheck：＋PLAYBOOK §C-11 本文與 §C-4 的「重跑全量／就跑全量」劃掉加〔更正〕（稽核 D 必修）；主持派工：modtest 不再叫各線跑全量，§G3——--rebase-check 影響大時建議差異題擴大＋tests/platform＋改到頁面的 e2e、全量交給列車、月台註明；一般模式自己動 fixture 層也照跑差異題、閘門過了 exit 3＝要註明排車頭；need_full 改名 high_impact；MODULE-GUIDE §7、PLAYBOOK §G1〔更正〕）｜3caea5d9｜（＋R-O1：「全量照舊」一句與表格 ③ 補「由列車跑」）rebase-check／bookkeeping 28 過（新題：輸出不出現重跑全量／--full、建議不含 fixture 層檔）；tests/platform（-n 2、低優先權）1007 過｜fixture 層：無；main.py：無（只動 tools/platform/modtest.py 與它的題）〔10:26 B 登記〕〔10:44 B 更新：b-rebasecheck-2〕〔12:41 B 更新：wip/b-rebasecheck-3〕
  - B｜`wip/b-m08-s-2`（取代 wip/b-m08-s：rebase 到第六班 a3263993；--onto 去掉已上車的 b-m08-3；衝突只在 analytics money 題（_patch_entries 與列車的 needs_subcontract 兩邊都留）；l1_to_l2 基線刪掉 M07 搬遷帶走的 system→bonus；tests/platform 1053 過；稽核 ⑰ S-1～S-8、O-1～O-9：採納 14 項各附突變、S-2 不改碼、S-4 文件路線、O-4 部分採納、O-9 暫緩寫進 ROADMAP；含 --prune 修正（原本會在拿掉模組的樹上刪真實的邊、並洗掉其他鍵）；analytics 1.0.1（sales-orders.html 改歸 M01、permissions、urllib）；**疊在 wip/b-m08-3 上**，第六班合回後 --onto rebase）｜5b70fb29｜閘門（tests/platform＋modules/analytics/tests＋受影響 10 檔，-n 2、低優先權）1307 過 0 紅（943d0bfe；之後只加 ROADMAP 一行）；D 複核 16 項已關閉（d4e909f8），O-9 待關｜fixture 層：無；main.py：無；**須排在 b-m08-3 之後**〔11:41 B 登記〕〔12:41 B 更新：wip/b-m08-s-2〕
  - B｜`wip/b-routes-3`（取代 wip/b-routes-2：rebase 到第六班 a3263993；衝突只在 MODULE-GUIDE 與 modules.json L1 前綴（兩邊都留）；歸屬錯誤 0；route／maps／boundaries 過；test_map 重產；取代 wip/b-routes；＋稽核 AUDIT-D-B-routes：RT-M1 錯誤②只看已安裝的群組（沒裝的模組不報、不列允許清單）、RT-S1 萬用寫法只接受「/*」結尾且至少第三層（跨分界、整個前綴都判不合格）。本體：modules.json 個別路由明列歸屬、優先於前綴；四種歸屬錯誤；origin 既有 5 條歸屬不明已處理）｜cb7a8928｜（＋D 觀察：已搬遷群組必須寫 key，check_group_keys＋反向控制）本包題 17 過（RT-M1 兩題反向控制、RT-S1 格式七種＋分界）、突變（拿掉已安裝判斷）紅；sparse 樹拿掉 netplan 實跑 17 過；tests/platform 1022 過｜fixture 層：無；main.py：無；排第七班〔11:52 B 更新：取代 b-routes〕〔12:41 B 更新：wip/b-routes-3〕
