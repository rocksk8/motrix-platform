...
- **列車月台**（PLAYBOOK §G3；各線登記：分支｜HEAD｜差異題結果｜是否動 fixture 層／main）：
  ...
  - B｜`wip/b-m08`（D1 階段 B：M08 營運分析搬進 `modules/analytics`，router 放 `api/`；**疊在 wip/c-module-files 之上，須排在它之後**；含 L1 新增 `helpers/receivables.py`（A8b 中繼，M05 搬遷時收回）、`routers/company_lookup.py`（GCIS／`/api/now`）、`/api/sales-orders` 移 M01、IP-1 dispatch.row 改走提供者；選單四項移入 module.json；tests/platform 四道守門改模組感知：選單對等、跨組邊基線、final_drill 冒煙清單（新增 `smoke_plan`）、IP-1／獎金發放串接點題）｜8f35be9b｜閘門（M08 相關＋tests/platform＋側欄／五頁 e2e＋modules/analytics/tests，-n 2、低優先權）：1866 過，另 1 題 e2e 冒煙 45s 存檔逾時（負載；單跑過、第一次閘門過）；**反向控制**（刪 modules/analytics，--continue-on-collection-errors，同一範圍）：1670 過 3 紅＝§B-11 允許兩題＋`test_deploy_dashboard_jobs` 1 題（history.json 尾端多出殘字、單跑過，疑非原子寫入的產品競態，非 M08，已報主持）；探針：伺服器起、M08 端點 404、出納／T100（同 /api/reports 前綴）／公司查詢／銷售訂單／選單 200，正對照模組在時全紅；**D1b**：改 reports.py 66.8%→23.3%（3265→1141 題）、dashboard.py 25.1%→22.2%、reports.js 22.0%→21.8%（契約 838 題固定）｜fixture 層：無；**main.py：有**（拿掉兩支 router 與月報排程，改 ModuleSpec）；L1 新增（CORE 暫取 1.22，列車上 core_bump 重定）〔07:09 B 登記〕
