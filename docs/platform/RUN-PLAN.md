...
- **列車月台**（PLAYBOOK §G3；各線登記：分支｜HEAD｜差異題結果｜是否動 fixture 層／main）：
  ...
  - B｜`wip/b-m08-s`（稽核 ⑰ S-1～S-8、O-1～O-9：採納 14 項各附突變、S-2 不改碼、S-4 文件路線、O-4 部分採納、O-9 暫緩寫進 ROADMAP；含 --prune 修正（原本會在拿掉模組的樹上刪真實的邊、並洗掉其他鍵）；analytics 1.0.1（sales-orders.html 改歸 M01、permissions、urllib）；**疊在 wip/b-m08-3 上**，第六班合回後 --onto rebase）｜2141dec1｜閘門（tests/platform＋modules/analytics/tests＋受影響 10 檔，-n 2、低優先權）1307 過 0 紅（943d0bfe；之後只加 ROADMAP 一行）；D 複核 16 項已關閉（d4e909f8），O-9 待關｜fixture 層：無；main.py：無；**須排在 b-m08-3 之後**〔11:41 B 登記〕
  - B｜`wip/b-routes-2`（取代 wip/b-routes；＋稽核 AUDIT-D-B-routes：RT-M1 錯誤②只看已安裝的群組（沒裝的模組不報、不列允許清單）、RT-S1 萬用寫法只接受「/*」結尾且至少第三層（跨分界、整個前綴都判不合格）。本體：modules.json 個別路由明列歸屬、優先於前綴；四種歸屬錯誤；origin 既有 5 條歸屬不明已處理）｜374e4c56｜（＋D 觀察：已搬遷群組必須寫 key，check_group_keys＋反向控制）本包題 17 過（RT-M1 兩題反向控制、RT-S1 格式七種＋分界）、突變（拿掉已安裝判斷）紅；sparse 樹拿掉 netplan 實跑 17 過；tests/platform 1022 過｜fixture 層：無；main.py：無；排第七班〔11:52 B 更新：取代 b-routes〕
