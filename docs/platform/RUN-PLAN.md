...
- **全量名額排隊**（PLAYBOOK §C-13，全機同時最多 2 組）：目前佔用＝B（3.12 全量，wip/b-venv）、R（wip/r-legal）。排隊：① C 補跑 wip/c-fix-x1 的 e2e 段（-n 4）② A wip/a-bonus 全量（動到 L1 email_notify／notification_prefs；R 合回並 rebase 之後）。名額一空出來，主持就通知下一位，並更新這一行。
