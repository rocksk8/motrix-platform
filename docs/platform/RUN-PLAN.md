...
- **列車月台**（PLAYBOOK §G3；各線登記：分支｜HEAD｜差異題結果｜是否動 fixture 層／main）：
  ...
  - B｜`wip/b-routes`（主持裁示 M06 ②：modules.json 個別路由明列歸屬、優先於前綴；dep_scan check_route_ownership——兩個群組明列、明列對不到、搶別人路由、歸屬不明四種錯誤；origin 既有 5 條歸屬不明一併處理（L1 補 /api/layout、/api/mail-types；M10 明列 /api/quotations/{quote_no}/network-plan）；MODULE-GUIDE 規則）｜ce329534｜本包題 7 過、突變（明列不優先）紅 2；tests/platform（-n 2、低優先權）1012 過｜fixture 層：無；main.py：無；M06 自己的 routes 由 C 搬遷時填〔11:41 B 登記〕
