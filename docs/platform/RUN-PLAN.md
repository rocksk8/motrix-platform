...
- **列車月台**（PLAYBOOK §G3；各線登記：分支｜HEAD｜差異題結果｜是否動 fixture 層／main）：
  ...
  - H｜`wip/h-hist`（部署儀表板歷史寫入加鎖＋原子取代：B 反向控制抓到的 history.json 殘字＝解除鎖定端點與 job 收尾同時讀改寫）｜f4c14dc5｜tests/platform -k deploy_dashboard/insights 117 過；新並發題＋突變（還原舊寫法 ⇒ 紅）｜fixture 層／main：無〔2026-09-26 07:44 H〕
