...
- **列車月台**（PLAYBOOK §G3；各線登記：分支｜HEAD｜差異題結果｜是否動 fixture 層／main）：
  ...
  - H｜`wip/h-o7`（O7：P9 排版器切換範圍的兩個競態——載入中編輯區 inert、序號丟棄舊回應；第五班全量抓到「發布第 2 版後表頭仍是第 1 版」）｜a234771d｜tender_radar P9 e2e＋test_p9_layout 7＋8 題綠；tests/platform 守門子集（-n 1）277 過；突變 2 項各紅｜只動 L1 前端＋模組測試；fixture 層／main：無｜待 B 稽核〔2026-09-26 10:34 H〕
  ...
  - B｜`wip/b-maps`（主持派工：產生檔一致性守門 tests/platform/test_generated_maps.py——dep_graph.json、test_map.json 必須等於現場重產，modules.json 歸屬錯誤必須是 0（直接呼叫 dep_scan.check_modules，涵蓋 js）；_boundaries.OWNED_KINDS 改取 dep_scan.ASSIGNED_KINDS（原本少 js、core，三支 js 未歸屬因此沒被抓）；modules.json M12 拿掉殘留的 '/api/settings'（主持裁示；all_routes 查證 M12 9 支路由無此前綴、reminder-send-failures 在 routers/system.py）；dep_scan 寫檔固定 LF）｜d25f7ef3｜tests/platform（-n 2、低優先權）1006 過（rebase 前）；rebase 到 a6dc4be6 並重產三份產生檔後本包守門＋boundaries＋unit_cards 46 過；反向控制：合成未歸屬 js、過期 dep_graph、過期 test_map 皆紅｜fixture 層：無；main.py：無；⚠ **產生檔每包都會衝突**：列車疊完照 §G3 重產三份產生檔後再跑全量（本守門就是在驗這一步有沒有做）〔09:48 B 登記〕
  - B｜`wip/b-o6`（O6：test_e2e_hard_cap 的探針目錄移出 tests/，改 backend/.hardcap_probe_<id>——別的題的子 pytest 收集時不會撞上它被刪（第二班全量 FileNotFoundError）；探針只需要吃到 backend/conftest.py；加靜態守門：探針目錄「.」開頭、不在 tests/、modules/ 底下）｜a8f60a28｜hard_cap 5 題過、探針目錄用完已刪（只動一個測試檔）｜fixture 層：無；main.py：無〔10:00 B 登記〕
  - B｜`wip/b-rebasecheck`（主持派工：modtest 不再叫各線跑全量，§G3——--rebase-check 影響大時建議差異題擴大＋tests/platform＋改到頁面的 e2e、全量交給列車、月台註明；一般模式自己動 fixture 層也照跑差異題、閘門過了 exit 3＝要註明排車頭；need_full 改名 high_impact；MODULE-GUIDE §7、PLAYBOOK §G1〔更正〕）｜e0137ddc｜rebase-check／bookkeeping 28 過（新題：輸出不出現重跑全量／--full、建議不含 fixture 層檔）；tests/platform（-n 2、低優先權）1007 過｜fixture 層：無；main.py：無（只動 tools/platform/modtest.py 與它的題）〔10:26 B 登記〕
