...
- 2026-09-26 01:43 C：推上 origin——P4／P5 定義文件庫＋自訂欄位、P8 自訂模組引擎後端、A8d 前端公司名（`/api/system/branding`）、P8 前端缺口 #1（custom.<key> 權限可在網站授權）／#2（舊單帶回該版定義）、S-CC07 N-1（最新一份距今 >2 天 ⇒ 暫停清理＋告警；週／月層 2 個週期）；CORE 1.10。經過：原本只合進共用樹的本機 platform、沒有推上 origin（本機從 609cd5b8 分岔），rebase 到 origin 後 1.8、1.9 已被 O-9 與 A2 使用，改取 1.10。抓到的產品問題：通知在寫入交易內另開連線（database is locked 且靜默消失）、核准後簽核紀錄被清空。突變共 41 項皆紅。全量（8db7312f，-n 4、低優先權）：主段 4062 過、1 紅（VR1 跨日，主持已在 origin 補 09-26a），e2e 385 過；rebase 到 origin 後 `--rebase-check` 判定只跑 tests/platform。版本紀錄併入「系統」未發版的 09-26a（VR3）。待推：wip/c-p8-gaps2（缺口 #3～#7）、wip/c-p2-legal（P2 開票憑據稅別依據＋勞報單版型化與個資告知）。
- 2026-09-26 00:45 C：合回 A8d 前端部分。首頁頁尾與登入頁的公司名／統編改讀公開端點 `/api/system/branding`，統編只在帶有效登入時回傳；突變 2 項皆紅；登入頁／首頁相關 e2e＋A8c 題共 96 過，tests/platform 528 過。初始帳號種子（`jeff`）屬販售前確認（U8），這輪不動。
- 2026-09-26 00:40 C：合回 P4／P5＋P8 後端（wip/c-p8，CORE 1.10；原先在本機以 1.8 合進共用樹、沒有推上 origin；rebase 到 origin 時 1.8、1.9 已被 O-9 與 A2 使用，改 1.10）。P5：定義文件庫 `core.definitions`，含草稿、發布、版本、差異、還原，套用順序 role＞company＞預設；另有每模組 migration 執行器 `core.migrations`。P4：自訂欄位的定義與 clean（L1；接進內建模組與畫面尚未做）。P2×P5：開票憑據依「凍結版本＞公司發布版＞預設」套版。P8：安全公式、定義驗證帶位置、文件式單據＋欄位索引（core v2）、分層簽核含條件、通知與事件、輸出、凍結版本、通用 API＋建構器輔助 API，規格見 CUSTOMIZATION-SPEC §3.7。抓到的產品問題：通知在寫入交易內另開連線 ⇒ database is locked 且靜默消失，改成 commit 後才送；核准後簽核紀錄被清空。突變 33 項全部轉紅。全量（89383bb4，-n 4、低優先權）：主段 3976 過、5 紅，e2e 377 過。其中 4 紅是本批守門（router 註冊、寫入稽核、EXEMPT、交易內 `_notify` 文字），已修（6cffb19a），修後受影響題 89 過＋tests/platform 549 過；第 5 紅＝O1，由 856e5497 修。建構器前端與 D4 由主持做。
- 2026-09-26 00:40 C：合回 P4／P5＋P8 後端（wip/c-p8，CORE 1.9；原先在本機以 1.8 合進共用樹、沒有推上 origin，與 O-9 的 1.8 撞號，rebase 到 origin 後改 1.9）。P5：定義文件庫 `core.definitions`，含草稿、發布、版本、差異、還原，套用順序 role＞company＞預設；另有每模組 migration 執行器 `core.migrations`。P4：自訂欄位的定義與 clean（L1；接進內建模組與畫面尚未做）。P2×P5：開票憑據依「凍結版本＞公司發布版＞預設」套版。P8：安全公式、定義驗證帶位置、文件式單據＋欄位索引（core v2）、分層簽核含條件、通知與事件、輸出、凍結版本、通用 API＋建構器輔助 API，規格見 CUSTOMIZATION-SPEC §3.7。抓到的產品問題：通知在寫入交易內另開連線 ⇒ database is locked 且靜默消失，改成 commit 後才送；核准後簽核紀錄被清空。突變 33 項全部轉紅。全量（89383bb4，-n 4、低優先權）：主段 3976 過、5 紅，e2e 377 過。其中 4 紅是本批守門（router 註冊、寫入稽核、EXEMPT、交易內 `_notify` 文字），已修（6cffb19a），修後受影響題 89 過＋tests/platform 549 過；第 5 紅＝O1，由 856e5497 修。建構器前端與 D4 由主持做。
- 2026-09-26 00:40 C：合回 P4／P5＋P8 後端（wip/c-p8，CORE 1.8）。P5：定義文件庫 `core.definitions`，含草稿、發布、版本、差異、還原，套用順序 role＞company＞預設；另有每模組 migration 執行器 `core.migrations`。P4：自訂欄位的定義與 clean（L1；接進內建模組與畫面尚未做）。P2×P5：開票憑據依「凍結版本＞公司發布版＞預設」套版。P8：安全公式、定義驗證帶位置、文件式單據＋欄位索引（core v2）、分層簽核含條件、通知與事件、輸出、凍結版本、通用 API＋建構器輔助 API，規格見 CUSTOMIZATION-SPEC §3.7。抓到的產品問題：通知在寫入交易內另開連線 ⇒ database is locked 且靜默消失，改成 commit 後才送；核准後簽核紀錄被清空。突變 33 項全部轉紅。全量（89383bb4，-n 4、低優先權）：主段 3976 過、5 紅，e2e 377 過。其中 4 紅是本批守門（router 註冊、寫入稽核、EXEMPT、交易內 `_notify` 文字），已修（6cffb19a），修後受影響題 89 過＋tests/platform 549 過；第 5 紅＝O1，由 856e5497 修。建構器前端與 D4 由主持做。
