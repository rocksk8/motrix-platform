...
| 3 驗證 | 在正式 port 以外的 port 啟動新版；`/api/ping`；各表列數與轉換前相同；關鍵設定逐項相等；資料目錄清單與轉換前相同 | 任一不過 ⇒ 自動進入回滾 |
| 4 回滾 | **完整回滾**：還原程式快照＋DB＋設定檔，雜湊與備份一致，啟動 V9 驗證 ping。**只回程式**：保留轉換後寫入的資料，只換回 V9 程式（依 §6 只新增的原則，V9 讀得了新庫） | 兩種模式都要有自動化測試：轉換 → 回滾 → 雜湊逐一相等、V9 可以啟動 |
