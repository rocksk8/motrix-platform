...
| 公式 | `helpers.formula`：數字、字串、欄位 key、`+ - * / %`、比較、`and／or／not`、`if(條件, 是, 否)`、`round`（**四捨五入**，與法規金額同一支 `helpers.legal_params.round_half_up`；不是銀行家捨入，稽核 D C-M4）、`min`、`max`、`sum`、`abs`、`coalesce`、`days_between`。只能一行，最長 500 字。**空值不等於 0**（`coalesce(x, 0)` 才當 0）；除以 0 ⇒ 那一欄空值並回報。循環引用在發布前擋下。數字欄位不收 NaN／無限大（稽核 D C-M5）；發布前用樣本資料實際算一次公式與簽核條件，型別錯誤在發布時就指出位置（C-S1） |
...
| 版本不可改 | 已生效（`effectiveFrom ≤ 今天`）的版本不能修改或刪除，只能新增；尚未生效的可以改或刪。要更正已生效的數字 ⇒ 新增一版。**更正的做法**（稽核 O-1）：新增的一版可以是「過去生效」，插入之後，該生效日以後**新開**的單據改用新版；已開的單有快照，不受影響（要套新版得逐張勾「重新套用規則」）。生效日不可與既有版本重複 ⇒ 整年更正用次一日（例：01-02）。〔O-2 待裁示：「開單日期」是否等同給付日——扣繳義務在給付時發生，12 月開單、1 月給付會套到開單年度〕 |
...
| 金額捨入 | 補充保費四捨五入到元（健保署「角以下 4 捨 5 入」）、扣繳稅額元以下捨去；一律用 L1 `round_half_up`／`floor_amount`（前端 `static/legal-round.js` 同一套整數算法，畫面試算＝存檔）。2026-09-26 稽核 D-1 之前後端是銀行家捨入（35,000 ⇒ 738） |
| 並行修改 | 修改勞報單時讀舊單、合併已告知紀錄與快照、寫回都在同一個寫交易（`core.txn.write_txn`）；兩人同時修改不會清掉已記錄的告知（稽核 D-2） |
| 日期 | 新單的開單日期預設＝**台北時間**的今天（原本是 UTC，台灣 00:00～07:59 會變成前一天，而日期決定法規版本；稽核 S-1） |
...
| 選項 | 零稅率：營業稅法 §7 第 1～9 款＋「其他法律規定」；免稅：§8 第一項第 1～32 款**逐字條文**（第 7 款已刪除，不列）＋「其他法律規定」；出處全國法規資料庫 G0340080 flno=8（2026-09-26 查，整編截止 115-09-18），畫面顯示出處。「其他」一律要填說明，§8 各款說明選填。〔原句（R）：「免稅：§8 第一項（款次與說明由使用者填）＋『其他法律規定』。『其他』與 §8 一律要填說明」⇒ 稽核 S-4 改逐款；R2 初版的代碼 `8` 只在 platform 分支存在過，舊資料照常顯示，再送出時要改選款次〕 |
...
| 已告知紀錄 | 勾選「已告知當事人」⇒ **伺服器**記下時間、人員與告知文字的雜湊；已記錄的不能被覆蓋或清除（並行修改也一樣，稽核 D-2）。勞報單存在 `data_json.privacyNotice`；承攬商存在 L1 設定鍵 `privacy_notice_acks`（`contractor:<id>`），並寫稽核紀錄 |
| 告知全文 | 寫入新紀錄時，當時的告知全文存進設定鍵 `privacy_notice_texts`（雜湊 → `{text, firstAckAt}`，只增不改）；`GET /api/legal-params/privacy-notice/texts/{hash}` 查回（稽核 S-5）。本功能之前的紀錄沒有全文 ⇒ 404 |
| 紀錄損毀 | `privacy_notice_acks`／`privacy_notice_texts` 讀不懂 ⇒ 拒絕寫入（409）並記 ERROR，原值不動；讀取也回 409（不顯示成「尚未告知」）。沒勾已告知的存檔不受影響（稽核 S-3） |
| 告知書日期 | 台北時間（稽核 S-2） |
