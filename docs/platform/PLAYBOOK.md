...
| ② 合回閘門 | push 到 platform 前 | ①＋tests/platform（契約與守門）＋有改到的頁面的 e2e；`--rebase-check` 判定要不要升到③ | ≤20 分鐘 |
| ③ 批次全量 | 每批合回之後、動到 fixture 層時 | 全量（2 個名額、-n 4、低優先權） | ≤60 分鐘 |
