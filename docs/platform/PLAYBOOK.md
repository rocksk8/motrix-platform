...
| 15 | **worktree 與暫存目錄用完就收**（2026-09-26 21:32，使用者問「測試是否一直複製創建」，實查 worktree 87 個（約 4.3 GB）、%TEMP% 測試目錄 1228 個（3.5 GB，其中 1055 個是 e2e hardcap 空目錄的漏洞））：分支合回 origin 或被 -N 取代後，**自己的** worktree 當輪用 `git worktree remove <路徑>` 逐一刪（有未提交內容先核對是否已在 origin，再 --force）；反向控制樹、稽核樹做完就刪；保留的只有「在月台／進行中／文件用」。巡視時 worktree 數超過 30 個就請各線清理。第一次清理：87→24 |
...
### G4. 省 token 而不降品質（使用者 2026-09-26 15:06 裁示：「思考如何減少 token 消耗同時保有品質跟穩定」，表單採用全部六項）

- **回報格式（各窗→主持、主持→使用者）**：≤6 行固定格式——①分支／SHA ②結果（數字：過／紅／突變） ③紅燈與處理 ④要裁示什麼（沒有就不寫）。細節、證據、原因寫進 AUDIT 檔或 RUN-PLAN，不寫在訊息裡；主持只在里程碑（列車合回、D 項完成）或需要使用者決定時回報使用者。
...
- **列車長清單**（派工只寫「第 N 班、乘客（分支＋SHA＋稽核狀態＋同班／相依條件）、例外」，其餘照這張清單；列車長子代理用 model: sonnet，稽核維持 Opus）：
  ...
  3. 列車上 core_bump 取號、IP 照 origin 已用號碼往後定號（引用處同步、歷史不改寫）、模組版號衝突往後排；重產 UNIT-INDEX／dep_graph／test_map 單獨一個 commit，三者 --check 一致。
  ...
  8. fetch → rebase 到 origin/platform（帶進程式改動就重跑受影響題）→ **另一個指令** `git push origin HEAD:platform`（不 force）；RUN-PLAN §6 記一筆、§5 月台移除已合回的包。
  9. 回報照上方格式（≤6 行）＋取號／IP 定號一行。
  10. （2026-09-26 18:32 補，第九班經驗）Python 一律用 `D:\MOTRIX-PLATFORM\.venv312\Scripts\python.exe`，不用 PATH 上的；**本班新功能相關的紅不可以判偶發**，交作者查（〈偶發失敗先當產品競態〉）；結束前確認自己沒有背景工作還在跑。
  11. （2026-09-26 23:32 補，第十班假綠）**判定一律看 modtest 的 exit code，並逐批讀 pytest 結果行**：選題超過命令列長度會分批，最後印的摘要只是最後一批（第十班 exit=1 而報「1993 過 0 紅」，第 1 批的 3 紅漏上 origin）。回報寫「共 N 批、各批過／紅」；exit≠0 而找不到紅＝不可以合回，交主持。wip/b-modtest-batch 修好後工具會印總摘要，本條仍保留。
