...
11. 反向控制：在另一個 worktree 刪掉這個模組的資料夾，伺服器要能啟動、ping 200、該模組的端點回 404；其餘測試除了「modules.json 列了但掃描不到」這一題，全部要綠。〔補充（2026-09-26 04:58 主持，D 稽核 M12）：允許紅的**產生檔一致性題**另加一題 `test_unit_index_is_current`（UNIT-INDEX 會反映實際的樹，拿掉模組之後，直接使用者數會變，這是預期的）。除此之外沒有其他例外；需要該模組才能跑的題，一律要搬進模組的 tests/。**反向控制的範圍**：tests/platform＋所有提到該模組的測試檔（grep 模組 key 與路徑），不可以只跑 tests/platform〕
