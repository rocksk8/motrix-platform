...
- e2e：開瀏覽器的題一律帶 `@pytest.mark.e2e`（逐題死線、全量的 e2e 段都看它，不看檔名）；**瀏覽器 fixture 一律經 `new_context`**
  （新的 fixture 依賴 `new_context`／`e2e_browser`／`new_page`，不自己呼叫 `sync_playwright`）——「用瀏覽器的題必有 marker」
  的守門靠 fixture 依賴認題（稽核 D MB-M1 射程）。守門：`tests/platform/test_e2e_classification.py`。
...
- 模組要能被選配，`module.json` 必須宣告 `version`、`core`、`pages`、`provides.api_prefixes`；另宣告 `provides.probes`（演練用的 GET 端點：真的路由、落在自己的前綴底下、模組在時最高管理者打它回 200）——`product_drill` 依 probes 驗證端點在或不在（2026-09-26 稽核 ⑰ M-3：原本拿前綴本身當端點打，前綴常常不是路由 ⇒ 完整產品假紅、排除時假綠）。沒宣告 probes 的模組：演練不打它的端點、不判紅，但列在輸出 `undeclared_probes`（主持過渡裁示；各模組擁有者補上）。守門：`tests/platform/test_product_drill_probes.py`。
  - probe 必須**純讀、無副作用**：不寫表、不寄信、不起背景工作、不記「已讀」（首次讀取的懶初始化要冪等）；**probe 路由不可以 log 回應內容**（演練目錄會保存伺服器 log；回應可能含個資）。演練工具只記狀態碼。守門：`tests/platform/test_probe_side_effects.py`（`test_declared_probes_are_read_only_and_do_not_log_their_responses`：前後每張表列數＋內容雜湊、smtplib、Thread.start、caplog／capsys 比對回應裡長度 ≥4 的字串值；`test_drill_tools_record_only_status_codes`；合成路由的反向控制）。〔主持裁示 2026-09-26，D7-CHECKLIST §4〕
...
- **蒐集自然人個資的表單都要有告知**（2026-09-26）：清單 `docs/platform/pii_forms.json` 列出「有個資輸入欄位的頁面 ⇒ 決定」，每一頁剛好一種：`notice`（頁面有告知區塊＝`data-privacy-card`／`data-print-notice`／`data-privacy-ack`／`data-privacy-missing`＋載入 `static/privacy-notice.js`，並列出伺服器紀錄端點 `ack_api`）、`covered_by`（個資由另一張 `notice` 表單帶入）、`not_natural_person`（法人資料）。**每一種決定都要逐欄列 `fields`**（`notice` 也是：稽核 D PN-M1，主持裁示 2026-09-26）；一頁上有兩種以上當事人時用 `notices`（清單，每個對象各有 `subject`、`fields`、紀錄端點，頁面上各有 `data-privacy-subject="<subject>"` 區塊；`notices` 內也可以逐欄寫 `covered_by`／`not_natural_person`）。所有對象的 `fields` 聯集＝掃描結果，一欄只能屬於一個對象。後兩種要寫 `reason`；身分證號、生日不可以用後兩種帶過。紀錄一律走 `helpers.privacy_notice.record_purpose_ack`（設定鍵 `privacy_notice_acks`），告知文字依用途（`PURPOSES`：承攬／聯絡人／帳號）分開。新增這類表單：照抄 `contractors.html`（單一當事人）或 `customers.html`（多位聯絡人，`MotrixPrivacyNotice.contactsState()`）或 `network-plan-form.html`（單據上手動輸入的聯絡人，`subjectState()`，紀錄鍵含姓名、只接受已存檔的那一位）的區塊，並在清單加一條。**主持裁示 2026-09-26：單據上可以手動輸入聯絡人 ⇒ 就是在蒐集個資，要 `notice`，不可以用 `covered_by` 帶過**（`covered_by` 只給「只能從主檔選、不能手打」的欄位）。守門：`tests/platform/test_pii_forms_notice.py`（掃描規則 `tests/platform/_pii_forms.py`：`x-model` 綁定最後一段的個資字尾）——新頁面有個資欄位而清單沒有決定、`notice` 頁拿掉區塊或端點、**任何一頁多一個個資欄位（含告知頁）**、一欄屬於兩個對象、兩個告知對象卻少一個區塊、過期的決定 ⇒ 紅；正對照＝暫存頁面＋承攬人員名冊；反向控制＝「全部寫成豁免」要紅。「不能手打」只認靜態的 `readonly`／`disabled` **屬性名稱**（屬性值裡的同名單字、`:disabled` 都不算；稽核 D PN-S3）。⚠ 守不到：不經 `x-model` 的輸入。
