...
> **量測基準 HEAD `f7145becebc2e7d0f2d69b47810f069934208fe0`**：本文所有 `file:line` 都以這一版為準。
> 讀取方式：先把這個 SHA 匯出成唯讀快照，再從快照讀取。工作樹當時有 dirty 檔（`quotations.py`、`approval-queue.html`），而且本文撰寫期間 HEAD 已經前進到 `318b9f1`，所以行號會慢慢過期（見 `MULTIWIN-PROTOCOL.md §5w`）。
...
| key 散在哪 | ① `helpers/auth.py:22` `_SUPERADMIN_MODULES`<br>② `frontend/pages/users.html:718` `ROLE_MODULES`、:792 `allModules`（權限目錄）<br>③ `frontend/static/sidebar.js:466-506` `computeFlags`、:552 `_FILE_MODULE`（只給徽章已讀用，:1074）、:1040 起 `_MOD_BADGES`<br>④ `routers/system.py:349-370` `_MODULE_ACTION_PREFIXES`（稽核徽章用）<br>⑤ `trail.py:62` `MODULE_LABELS`<br>⑥ `static/notif.js:128` `modBadge`<br>⑦ 各 router 自己的常數，例如 `routers/vouchers.py:69` `_VOUCHER_MODULES`<br>⑧ `db.py` v84 的一次性回填樣板（約 :3760-3775）<br>**`helpers/licensing.py` 沒有自己的 key 清單**：它只把授權檔的 `modules` 原樣往下傳（:117、:380）。 |
...
| 項目 | 內容 |
|---|---|
| 在哪 | 共用：`db.py:5486` `next_entity_code(conn, table, prefix, code_col="code")`，做法是 MAX+1，遇到已存在的號就用迴圈跳過。<br>各自一套的有四支：報價 `_peek_next_no`＋`quote_seq`（`routers/quotations.py:658`，撞號重試一次後回 409，:1281-1290）；薪資 `payslips.py:112`；傳票 `helpers/voucher.py:252`（不加鎖，靠 UNIQUE 擋，:268-273）；料號 `parts.py:26`。 |
...
| 項目 | 內容 |
|---|---|
| 在哪 | `frontend/static/sidebar.js`：`computeFlags` :466-506、`_FILE_MODULE` :552、`_navGroups` :609、`ni()` :612、`sec()` :628、`renderMainNav` :637、`buildSidebar` :681 起。<br>頁面層禁入：`_deniedPages`（:604、:614、:837）由 `ni()` 的顯示條件推出，命中時呼叫 `_showNoPermission`（:932）。<br>`static/auth-guard.js` **只驗 session，不驗模組**（:22-57）。 |
...
**前提**：`frontend/css/style.css` 共 1668 行，**沒有表單類，也沒有被頁面使用的清單＋明細分割類**。61 頁中有 59 頁自帶 `<style>`（只有 `receivables.html`、`sales-orders.html` 沒有）。因此下面「照這頁做」挑的是**最接近共用類的現存頁**，不是理想樣板。
...
| 頁面骨架 | `.topbar` :215、`.main` :437、`.mnav` :454、`.page-header` :507。<br>**新頁建議用 `.pg / .pg__eyebrow / __t / __s / __a`**（:540-583） | `warranty.html`（`.pg*` 在 :94-98） | 25 頁自己定義 `.page-main` 取代 `.main`，不要再增加。 |
| 清單＋明細 | 共用類只有 `.main--queue`（:1446）和 `.queue-view-container`（:1457），**沒有任何頁面使用** | `payslips.html`：共用 `.panel`＋`.data-table`（:102-104），配本頁的 `.detail-pane`（:34）。`customers.html` 同一模式（:118、:344） | 兩頁都各自重寫了 `.data-table`（payslips :20-25、customers :58-70）。**平台層應該把 detail-pane 收成共用類再開始用。** |
| Modal | `.modal-overlay` :1475、`.modal-box` :1483（max-height `calc(100dvh - 2rem)`）、`.modal-head/body/foot` :1491-1502。手機版在 :1373-1385 | `customers.html`：modal 放在 body 直下（:476、:529） | `account-items.html` 只用共用類（:167-186），但它的 modal 包在 `<main>` 裡（:60-61）。依 :139-144 的註解，深色模式下 fixed 定位可能跑版（未實測）。 |
| 表單元件 | **沒有共用類** | `customers.html`：`.form-grid/-2/-3/.form-group/.form-label/.form-input`（:99-116），用法見 :619-751 | 這是 15 頁以上複製的寫法，但仍屬本頁自訂。 |
| 狀態徽章 | `.badge` :915＋修飾類 `--draft/pending/signing/approved/sent/done/danger/rejected/warning/running/settled/lost/active/repair`（:926-944）。<br>視覺化燈號 `.vm-*`（:1535-1555）：品牌紅不參與狀態色（:1511-1530） | `quotations.html`（:287，對照表在 :649-660，沒有本地重寫） | 主要動作和危險動作靠形狀區分，不靠顏色（:55-57）。 |
| 空狀態 | `.empty / __icon / __text`（:1188-1194） | `warranty.html:147` | payslips、customers 用自訂的 `.empty-state`，不要照做。 |
| 按鈕 | `.btn` :952、`-primary` :971、`-ghost` :974、`-danger` :977、`-sm` :983、`-approve/-reject` :987-996、`.btn-icon` :1001 | `quotations.html` | — |
...
| 字級 zoom | `static/sidebar.js:62-70`：`FZ_STEPS` 0.85/1/1.15/1.3，設定 `document.documentElement.style.zoom`；`motrixSetZoom` :114-116 | — | 🔴 **已知問題**（`docs/windows/HANDOFF-PENDING-2026-09-23.md:187-211`）：根元素 zoom 會把 vh/dvh 一起放大，fixed 元素超出畫面。1366×768 在「特」字級時，`90vh` 彈窗的底邊超出 180px（:192-199）。<br>修法排在傳票之後（:211、:226）。**新頁面暫時不要用 `Nvh` 限制 fixed 元素的高度**，或預先寫成 `calc(Nvh / var(--fz,1))`，等修法落地後就能生效。<br>`.mnav__panel`（:475-483）目前沒有 max-height。**沒有任何測試守 zoom。** |
| 斷點 | 1200（:1250）、1100（:500）、1023（:1419）、768–1023（:1258）、767（:1305、:1428、:1464、:1664）、479（:1399） | — | 新頁只用這組寬度，不要新增斷點。 |
...
| 傳票 ← 各單據 | `vouchers.py:460-477`（派工、額外支出）、:526（報價）；附件取自 `helpers/voucher_attachments.py:109`、:140、:297-303 |
...
| 9 | 側欄項目與分組條件 | `sidebar.js:609-681` | `test_sidebar_finance:334/500/550`；e2e 清單 `test_e2e_page_module_guard:65` 要手動補（漏了不會紅） |
...
### 5.0 先要裁示的一件事：「登記表加一行」算不算改原碼

第 4 節的清單說明：**任何新模組都一定要在既有檔案的登記表裡加列**，包括 `main.py:28`、`db.py:342`、`archive.py:1506`、`users.html:792`、`sidebar.js`、`version_manifest.json`。完全不碰既有檔案，平台層連掛載都做不到。

**建議**：把「修改」分成兩類，各自裁示。
- **A 類「附加登記」**：在既有清單的尾端加一列，不改任何既有列、不改邏輯。**視為允許**。一致性測試本來就是為這類動作設計的，會自動驗證。
- **B 類「修改行為」**：改動既有函式的邏輯或既有列的值。**每一點個別裁示**（也就是盤點 §4 那四點）。

下面 5.6 的每一列都會標明它屬於 A 類還是 B 類。

### 5.1 定義登錄（doc_types／fields／menus）

**建議做法：第一階段不開新表，存在 `system_settings`，並使用 `platform.` 命名空間。**

理由（都是讀碼確認的）：
- `system_settings` 已經在每日 JSON 備份內（`archive.py:1583-1587`），也已經分類為展示時整張清（`db.py:364`）。**不需要 migration、不需要改展示清單或備份清單，一致性測試全部不受影響。**
- 讀寫接點已經存在：`helpers/settings.py:11` `_get_setting`、:28 `_set_setting`。前例是條款組 `quote_terms_presets`（`routers/system.py:1672-1723`）。

| key | 形狀（草案） |
|---|---|
| `platform.doc_types` | `{"<docType>": {label, baseDocType?: "quotation"…, moduleKey, numberPrefix, templateId, approvalDocType}}`。`baseDocType` 有值時表示「延伸一張既有單據」，沒有值時表示「全新單據」（P5） |
| `platform.fields.<docType>` | `[{key:"cf_xxx", label, type:"text\|number\|date\|select\|bool", options?, required?, order, section, showOnPdf}]` |
| `platform.menus` | `[{group, label, href, moduleKey, order}]`，只能**追加**到既有選單之後，不能改寫既有項目 |

**升級條件**：定義數量或查詢需求超過一個 JSON 值能負擔時，再改成實表 `pf_doc_types`、`pf_fields`、`pf_menus`。屆時照第 4 節 1-4 步做（A 類）。

### 5.2 customFields 命名空間

- 位置：`data_json.customFields = {"cf_<snake>": value}`。**鍵一律以 `cf_` 開頭**，保證不會和核心鍵相撞；核心鍵清單見盤點 §2。
- 值只存原始值，不存標籤。標籤從「凍結的樣板版本」取（5.3），所以之後改標籤不會回溯改寫舊單據。
- 驗證：存檔時比對定義，未知的 `cf_*` 一律**拒絕**，不可以靜默丟掉。前例是 `voucher_template.validate_template_body` 在儲存當下驗證（`helpers/voucher_template.py:100`）。
- 反面教材：`vouchers_all.custom_fields`（`db.py:5265`）**有 schema，但全 backend 沒有任何寫入點**（grep `custom_fields` 在 routers 與 helpers 都是 0 筆）。驗收必須釘死「寫入 → 讀回 → PDF 印出」整條路（盤點原則 5）。

### 5.3 樣板版本化與凍結

- **表結構前例**：兩表制，`<x>_templates` 管穩定的 id，`<x>_template_versions` 管歷史；每次編輯都新增一列，不覆蓋（`db.py:5170-5180`，傳票）。獎金也是同一個模式（`bonus_template_versions`，`db.py:5041`）。
- **凍結前例**：`company_identity.snapshot_for` 在送出當下寫入 `data_json["locationIdentity"]`，事後不重查（`company_identity.py:163-186`）。
- 命名草案：單據送審時寫入 `data_json.platformTemplate = {id, version, frozenAt}`，同時把該版欄位定義快照到 `data_json.platformTemplate.fields`。舊單據永遠用自己的快照顯示。這也補上盤點提過的缺口：`FORM_VERSION` 只顯示、沒存（`quotation-form.html:2224`）。
- 第一階段存在 `system_settings`：`platform.templates.<docType>` 存成 `{currentVersion, versions:[{version, fields, output, createdAt, createdBy}]}`，**只 append，不改寫**。

### 5.4 簽核條件

- 命名草案：`platform.approval_rules.<docType>` 存成 `[{when:{field, op:"gt|gte|eq|in", value}, flowKey}]`。規則由上往下比對，第一個命中的生效；都沒命中就回到既有的 `resolve_active_flow_setting` 結果。
- `flowKey` 指向一把 `system_settings` key，裡面的形狀和既有 flow 相同：`{tiers:[...], includeSubmitterManagerTier}`（`tiered_approval.py:296-324`）。拿到之後交給 `setting_to_active_tiers` 展開。**引擎本體不需要修改。**
- `field` 只能引用核心鍵的唯讀值（例如 `tot.total`），或 `cf_*`。
- ⚠ 限制：既有單據的流程選擇寫在各 router 的送審端點（例如 `case_extra_expenses.py:336`、`bonus.py:1171`），所以**要對既有單據套條件，必須改那一行（B 類）**。平台自建的新單據則可以直接用，不違反約束。

### 5.5 輸出樣板

- **語言**：沿用 `voucher_template.py` 的模式：封閉的佔位符清單（:36）、每個佔位符都有取值來源（`PLACEHOLDER_RESOLVERS` :52，兩個方向都要守）、單層大括號語法（:77）、在**儲存樣板時**驗證（:100、:112）。每種 doc type 各有一份清單，`cf_*` 由欄位定義自動加入。
- **渲染**：平台層自己組 HTML，但必須走 `run_edge_pdf`（`helpers/startup.py:75`，自動取得 `EDGE_PDF_SEMAPHORE`）並檢查 0 byte。抬頭用 `apply_snapshot(location_identity(...), payload)`（`company_identity.py:82`、:220）。
- **一份定義驅動所有出口**：表單、預覽、PDF 都讀 5.3 的同一份凍結定義，避免重演「報價預覽和正式 PDF 各自寫死」（盤點 §1 輸出層）。
- 範圍依盤點 R1：只做單據 PDF，不做多段式報表。

### 5.6 必須先抽成可引用接點的既有程式碼

| # | 接點 | 現況 | 平台層的用法 | 違反「不改原碼」嗎 |
|---|---|---|---|---|
| 1 | `run_edge_pdf` | 公開函式，`startup.py:75` | 直接 import | **不違反** |
| 2 | `_identity_head` / `_identity_foot` | `pdf_gen.py` 的私有函式（:522、:538） | 直接 import 私有函式，或平台層自己實作一份抬頭 | import 不違反，但會綁住私有 API；**若要搬進 helper，屬於 B 類** |
| 3 | `location_identity` / `snapshot_for` / `apply_snapshot` | 公開，`company_identity.py` | 直接用 | **不違反** |
| 4 | `tiered_approval` 全部函式 | 公開、純邏輯 | 平台新單據直接用 | **不違反**；新 doc type 加進 `APPROVAL_DOC_TYPES`（:54）屬於 A 類 |
| 5 | 既有單據套用簽核條件 | 流程選擇寫在各 router | 改送審那一行 | **B 類**（5.4） |
| 6 | 簽核佇列 | `get_approval_queue` 每種單據一段 SQL（`quotations.py:3784`） | 平台單據要出現在佇列裡，就必須加一段 | **B 類**，無法避免；受 `test_queue_and_feed_scoping:158` 約束 |
| 7 | `_notify` / `_audit` | 公開，`helpers/audit.py:11`、:92 | 直接用 | **不違反** |
| 8 | email 事件 | `EVENT_GROUPS` 寫死 | 新事件要加列 | **A 類** |
| 9 | `next_entity_code` | 公開，`db.py:5486` | 直接用，並自己接住 IntegrityError | **不違反** |
| 10 | `save_document_files` | 公開，`helpers/uploads.py:41` | 直接用，**但呼叫前要先驗 `doc_no` 沒有穿越目錄**（:63 的缺口） | **不違反**；在原函式補檢查屬於 B 類 |
| 11 | `_get_setting` / `_set_setting` | 公開，`helpers/settings.py:11/28` | 定義登錄的存取層 | **不違反** |
| 12 | 模組 key 單一來源 | 散在 8 處（1.1） | 平台層只讀一份自己的登錄 | 新增登錄**不違反**；**要讓既有 8 處改讀它，屬於 B 類**，而且必須同步改寫 9 支用 regex 讀 sidebar.js 的測試 |
| 13 | 選單追加 | `sidebar.js` 寫死 | 在 `_navGroups`（:609）渲染前合併 `GET /api/nav` 的結果 | **B 類**（盤點 §4） |
| 14 | 報價表單、PDF 掛載點 | Alpine 手寫；`_build_quote_html` 是 f-string（`pdf_gen.py:96`） | 加掛載點或 hook | **B 類**（盤點 §4） |
| 15 | 平台 router 掛載 | `main.py:28`、:617-662 | 加一個 include | **A 類** |
| 16 | caseRecord 存取層 | 5 處以上直接讀 JSON 形狀（3.4） | 平台層不要再多讀一處。需要時新增唯讀 helper | 新增 helper 不違反；**要讓既有讀取改走它，屬於 B 類** |

**建議順序**（沿用盤點 §5 P1）：先用 #1、#3、#9、#10、#11 這些「不違反」的接點，加上 A 類 #15，做出「平台自建的一張新單據：定義 → 表單 → 存 → 讀回 → PDF」這條端到端細線。B 類 #5、#6、#13、#14 等細線站穩之後，再逐點裁示。

---

## 我沒查什麼

- **行號的時效**：全部以 `f7145be` 為準；撰寫期間 HEAD 已經前進到 `318b9f1`，沒有重新比對。
- **沒有跑任何測試、沒有啟動伺服器**。「會紅在哪」是讀測試原始碼推出來的，沒有實際讓它紅過一次。
- **讀碼方式**：大部分事實由 6 個唯讀 agent 讀快照蒐集，我抽查了約 40 個行號，全部相符。沒抽到的行號沒有逐一驗證。
- **有一次抽查發現錯誤並已更正**：agent 回報 `require_any_module` 有 134 處，實數是 105 行呼叫（扣掉定義與測試）。
- **沒有逐一打開的內容**：
  - 206 個測試檔中，只 grep 了掃描型的關鍵字，單一功能的守門可能有漏。
  - `test_spec_coverage_2026_09_21.py` 沒有細讀。
- **沒有實測的事項**：
  - vh/dvh 的處數：agent grep 出 53 檔 134 處，HANDOFF 寫 52 檔 132 處，沒有逐檔比對。
  - account-items 的 modal 在深色模式下是否真的跑版。
  - 字級 zoom 的實際影響。
- **沒有確認的事項**：
  - 前端 `bonus` 缺席於簽核設定頁（1.2）是否刻意。
  - `next_entity_code` 併發撞號時，各 router 是否回 500。
  - 完工單和薪資單的 PDF 是否另有歸檔路徑。
- **沒有查的範圍**：
  - 盤點 §7 列的兩項：其他單據表單的欄位寫死程度、`json_extract` 的效能。
  - `search`、`accounting_export`、`licensing` 的前端入口。
  - 手機版版型。
- **5.1 的疑慮**：用 `system_settings` 存定義，JSON 值大小與併發寫入（兩人同時編輯樣板）沒有評估。
- **5.0 的 A／B 類分法是本文提出的建議**，不是既有裁示。
