...
def _status(valid, reason, env=None, customer=None, modules=None,
    ...
    """七個欄位固定都在。缺欄位在前端會變成 undefined，比錯的值更難查。"""
    return {
        "valid":     valid,
        "reason":    reason,
        "env":       env,
...
def _read_license_file():
    """讀 LICENSE_PATH。回 (內容, 失敗原因)，其中一個必為 None。

    **在函式裡讀模組層級的 `LICENSE_PATH`**，所以測試 monkeypatch 得掉，
    而且每次呼叫都重讀——這是第 6 步「到期提醒」的前提。
    """
    path = LICENSE_PATH
    try:
...
def verify_license(blob=None):
    """驗一把金鑰，回 LicenseStatus（七個欄位）。**任何情況都不丟例外。**
    ...
    **簽章一定排在 machine／expires 之前**——簽章驗過之前 payload 沒有任何一個欄位
    ...
    for name, pem in (("dev", _PUBKEY_DEV), ("prod", _PUBKEY_PROD)):
        if not pem or not pem.strip():
            continue  # 本輪 _PUBKEY_PROD 是空的：略過，不丟例外
        try:
    ...
    if days_left is None:
        ...
        # 留一個例外會讓「malformed 就是 None」這條規則變成「大部分時候是 None」，
        # 而日後打包關卡是照規則寫的。
        return _unverified("malformed")
