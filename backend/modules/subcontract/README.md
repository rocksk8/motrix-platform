...
- 匯款申請的案件存取守門：案件表不存在 ⇒ 404（L1 `helpers.case_access`）。
...
- 承攬人員的個資告知紀錄仍存在 L1 設定鍵 `privacy_notice_acks`（`contractor:<id>`，R3）。ROADMAP 要求搬遷時改用本模組自己的表，但那需要模組 migration 加上把既有紀錄搬過去（轉換時的資料移轉與驗證），不在這次搬檔範圍，另開題。
