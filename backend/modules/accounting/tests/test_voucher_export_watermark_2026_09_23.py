...
# 🔑 與 `BN13` 互相指名——同一類缺陷的兩半

D 盤點：全庫「有作廢概念且能匯出 PDF」的只有傳票與獎金分潤單兩種，
根因同一個舊假設（作廢的單不會走正式匯出）沒跟上同一次政策放寬
（`JV15`／`BN8` 各自放行作廢單匯出，視覺標記沒有跟著補）。獎金單那一半
在 `test_bonus_award_export_watermark_2026_09_23.py`（`BN13`）——只修好
其中一半會被誤以為整件事已經完成，兩檔都要綠才算兩個編號都結案。
