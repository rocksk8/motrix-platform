...
#: `approval.reassign`：簽核鏈在 payment_requests.data_json.$.approval
REASSIGN = _aq.DataJsonApproval("payment_requests", "request_no")


def queue_items(conn) -> list:
    """`approval.queue_items`：待審核／簽核中的請款單（`type`＝`payment_request`）。"""
    rows = conn.execute("""
    ...
    for r in rows:
        ...
        out.append(_aq.base_item(
            "payment_request", r["request_no"], f,
...
def queue_detail(conn, doc_no):
    """`approval.detail`（payment_request）：簽核佇列詳情的單據內容；權限、案件抬頭、金額遮蔽在 M01。"""
    r = conn.execute("SELECT * FROM payment_requests WHERE request_no=?", (doc_no,)).fetchone()
