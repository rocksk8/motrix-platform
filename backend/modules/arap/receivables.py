"""應收收入與銷項發票的資料收集（M05 應收應付；2026-09-26 收回模組，ROADMAP A8b）。

〔沿革〕M08 搬遷 ③ 自 routers/reports.py 下沉 L1 `helpers/receivables.py`（中繼）；M05 搬遷時收回本模組，
對外只經 provider：`receivables.income_items`（M08 現金口徑收入）、`receivables.tax_invoices`（M08 稅務匯出、M06 T100）。
本模組不在 ⇒ 兩個使用方各自說出原因（不是回空清單）。以下是下沉時的原說明：
