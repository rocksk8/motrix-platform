...
## 1.31 — 2026-09-26（C，M01-PLAN §3-2：兩支通用函式下沉 L1；疊在 T 之上）〔core_bump：暫用 1.99 → 1.31〕
...
- L1（改 import 來源）：`helpers.norm_at`／`helpers._steps_to_tiers` 改自 L1 再匯出；`routers/system` 改自 tiered_approval import ⇒ system 對 M01 只剩 `quote_terms.DEFAULT_TERMS`（CA-O4）
