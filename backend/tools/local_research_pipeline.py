...
**⚠️ 只能在開發／測試機（hichan 帳號，有獨立顯卡）跑，正式機沒有 GPU 不要嘗試執行**：
正式機是 H255（低功耗 CPU，無獨立顯卡）架構，不具備跑本地 LLM 推論的硬體條件，也沒有安裝
Ollama；這支腳本純粹是「開發階段用來準備候選內容」的工具，產生的候選資料經 Claude/人工核對
後，一律透過既有的 `sync_*.py`（如 `sync_2026-08-09e_monitor_brands.py`）或 CRUD API 把最終
資料同步到正式機，正式機本身完全不需要、也不應該執行這支腳本或啟動 Ollama。此檔案雖然會隨
`git archive` 一起打包進部署包（跟其他 `backend/tools/` 裡的開發工具一樣），但那只是附帶
現象，不是要在正式機上跑的意思。
