<# 正式機診斷腳本 — 在正式機 Motrix 帳號的 V9.0 根目錄執行 #>
...
# 4. 檢查 log
Write-Host "`n[4] 最近 server.log 的關鍵錯誤..." -ForegroundColor Yellow
$logPath = "backend\logs\server.log"
if (Test-Path $logPath) {
    $tail = Get-Content $logPath -Tail 100 -Encoding UTF8 | Where-Object {
        $_ -match "ERROR" -and $_ -notmatch "_ProactorBasePipeTransport"
    }
    if ($tail) {
        $tail | Select-Object -Last 5 | ForEach-Object { Write-Host "  $_" }
    } else {
        Write-Host "✓ 最近無 ERROR（除了已知 asyncio 噪音）"
    }
} else {
