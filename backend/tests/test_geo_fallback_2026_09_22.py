...
def test_a14_a_remembered_miss_does_not_survive_a_restart(client, monkeypatch):
    """🔴 A14：**負快取不可以活過重啟**（`GC9` 的可觀測後果）。

    ## 🔴 原題名是 `test_a14_failures_are_not_cached_forever`，而它的前提被取代了
    ...
    ☠️ **而 `§15 補三` 定案的正是相反的事**：
    ```
    查無此地址（三階都 miss ＋ 退階抽不出行政區）→ **負快取，TTL 7 天**  ← 刻意的
    查詢失敗（配額／逾時／金鑰錯）                → 絕對不可以寫負快取
    ```
    🔑 ⇒ **不是這一題綠得可疑，是這一題的題目過期了。**
    A 2026-09-22 裁定走甲（改條文），理由不是「現況比較方便」，
    是 **`a14` 寫成的時間早於 `§15 補三` 的定案**。

    ## ⇒ 那兩半現在各自有人守，而且守得更準

    ```
    a14c   **錯誤**路徑：某一階查詢失敗 ⇒ 絕不寫負快取   ← a14 原本想守的
    a14d   **乾淨** miss ⇒ 仍然要記住                    ← 與 a14 原意正面相反，
                                                          而它才是條文
    a14    **本題**：重啟之後不可以還留著                ← GC9（不落 DB）的後果，
                                                          本來沒有任何題在守它
    ```

    ## ⚠️ 而它轉綠的原因我留著，因為它是一個教訓

    ```
    2026-09-22 17:45  我改 _forget_memory() 讓它也清 _MISS_CACHE
    ⇒ 這一題從紅變綠，**而 B 的產品修正對它無效**
      （B 改的是錯誤路徑，而這一題走的是乾淨 miss）
    ```
    ☠️ **它不是被修好的，是被我換了測量條件。**
    🔑 而那個 helper 改動**本身是對的**（負快取是行程內的，重啟真的會清掉）——
    📌 危險的正是這一點：**一個技術上正確的修正，把一個未解的衝突消化掉了。**
    ⇒ 判準：**當一個紅燈在「一個與它無關的修正」之後變綠，
    要問「它是被修好的，還是被換了測量條件」。**

    ## 📌 本題現在的判準

    🔑 只驗「**模擬重啟之後，再查一次會真的再去問**」——
    ⚠️ 而**同一個行程內**，`GC8` 刻意讓它七天不再問，那是條文要的行為，不是缺陷。
    """
    _enable(monkeypatch)
    calls = []
