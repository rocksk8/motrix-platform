...
from tests.test_e2e_bonus_case_page_2026_09_24 import live_server, _login  # noqa: E402,F401
...
@pytest.mark.e2e
def test_changing_the_rate_updates_amounts_before_saving_and_saving_stores_them(live_server, client, team):
    no = "MQ-LRE-001"
...
@pytest.mark.e2e
def test_a_split_that_is_not_100_blocks_saving(live_server, client, team):
    no = "MQ-LRE-002"
    _draft(client, team, no)
    with sync_playwright() as p:
        browser = p.chromium.launch()
...
@pytest.mark.e2e
def test_replacing_and_removing_people_recalculates(live_server, client, team):
    no = "MQ-LRE-003"
    _draft(client, team, no)
    with sync_playwright() as p:
        browser = p.chromium.launch()
...
@pytest.mark.e2e
def test_editing_in_review_warns_and_voids_signatures(live_server, client, team):
    _set_flow(["lr_sa2"])
    conn = _db()
    try:
