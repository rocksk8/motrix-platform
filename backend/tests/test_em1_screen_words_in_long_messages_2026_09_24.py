...
_UNTOUCHED = (
    ...
    ('modules/supply/api/inventory.py',
     '僅在庫（未出貨/未登載）狀態可直接刪除，其餘狀態請用「退回庫存」調整'),
    ('modules/supply/api/inventory.py',
     '序號不得為空白（每一台實體設備都需要實際序號才能建立庫存）'),
    ('modules/arap/api/invoice_vouchers.py',
     '此帳號沒有檢視財務金額的權限（需要「財務金額可視」模組）'),
    ('modules/arap/api/invoice_vouchers.py',
    ...
    ('modules/arap/api/payment_requests.py',
     '此帳號沒有檢視財務金額的權限（需要「財務金額可視」模組）'),
    ('modules/arap/api/payment_requests.py',
...
CASES = (
    ("modules/case/api/case_action_items.py", None, "需要工程主管確認", ("project_approve_eng",),
     ("案件待辦－工程主管確認",), "pages/users.html"),     # CU2b 用詞裁示：代辦→待辦
    ("modules/case/api/case_action_items.py", None, "需要業務確認", ("project_approve_biz",),
