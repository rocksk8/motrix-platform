...
from tests.test_e2e_bonus_case_page_2026_09_24 import (  # noqa: F401
    live_server, _login, _users, _award, NO, DATA_JS)
...
@pytest.mark.e2e
def test_cashier_picks_the_bank_and_sees_the_voucher(live_server, make_user):
    u = _users(make_user)
    _bank_config()
...
@pytest.mark.e2e
def test_superadmin_voucher_account_settings_refuse_bad_codes(live_server, make_user):
    u = _users(make_user)
    with sync_playwright() as p:
        browser = p.chromium.launch()
        try:
...
@pytest.mark.e2e
def test_return_shows_the_notice_when_the_voucher_was_already_submitted(live_server, make_user):
    u = _users(make_user)
    with sync_playwright() as p:
        browser = p.chromium.launch()
        try:
