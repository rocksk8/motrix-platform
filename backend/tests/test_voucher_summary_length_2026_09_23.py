...
# ⚙️ 那使用者在講的是什麼：B 量到的 `JV14` 正是同一件事的另一半

```
B：長摘要匯出後 pypdf 抽不到中段（JV14）
helpers/voucher_pdf.py 的 CSS：
  td, th { height: 20.1pt; ... word-break: break-all; }
  table  { table-layout: fixed; }
```
🔴 `height` 是**固定值不是 `min-height`**，配合 `word-break: break-all`
（長摘要會換行）與 `table-layout: fixed`——一個長摘要在**列印輸出**上
可能撐不開那一列，而「哪裡被裁掉、裁多少、在螢幕上長什麼樣子」
是**版面高度**問題，這是**人工題**。
...
def test_jv12_a_long_summary_is_not_truncated_in_the_exported_pdf(client,
    ...
    ```
    B 量到的現象  長摘要匯出後 pypdf 抽不到中段（JV14）
    這一題        用同一個方法（pypdf 抽字）驗同一件事，釘成迴歸測試
    ```
    ⚙️ 三個標記分開驗，紅了看得出**缺哪一段**：
    ```
    只缺中段   ⇒ 符合 B 的觀察，像是「中間被蓋掉／裁掉」
    全部都缺   ⇒ 這一列可能整個印不出來，成因不同
    只缺結尾   ⇒ 像是「超出邊界的部分被裁切」
    ```

    ## 🔴 誠實記錄：我這裡**沒有重現** B 的 JV14 現象

    用本檔 `_LONG_SUMMARY`（約 60 字）與另外實測一個 323 字的版本，
    三個標記（起始／中段／結尾）**都完整出現**，`pypdf` 讀到的頁數是 1。
    ⇒ 這一題**今天是綠的**——而我不能因此宣稱「JV14 那個現象不存在」
    或「已經修好了」：**我不知道 B 當時的確切重現條件**（例如是不是
    多行都很長、是不是含附件的匯出、是不是已過帳的凍結快照路徑、
    或者是不是特定的字元／換行位置）。
    ⇒ **這一題先留著當迴歸守門**（它今天綠，值得保持綠），
    而 B 的原始 JV14 現象**還沒有被這一題排除或重現過**，需要跟 B
    要確切的重現步驟（哪一張傳票、多長的摘要、走哪一條匯出路徑），
    不要把「我這個合成案例是綠的」當成「那個問題已解決」的證據。
    """
