...
def test_mg1_the_packaging_gate_reads_the_same_numbers():
    ...
    ```
    ## 🔴 我把這一題的理由寫錯過**兩次**，兩列都留著

    ```
    ❌ v1  「打包擋關那一份實作」
           前提不成立：grep verify_package build_deploy_package.ps1 ⇒ 0 處（B 查），
           A-2 擴大到全 repo 也是 0 ⇒ **沒有任何自動流程呼叫它**，它是人工執行的
    ❌ v2  「正則看不見了 ⇒ 回 0 ⇒ 拿 0 去比 ⇒ **安靜地放行**」
           ☠️ 不會放行。A-2 指出 `verify_package.py:281-282` 有守衛：
             consistent = (f["current"] is not None
                           and f["current"] == f["count"] == f["last_num"])
           我**實跑量過**（改排版讓正則失效，再餵給它）：
             排版改了      current=92   count=0   ⇒ 92 == 0 為 False ⇒ **R.fail**
             連 current 也解析不到 ⇒ None ⇒ `is not None` 直接擋掉 ⇒ **R.fail**
           ⇒ 兩種都**擋住了**。那個守衛正是防這一件事的，而且它有效。
    ```
    ...
    它不守 「打包時會被擋下來」——現在沒有那個流程（VP5 才有）
    ```
    ☠️ 少了它，那支解析器爛掉之後**沒有任何東西會說**（因為沒有人跑它）
    ⇒ 而在 `VP5` 把驗包接進打包流程那一天，**每一次打包都會失敗**，
      錯誤訊息是「`db 版本三源一致` 不一致」——**它指向 `db.py`，而壞的是解析器**。
    🔑 那才是這一題防的東西：**一個指錯方向的紅燈，在一個最不方便的時刻出現。**
    📌 B 的原話留著：「兩者算出同一組數字是好事，**而生效的只有一邊**。」
    ⚠️ 在 `VP5` 之前，**擋住 `_m093` 重號的是本檔這支 pytest，不是那支腳本。**
    """
