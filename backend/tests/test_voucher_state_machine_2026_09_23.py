...
規格來源：`docs/windows/STATE.md §106c`（**在 repo 裡**）。
...
# ⚠️ 定稿規格 `SPEC-VOUCHER-FINAL.md` **不在 repo 裡**（在 A-2 的 scratchpad）

⇒ 本檔只引用 `STATE.md` 裡查得到的那幾段（`§103e`／`§106c`／`§107`），
**不引用我沒有辦法重新取得的東西**。
🔑 〈要求寫在訊息裡等於沒下達〉：`RC1–RC4` 那次是同一個形狀，
   而那次的處置是「**請 A 落進權威檔**」。這次 A 已經把狀態機落進 `STATE.md` ⇒ 可引用。
📌 而 DDL／欄位那一層仍然只在 scratchpad ⇒ **本檔不釘欄位名**，
   ⑤（TRIGGER）那一題要等它進 repo 才寫得準。
...
#: `§106c` 定版的狀態。第六個是「作廢」（已過帳不可退回 ⇒ 只能作廢重開）。
STATUSES = ("草稿", "待審核", "簽核中", "已核准", "已過帳", "作廢")
#: `§103e`：選甲之後只有一個值。
EDITABLE = ("草稿",)
...
def test_voucher_statuses_are_exactly_the_six_from_the_spec():
    """🔴 **狀態剛好是 `§106c` 定版的那六個。**

    ```
    草稿 ／ 待審核 ／ 簽核中 ／ 已核准 ／ 已過帳 ／ 作廢
    ```
    ⚠️ 釘**集合相等**不是「包含」：
    ```
    少一個  ⇒ 有一條路走不到
...
def test_a_posted_voucher_cannot_be_sent_back():
    ...
    assert fn("已核准") is True, (
        "`已核准` 不能退回 ——\n"
        "⚙️ 這是正對照：少了它，「全部都不准退」會讓下面那個斷言綠，\n"
        "   **而退回是這個流程的正常動作**。")
