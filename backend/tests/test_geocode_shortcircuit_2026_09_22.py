...
def test_gc10_fixing_a_typo_in_the_address_retries_immediately(monkeypatch):
    ...
    ## ⚠️ 而它現在是綠的，**而那個綠證明不了任何事**

    負快取還不存在 ⇒ **兩個地址本來就都會被查** ⇒ 斷言自然成立。
    ☠️ 它要等 `GC8` 落地之後才開始有鑑別力 ——
    📌 我把這件事寫在這裡而不是等它自己出問題：
    🔑 **一個「因為功能還沒做所以通過」的測試，與一個真的通過的測試，
    在報告上長得一模一樣。**
    ⇒ B 做完 `GC8` 之後**要回來確認這一題還是綠的** —— 那一次的綠才算數。
    """
    geo = _geo()
    calls = []
    monkeypatch.setattr(geo, "_locate_google",
    ...
    geo.locate_cached("台中市西屯區台灣大道三段301號")
    geo.locate_cached("台中市西屯區台灣大道三段302號")   # 改一個字
    assert len(calls) >= 2, (
        f"改了一個字的地址沒有重新查（只查了 {len(calls)} 次）——\n"
        "⇒ 使用者改好了地址而系統還是不去查，而畫面上沒有東西告訴他為什麼。")
