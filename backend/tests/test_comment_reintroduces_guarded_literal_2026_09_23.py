...
#: 「產品還沒做」型的講法 —— 它在**寫的時候準確**，而產品做完之後過期。
#: ⚠️ 中間要容許**反引號與標點**：我第一版寫 `\s*`，而實際是
#:    「`` `v93` 還沒做 ``」—— 反引號夾在中間 ⇒ **儀器自檢當場抓到它**。
#:    🔑 那正是 `MD1` 同一族：**行內標記把字拆開**，而這次咬的是我自己的 pattern。
_GAP = r"[\s`'\"）)：:，,、]*"
_WILL_EXPIRE = re.compile(
    r"(v\d{1,3}|migration|接縫|[A-Z]{1,3}\d{1,2})" + _GAP
    + r"(還沒做|還沒有|尚未實作|尚未完成|不存在 ——)"
    r"|(還沒做|還沒有)" + _GAP + r"(v\d{1,3}|migration)")

#: ⚙️ 豁免：**描述這個規則本身**，以及**引述一句被改掉的舊訊息**的那幾行。
#: ⚠️ 只放這兩種，不放「我還沒改到」的 —— 後者要改，不是要豁免。
#:
#: 🔴 第二種是必要的，而它的理由與 `QA2` 同一族：
#:    我在 docstring 裡留著「**我的訊息說「v95 還沒有」**」這句話，
#:    是〈更正要留著錯的那一列〉—— **把錯的那一版留下來給下一個人看**。
#: ☠️ 沒有這個豁免的話，這道守門會**逼我刪掉那筆紀錄** ——
#:    而那正是 `QA2` 講的「守門會讓人刪掉解釋」，這次對象是我自己。
_QA3_ALLOW = ("_WILL_EXPIRE", "§138", "QA3",
              "我的訊息說", "我原本寫", "第一版", "舊訊息")


def test_qa3_no_assertion_message_will_expire_when_the_product_is_done():
    """🔴 `QA3` **訊息不可以只描述「現在」** —— 它要涵蓋兩個時期。

    ```
    寫的時候（準確）  「`account_items` 表不存在 —— **v93 還沒做**」
    產品做完之後      那題變綠 ⇒ 沒有人會想起來改它
    日後它再紅        訊息仍然說「v93 還沒做」
                      ⇒ **排查的人會去看 migration，而問題在別的地方**
    ```
    ✅ 改成一句涵蓋兩個時期、**不必在產品做完時被想起來改**：
    > 「我在 `sqlite_master` 裡沒有找到 X。
    >  ⚠️ 它**應該**存在 ⇒ 看 `v93`；`v93` 已完成 ⇒ **那它是被刪掉或改名了**。」

    📌 而「現在就該綠」型的題**沒有這個問題** —— 它的訊息本來就寫
       「它紅表示**我的前提錯了**」，那句話不會過期。
    🔑 差別不在題的種類，在**寫作時機**：一句只描述當下狀態的話，
       **在狀態改變的那一刻就開始說謊，而沒有任何東西會紅。**
    """
    bad = []
    me = Path(__file__).name
    for p in sorted((_BACKEND / "tests").glob("test_*.py")):
        # ⚠️ **本檔自己豁免**：它是**描述**這個形狀的那一份（pattern、誘餌、
        #    docstring 裡的反例）。一支掃描器掃不了自己的規格。
        # ⇒ 而它的正確性由下面那題（儀器自檢）守，不是靠這個豁免。
        if p.name == me:
            continue
        src = p.read_text(encoding="utf-8", errors="replace")
        for i, line in enumerate(src.splitlines(), 1):
            if not _WILL_EXPIRE.search(line):
                continue
            if any(tok in line for tok in _QA3_ALLOW):
                continue
            if line.lstrip().startswith("#"):
                continue          # 註解裡講它是在解釋，不是在斷言
            bad.append("%s:%d\n      %s" % (p.name, i, line.strip()[:72]))
    assert not bad, (
        "這幾行的訊息**會在產品做完之後過期**：\n  " + "\n  ".join(bad)
        + "\n☠️ 它們現在準確，而那題一綠就沒有人會想起來改；\n"
          "   日後它再紅，訊息會把排查的人**送到錯的地方**。\n"
        + "✅ 改成涵蓋兩個時期：「它**應該**存在 ⇒ 看 `vNN`；"
          "`vNN` 已完成 ⇒ **那它是被刪掉或改名了**。」")


def test_qa3_the_expiry_scanner_can_see_both_shapes():
    """⚙️ **儀器自檢**：兩種寫法都要認得，而改好的寫法不可以被抓到。

    ☠️ 少了它，上一題的綠可能只是**掃描器什麼都看不到**。
    """
    for sample in ("`v93` 還沒做", "migration 還沒有", "`v95` 還沒有：缺 x",
                   "接縫尚未實作"):
        assert _WILL_EXPIRE.search(sample), (
            "掃描器看不到 %r —— **儀器壞了**。" % sample)

    fixed = ("它**應該**存在 ⇒ 看 `v95`；`v95` 已完成 ⇒ "
             "**那它是被刪掉或改名了**。")
    assert not _WILL_EXPIRE.search(fixed), (
        "改好的寫法被判成會過期：%r ——\n" % fixed
        + "☠️ 那個方向會逼人把正確的訊息改回去。")
