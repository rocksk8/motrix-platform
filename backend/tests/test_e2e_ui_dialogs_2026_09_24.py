...
@pytest.mark.e2e
def test_confirm_keyboard_and_clicks(live_server, e2e_browser):
...
@pytest.mark.e2e
def test_danger_confirm_starts_on_cancel_so_a_stray_enter_does_not_do_it(live_server, e2e_browser):
    p = e2e_browser   # PERF #5：共用瀏覽器
    browser, page = _page(p, live_server)
    try:
...
@pytest.mark.e2e
def test_prompt_values_cancel_and_required(live_server, e2e_browser):
    p = e2e_browser   # PERF #5：共用瀏覽器
    browser, page = _page(p, live_server)
    try:
...
@pytest.mark.e2e
def test_focus_is_trapped_and_returns_to_the_opener(live_server, e2e_browser):
    p = e2e_browser   # PERF #5：共用瀏覽器
    browser, page = _page(p, live_server)
    try:
        page.focus("#opener")
...
@pytest.mark.e2e
def test_dialogs_queue_one_at_a_time_and_helper_answers_them(live_server, e2e_browser):
    p = e2e_browser   # PERF #5：共用瀏覽器
    browser, page = _page(p, live_server)
    try:
...
@pytest.mark.e2e
def test_toast_and_banner(live_server, e2e_browser):
    p = e2e_browser   # PERF #5：共用瀏覽器
    browser, page = _page(p, live_server)
    try:
...
@pytest.mark.e2e
def test_dark_mode_reads_the_dark_tokens_and_is_not_inverted_twice(live_server, e2e_browser):
