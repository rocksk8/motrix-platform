...
#    那不只是「指錯方向」——**它在同一行裡同時給了正確與錯誤的資訊，而錯的那半在前面。**
# ☠️ 而它騙過的人包括 A：那句話被當成**選單工具的 UI 故障**，
#    還回覆使用者「那不是答案」——**手上有完整脈絡的讀者都被它帶走了。**
#
# ✅ **而正確的慣例就在同一支檔裡，12 個裡有 8 個已經做對**：
# ```
# 對  匯出失敗：／標記失敗：／儲存失敗：／T100 預覽失敗：／確認已匯入失敗：
#     ⇒ 講**哪一個操作**失敗（那是寫的人知道的）
# 錯  網路錯誤：
#     ⇒ 講**為什麼**失敗（那是 catch 不知道的）
# ```
# 🔑 ⇒ `FE3` 不是引入新慣例，是**讓 4 個異類跟上同檔已有的 8 個**。
# 📌 A 的裁示：**只改措辭，不要順手加分類邏輯**——那是另一件事。

#: `catch` 不可以宣稱的成因。
#: ⚠️ 刻意**小**：只列「這個 `catch` 不可能知道」的那幾種。
#:    列太多會誤傷真的在處理網路的地方（例如一支專門重試逾時的函式）。
_CAUSE_WORDS = ("網路", "連線", "逾時", "斷線")


def test_fe3_error_messages_do_not_claim_a_cause_they_cannot_know():
    """🔴 `FE3`：**`cashier.js` 的錯誤訊息不可以宣稱成因。**
    ...
    ⇒ 它不可以說「**為什麼**失敗」（不知道）
    ```
    ⚠️ 判準**剝掉註解之後**再比對 —— `cashier.js:567` 有一段解釋這次事故的註解，
    裡面寫著「網路錯誤」四個字。
    🔑 **那段說明是這次事故最好的紀錄，它應該留著** ——
       而守門剝註解之後，**寫註解的人就不必再自我審查**。
    📌 那正是 B 今天那條（「解釋『為什麼沒有 X』時不要寫出 X」）的**反例**：
       那條是**在守門很粗糙時**才需要的自我約束。

    ⚙️ 正對照在下一題：**不可以靠「把整句刪掉」變綠。**
    """
    where, raw = _cashier_js()
    src = _blank_js_comments_local(raw)
    bad = []
    for i, line in enumerate(src.splitlines(), 1):
        ...
        for w in _CAUSE_WORDS:
            if w in line:
                bad.append((i, w, line.strip()[:72]))
                break
    assert not bad, (
        "`%s` 的訊息裡宣稱了成因：\n  " % where
        + "\n  ".join(":%d [%s] %s" % b for b in bad)
        + "\n☠️ 使用者看到「網路錯誤：this._displayName is not a function」——\n"
          "   **前半句是假的、後半句是真的，而人會先讀前半句。**\n"
        "✅ 同一支檔裡已經有 8 處做對了（`匯出失敗：`／`標記失敗：`／`儲存失敗：`⋯）\n"
        "🔑 那些講的是**哪一個操作**失敗（寫的人知道），不是**為什麼**（不知道）。")


def test_fe3_the_underlying_message_is_still_shown():
    """⚙️ **正對照：不可以靠「把整句刪掉」讓上一題變綠。**

    ☠️ 最省力的變綠方式是把 `alert(...)` 整個拿掉 ⇒ 上一題全綠，
    🔑 **而使用者從此連「發生了什麼」都看不到** —— 那比一個指錯方向的訊息更糟：
    ```
    指錯方向  他查錯地方，而他**知道出事了**
    什麼都沒  他以為存好了
    ```
    📌 而這次的實情正是「他以為存好了」的相鄰狀態：按了確定、什麼都沒存，
       **而畫面說是網路問題** ⇒ 他會重試，而重試也不會成功。

    ⚙️ 基準是**量出來的**（`e.message` 目前 12 處），不是猜的。
    """
    _where, raw = _cashier_js()
    src = _blank_js_comments_local(raw)
    shown = src.count("e.message")
    assert shown >= 12, (
        "`cashier.js` 裡把底層訊息顯示出來的地方只剩 %d 處（基準 12）——\n" % shown
        + "☠️ 「把整句刪掉」會讓上一題變綠，**而使用者從此連發生了什麼都看不到**。\n"
          "🔑 那比一個指錯方向的訊息更糟：指錯方向他至少**知道出事了**。")
