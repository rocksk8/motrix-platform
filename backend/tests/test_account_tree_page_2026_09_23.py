...
# 🔴 而 ① 那個「不可以靠前綴」的規模，我量了，比 A 寫的大

```
A 寫的   「1268 會被掛錯」（一個例子）
我量到   **232 筆**的 parent_code 不是自己的前綴（共 547 筆）
```
⚠️ 而我先前量過的「範圍代號共 13 個」是**另一件事**（有幾個父節點長成範圍），
   兩個數字都對而**答的不是同一個問題**：
```
13   有幾個**父**是範圍代號
232  有幾個**子**會被前綴法掛錯／掛不上
```
📌 ⇒ 前綴法不是「有幾筆例外」，是**超過四成的節點會出問題**。
"""
import importlib
import json
import re
