...
## §6 · 2026-09-14 續：取消 admin 直通

2026-09-13 那輪把後端檢查補齊了（35 個 key 裡 16 個原本後端完全不讀 → 33 個有讀），
但 `require_any_module()` 本身仍然讓 **admin 直通**——所以對四個真實 admin 帳號來說，
那 94 處新增的檢查一處都沒有生效過。使用者 2026-09-14 裁示把這層拿掉。

| 項目 | 2026-09-13 之後 | 2026-09-14 之後 |
|---|---|---|
| `require_any_module` | `role in (superadmin, admin)` 直通 | **只有 superadmin** 直通 |
| 側欄旗標 | `mods.indexOf(x) >= 0 \|\| ad` ×20 餘處 | 單一 `has(x)`，只有 `sa` 例外 |
| 其他非模組放行 | `\|\| eng`、`\|\| role !== 'viewer'` | 已移除（v84 回填對應模組） |
| 可授權模組 | 35 個 | **40 個**（補上 5 個原本只能靠角色寫死的） |
| 分組名稱 | — | `renderMainNav()` 本來就過濾空分組，自動達成 |

**關鍵在順序**：先用 DB v84 把「直通實際給出去的東西」寫成真的模組，再取消直通。
不這樣做的話，四個 admin 會一次少掉 13～17 個 key 的功能——而那些缺漏**本來就
存在**，只是被直通遮住了。

§4「仍未收的」裡的三列（16 個模組後端不讀／`financial_view` 是顯示偏好／
`/api/sales-orders` 任何登入者可讀）到這裡實質收斂完畢，剩下的只有「金額欄位
要不要也納入模組制」這個未決問題。
