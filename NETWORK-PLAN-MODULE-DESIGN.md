...
> 狀態：**開發完成，第一輪已部署正式機但發現第二個匯出 bug，修復後尚未重新部署**（§10 步驟 1～10、12 已完成＋額外新增 Excel 匯入；步驟 13 部署已跑過一輪 §15，但下方 §2026-08-26b 的修復還沒套用）｜DB migration：**v64 已套用**｜每次要繼續調整這個模組前先讀本文件 + `MOTRIX-ERP-QUICK.md`

## 2026-08-26b 追加修復：Excel 分頁名稱不能有全形斜線

§2026-08-26 第 1 項修復（空字串→None）套用到正式機後，使用者在正式機上對真實案件（小林機械，MQ-202607-045）建立規劃書並匯出，**Excel 仍然跳出修復對話框**，且錯誤訊息明確指出「第二個分頁」被吞掉重組成「復原_工作表1」。用 zipfile／XML well-formedness／CRC 檢查這份實際壞掉的檔案，結果**完全找不到問題**——包含之前修好的空字串 bug 在內都沒有復發。

**靠使用者提供 Excel 實際跳出的 `recoveryLog` 錯誤訊息全文才查到真正原因**：分頁名稱含全形斜線「／」（U+FF0F）會讓 Excel 判定 `workbook.xml` 損毀——這是 Excel 自己額外、沒寫進公開 OOXML schema 的分頁名稱驗證規則（比公認的「不能有半形 `\ / ? * [ ] :`」更嚴格，連這 7 個字元的**全形版本**也不行），openpyxl／任何通用 XML 工具都不會擋下或抓到這個問題。受影響的兩個分頁名稱「WAN／對外線路」「IP／Port 群組」已改成「WAN 對外線路」「IP-Port 群組」（`network_plan_export.py::SECTIONS`，前端 `network-plan-form.html::DATA_TABS` 對應標籤同步改），並新增靜態迴歸測試 `test_excel_sheet_names_avoid_fullwidth_forbidden_chars` 掃描分頁標題防止再犯。詳見 [[feedback_excel_sheet_name_fullwidth_slash]] memory。

**⚠️ 這代表第一輪 §15 部署（`bb53dcc`）雖然修好了空字串問題，但這個分頁名稱問題仍然存在，正式機上目前的版本匯出的 Excel 還是會壞——需要再跑一輪 §15 部署套用這次的修復。**
...
**驗證：** 上述 4 項都已用真實瀏覽器（本機 dev server，`nptest_temp2` 臨時帳號）跑過端到端流程——修復後的匯出檔案直接用 `zipfile`+正則掃描確認沒有壞掉的 `inlineStr` 儲存格；卡片版面截圖確認同一筆資料完全不需要水平捲動；批次套用勾選兩筆設備、選「管理 VLAN」、填 45、套用後兩筆同步更新且存檔後重新載入仍正確；Excel 匯入用 `file_upload` 工具（不是點擊 input，會跳出無法互動的原生檔案選取視窗）把一份匯出檔匯入回同一份規劃書，成功訊息與修訂紀錄皆正確，且各分頁資料跟原始匯出內容一致。測試資料與臨時帳號已清除。後端新增 4 題 pytest（匯出/匯入 round-trip、上傳非法檔案 400、無權限 403、分頁名稱字元靜態掃描），全套 169/169 全過。
