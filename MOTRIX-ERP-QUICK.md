...
| 📋 | **待開發（2026-09-08 使用者提出，尚未設計/實作）：QR 登入手機端免密碼**——目前手機掃碼流程（`login-qr-approve.html`，見 §12 2026-09-08「更晚」條目）掃到 QR 後仍要在手機上手動輸入一次登入密碼才算核准；使用者希望改成手機瀏覽器本身完成驗證（例如手機該瀏覽器已登入過、或用其他免密方式核實身分）後就直接自動完成核准，不用再打一次密碼。**尚未評估技術方案與安全性影響**（例如：手機瀏覽器要怎麼「已驗證」——沿用手機自己的 session？生物辨識？跟現有 5 次失敗鎖定機制如何共存？），下次要動手前應先進 Plan Mode 完整設計，比照 QR 核准功能當初的作法（見 [[feedback_check_existing_before_building]]，認證流程設計優先權高於一般功能）。 |
