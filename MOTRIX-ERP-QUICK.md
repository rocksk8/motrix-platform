...
| 📋 | **待開發（2026-09-08 討論，尚未實作）：`build_deploy_package.ps1` 打包關卡用 `pytest-xdist` 平行化**——目前非 e2e 測試（464 題）序列跑約 5.5~6 分鐘，這台開發機有 6 實體核心/12 邏輯執行緒，改用 `pytest -n auto`（動態分派給多個 worker process，不是手動均分成固定幾堆）估計可壓到 1.5~2 分鐘。**動手前務必先查**：`conftest.py` 現有的測試隔離機制（每題測試的 DB/temp 檔案是否真的各自獨立）能不能撐住多個 process 同時跑，不要假設沒問題；驗證方式是跑一次序列、一次 `-n auto`，比對兩邊通過/失敗清單完全一致再信任平行結果——不需要每次都「平行完再序列驗證一次」，那樣會抵銷平行化的意義。改動範圍預期很小：`requirements-dev.txt` 加 `pytest-xdist`，`build_deploy_package.ps1` 的 pytest 呼叫加 `-n auto`。 |
