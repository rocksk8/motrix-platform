...
| GET | /user-activity/trail?user&start&end&limit&before_id | **限 superadmin**：逐條操作軌跡（DB v80 `user_request_log`）。輪詢端點不記、30 秒內同端點收斂、保留 90 天 |
...
| 優先 | 項目 |
|------|------|
| 🔴 | **模組權限要真的擋住、未開啟的連模組名稱都不顯示（2026-09-14 交辦，尚未施作）**。使用者裁示：「管理者一樣依據有開權限的內容去顯示，沒開的就不顯示，包含模組名稱。超級管理者預設全開，使用者部分看模組內容去檢核，未開啟的直接不顯示」。**這一筆同時收斂本節既有的三列**：下面「16 個模組後端完全不讀」「`financial_view` 是顯示偏好不是權限」「`GET /api/sales-orders` 任何登入者可讀」講的都是同一件事的不同切面——現況介於「勾掉＝擋掉」與「勾掉＝只是介面偏好」之間，最容易讓人誤以為已經擋住了。**動工前要先決定的**：①未開啟的模組是「側欄不顯示」還是「連 API 都回 403」（使用者的敘述是前者為底線、但『沒開的就不顯示』若只做前端等於沒有安全性）②admin 跟 superadmin 的差別到底是什麼（若 admin 也受模組限制，那 superadmin 預設全開就是唯一的全視角角色）③金額欄位（毛利、淨利）要不要一起納入這套，還是維持另一條線。**先讀 [`MODULE-AUDIT-2026-09-13.md`](MODULE-AUDIT-2026-09-13.md) 再開工**，那份已經把 16 個模組逐一盤過。 |
...
| 不記什麼 | 輪詢端點（通知／紅點／在線名單／ping／附件載入）；**請求內容一律不記**（body/query 值）——那等於在資料庫裡多存一份業務資料副本，外洩風險與價值不成比例 |
| 收斂 | 同一支端點 30 秒內連續請求只記一次（自動存檔 1.5 秒防抖會打很多次） |
