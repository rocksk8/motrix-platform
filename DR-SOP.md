...
| 需要還原到 **60 天以前**的時間點 | 該月的月備份（粒度：**一個月一份**，取當月第一次成功備份的內容） | 未演練 |
...
執行前提：手上至少要有其中一份資料庫來源——優先序照 QUICK §8.3：
**舊硬碟還能讀的本機 `db_backups`** → **雲端 `每日備份/YYYY-MM-DD/motrix_erp.db`**（保留 60 天）→ **雲端 `月備份/YYYY-MM/motrix_erp.db`**（2026-09-14 新增，**永久保留**）→ **JSON 重建（最後手段）**。

> **⚠️ 2026-09-14 保留政策改版**：雲端每日備份只留 **60 天**、週備份 **90 天**。
> 要還原三個月以前的時間點，**只剩 `月備份/YYYY-MM/`** 那一層——它永久保留，
> 而且含整份 `.db`（不是只有 JSON）。做 DR 時先確認手上那份來源的日期落在哪一層。

> **⚠️ 新機器第一次啟動前先處理存檔所有權**（2026-09-14 新增，QUICK §8.1b）：
> 雲端存檔根目錄有 `.motrix_archive_owner` 標記，內容是**舊機器資料庫**的識別碼。
> 如果是「用舊資料庫還原到新機器」，識別碼跟著資料庫走，新機器會直接通過、不需要處理。
> 但如果是**從 JSON 重建出一個新資料庫**（識別碼會重新產生），新機器會被自己的防呆擋下來、
> 雲端備份全部停寫並寄信。這時**刪掉 `.motrix_archive_owner` 再重啟**即可重新認領。
> 本機 SQLite 快照在任何情況下都不受這個標記影響。
...
| **寫一支還原演練腳本** | 2026-09-14 提出、**尚未施作**。目前 §6 演練紀錄整張空白、整機故障 RTO 寫「未知」，而且 `tools/` 裡只有 `rollback_update.ps1`（程式碼回滾）、**沒有任何還原工具**。第三層（JSON 重建）現在涵蓋 41 張表、又刻意剝掉憑證與內嵌影像，那條路徑的正確性**一次都沒被執行過**。建議做法：`restore_drill.ps1` 吃一個備份來源 → 還原到 temp 目錄 → 跑 `init_db()` → 起一個 port 667 的實例 → 打幾支 API 核對筆數 → 丟掉。每季跑一次，結果寫進 §6。**沒驗證過的備份等於沒有備份**，這件事比再加一層備份有價值得多 |
| **S3 後端從未對真實帳號驗證過** | QUICK §8.0 自己註明「目前沒有真實 S3/B2 帳號可測試」，只用假的記憶體 client 做過單元測試。跟上一列同一類：看起來有退路，實際沒驗證過 |
