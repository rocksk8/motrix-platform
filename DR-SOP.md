...
## 3a · 新版（platform）個資資料夾（2026-09-25 使用者裁示；V9 原版不適用）

> 🔴 **新版起，整庫 `.db` 與 F2 個資只放在獨立的個資資料夾**，一般的「系統存檔」只留去掉個資的 JSON 與一般單據鏡像。

| 位置（雲端硬碟「我的雲端硬碟」底下） | 內容 |
|---|---|
| `系統存檔\每日備份\YYYY-MM-DD\*.json` | 一般每日 JSON：**不含**承攬人員的個資欄位（影像、身分證字號、聯絡方式、帳戶）、勞報單 `data_json` 的身分證字號／地址／電話／Email／帳戶、承攬付款憑據 `snapshot_json.personnel[]` 的外包人員帳戶（稽核 X-9b M-3）、協力廠商（承攬商本身）的帳戶：`協力廠商.json` 的 `data_json` 與承攬付款憑據 `snapshot_json` 最上層的戶名／帳號／存摺影像（稽核 X-9b O-9，使用者裁示屬個資） |
| `系統存檔_個資\每日備份\YYYY-MM-DD\motrix_erp.db` | 當日整庫（保留天數同一般每日層） |
| `系統存檔_個資\每日備份\YYYY-MM-DD\承攬人員.json`、`薪資單.json`、`承攬付款憑據.json`、`協力廠商.json` | 這四張表的**完整列**（含 F2） |
| `系統存檔_個資\月備份\YYYY-MM\motrix_erp.db` | 當月整庫（永久保留，同一般月備份） |
| `系統存檔_個資\勞報單存檔\` | 勞報單 PDF 鏡像（只增不減） |

**建立與權限（只做一次，由人做；程式永不自動建立根目錄）**

1. 在伺服器的檔案總管，於「我的雲端硬碟」底下建立 `系統存檔_個資`（與 `系統存檔` 並列）。
2. 在資料夾上按右鍵 → **Google 雲端硬碟 → 共用**，確認或設定權限（只留需要的人）。不用登入網頁。
3. ⚠️ **不可以建在已分享的資料夾裡**——子資料夾會繼承上層分享，權限反而更寬。
4. ⚠️ **Windows 的「安全性」權限管不到雲端**——雲端的存取只由 Google 雲端硬碟的共用設定決定。
5. 底下的 `每日備份／月備份／勞報單存檔` 由程式在根目錄存在時自動建立（繼承根目錄的窄權限）。只會逐層往下建：根目錄在寫入途中消失 ⇒ 那一輪寫入失敗並告警「個資資料夾在寫入途中消失」，**不會被建回來**（稽核 X-9b S-5）。

**個資資料夾未建立時長什麼樣**

- 設定頁「雲端備份目標」區塊：「個資存檔（勞報單）雲端資料夾：**未上雲**」，並列出預期路徑。
- 每次狀態改變寄一封 ERROR 告警「個資資料夾未建立：…勞報單只有本機一份。整庫備份與個資欄位也不在雲端」（同原因每日最多一封）；月備份不會標記完成，並每日告警一次，直到資料夾建好。
- 🔴 **這時雲端沒有任何整庫備份**；還原來源只剩本機 `backend\db_backups\`。處置：照上面 1–4 建立資料夾，下一次每日備份起自動補上。

**從 JSON 重建時（最後手段）合回個資欄位**

一般份的 `承攬人員.json`／`薪資單.json`／`承攬付款憑據.json`／`協力廠商.json` 缺 F2 欄位（清單以 `archive._F2_FIELDS` 為準）。用同一天的 `系統存檔_個資\每日備份\{日期}\` 同名檔合回：
`archive.merge_general_and_pii(檔名, 一般份["data"], 個資份["data"])` → `(合回的列, 個資份缺的 id)`。
依 `id` 對齊，F2 欄位取個資份的原值；個資份缺的列保留一般份並列出 id，不補空值。
（守門：`tests/test_pii_archive_mirror_2026_09_25.py`——一般＋個資合回後逐欄等於原表。）
...
> **新版（platform）**：第二、三層的整庫改在 `系統存檔_個資\每日備份\…` 與 `系統存檔_個資\月備份\…`（§3a）；一般 `系統存檔` 底下不再有 `.db`。JSON 重建時個資欄位要從個資資料夾合回（§3a）。
