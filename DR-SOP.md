...
|---|---|
| `系統存檔\每日備份\YYYY-MM-DD\*.json` | 一般每日 JSON：**不含**承攬人員的個資欄位（影像、身分證字號、聯絡方式、帳戶）、勞報單 `data_json` 的身分證字號／地址／電話／Email／帳戶、承攬付款憑據 `snapshot_json.personnel[]` 的外包人員帳戶（稽核 X-9b M-3） |
| `系統存檔_個資\每日備份\YYYY-MM-DD\motrix_erp.db` | 當日整庫（保留天數同一般每日層） |
| `系統存檔_個資\每日備份\YYYY-MM-DD\承攬人員.json`、`薪資單.json`、`承攬付款憑據.json` | 這三張表的**完整列**（含 F2） |
...
一般份的 `承攬人員.json`／`薪資單.json`／`承攬付款憑據.json` 缺 F2 欄位。用同一天的 `系統存檔_個資\每日備份\{日期}\` 同名檔合回：
