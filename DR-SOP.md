...
|---|---|
| `系統存檔\每日備份\YYYY-MM-DD\*.json` | 一般每日 JSON：**不含**承攬人員三個影像欄、勞報單 `data_json` 的身分證字號／地址／電話／Email |
| `系統存檔_個資\每日備份\YYYY-MM-DD\motrix_erp.db` | 當日整庫（保留天數同一般每日層） |
| `系統存檔_個資\每日備份\YYYY-MM-DD\承攬人員.json`、`薪資單.json` | 這兩張表的**完整列**（含 F2） |
...
5. 底下的 `每日備份／月備份／勞報單存檔` 由程式在根目錄存在時自動建立（繼承根目錄的窄權限）。
...
一般份的 `承攬人員.json`／`薪資單.json` 缺 F2 欄位。用同一天的 `系統存檔_個資\每日備份\{日期}\` 同名檔合回：
