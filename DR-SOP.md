...
## 3 · 已知阻塞點（寫這份 SOP 時發現，必須先解決）

在能真正照著 §4 走之前，以下兩個缺口會讓復原流程卡住，**應優先處理**：
...
1. 🔴 **`setup_autostart_task.ps1` / `setup_heartbeat_task.ps1` 只存在於正式機，開發機的 git repo 沒有這兩個檔案**（QUICK §0 已知落差第 2 筆，記錄於 2026-08-01，至今仍是「⏳ 待處理」）。這代表如果正式機的硬碟連同這兩支腳本一起報銷，**沒有任何地方留著能重新註冊這兩個排程工作的腳本**——重建時只能憑記憶重寫。
   **行動**：下次能接觸正式機時，把這兩個檔案複製回開發機並 commit 進 git（跟 `setup_backup_task.ps1` 放在一起），比照 §14 跨機核對流程。
2. 🟡 **`uploads/`（專案照片）、PDF 存檔（報價單/出貨單/勞報單）目前沒有看到離線／雲端備份機制**——QUICK §8 的備份範圍是 `motrix_erp.db` + 報價單/客戶/供應商 JSON，不含 `uploads/` 底下的實體檔案。如果整機硬碟報銷，這些檔案會直接遺失，資料庫裡的路徑欄位會變成指向不存在檔案的死連結。
   **行動**：評估是否要把 `uploads/` 也納入既有的每日/週雲端備份範圍（`archive.py` 的 `_daily_backup()`/`_weekly_backup()`），或至少先確認目前遺失的實際影響範圍多大（跑一次 `uploads/` 目錄大小統計）。
...
2. 還原 `uploads/`（見 §3 第 2 點的已知缺口——目前可能沒有離線副本可還原，這一步可能直接卡住）
3. 還原 PDF 存檔目錄（`backend/_demo_pdf_archive` 等不需要，正式的 PDF 輸出目錄看 `pdf_gen.py`/`system_settings["pdf_base_path"]`）
...
| 項目 | 說明 |
|------|------|
| 補回 `setup_autostart_task.ps1`/`setup_heartbeat_task.ps1` | 見 §3 第 1 點，優先度最高 |
| `uploads/` 納入備份範圍 | 見 §3 第 2 點 |
