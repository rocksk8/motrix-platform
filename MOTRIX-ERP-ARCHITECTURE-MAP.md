...
superadmin 原本是「密碼 + Bearer token in localStorage」單一因子。已用 `pyotp`＋`qrcode`（皆不需外部服務）新增 TOTP 兩步驟驗證，DB v72（`users.totp_secret`/`totp_enabled`/`totp_recovery_codes`）。**刻意做成自助啟用而非強制**：正式機 superadmin 是 jeff/corbin 兩位真人業主，強制下次登入即進入設定流程，部署當下他們沒先裝好驗證 App 會直接被鎖在外面，是會中斷真實業務的風險——這個取捨經與使用者確認後定案。任何角色皆可在「修改密碼」頁自助開啟，admin/superadmin 未啟用時有提醒 banner（不阻擋操作）。細節見 `MOTRIX-ERP-QUICK.md` §3.3b、`db.py::_m072_totp()`、`routers/auth.py` 的 `totp_*`/`auth_login_totp()` docstring。**✅ 2026-09-11 補上救援碼這一塊**：`POST /api/auth/totp/recovery-codes/regenerate`（需目前密碼，整組換掉、舊碼全數失效），並在 `totp/status` 與「用救援碼登入」的回應裡回傳剩餘組數。**真正的缺口其實比「重產端點」更前面一步**：救援碼用一組少一組，但先前沒有任何地方看得到還剩幾組，使用者只會在最後一組也用完、驗證 App 又不在手邊時才發現被鎖在外面——而那時已經來不及自助補救（重產端點要先登入）。先前唯一的補救方式是停用 TOTP 再重跑一輪 setup/enable，那會連帶換掉密鑰、要重掃一次 QR code。「修改密碼」頁現在會顯示剩餘組數，≤ 4 轉黃、≤ 2 跳紅字。**尚未做**：超過某個角色風險等級才觸發的強制提醒升級。

### 6.3【中】Session 安全：Bearer-in-localStorage → 考慮 httpOnly Cookie
目前 XSS 防護靠「全站禁止 innerHTML 插入動態內容」的紀律（DOM API only），這個紀律本身做得不錯，但屬於「靠自律」而非架構性防護。專業 Web 應用（銀行/SaaS 常見模式）用 httpOnly + Secure + SameSite cookie 存 session，即使真的出現 XSS 漏洞也偷不到 token。**這是架構級改動，不建議現在動**（牽動全站 API 呼叫方式）。**2026-09-11 釐清觸發條件**：原本寫「要做外網暴露時優先序會提升」，但使用者規劃中的是 **VPN**（見 `MOTRIX-ERP-QUICK.md` §3.3c 排除 Cloudflare Tunnel 的理由）——走 VPN 的話瀏覽器面對的仍是內網來源，token 被竊的風險輪廓沒有改變，**這個條件不會因為導入 VPN 而觸發**。真正的觸發條件是「ERP 直接暴露在公開網際網路上」。維持不動。
...
### 6.5【中】專案時程視覺化：目前是清單/看板，缺真正的甘特圖　**✅ 本文件寫成時其實已經做了，記載過期**
**2026-09-07 複查更正**：這條建議寫的當下（本文件標頭日期 2026-09-01）就已經過時——`frappe-gantt` 其實早在 2026-08-24（commit `9f86d93`「案件管理／案件執行看板視覺化改版」）就已經整合進 `case-management.html`／`case-stage-board.html`，比本文件成文還早一週，屬於盤點時沒交叉核對到的既有功能，不是本文件寫完之後才補的。依賴關係箭頭／甘特圖渲染皆已上線，非未來待辦事項。**Why**：這是繼 §6.2（Phase 3b 已完成卻記載進行中）之後第二次「文件盤點當下沒有先查程式碼」的案例，之後產出建議清單前，先對每一項做一次快速 `grep`/`git log -S` 核對是否已存在，而不是只憑既有 QUICK.md 記載的舊印象。

### 6.6【中】庫存管理：已有安全庫存燈號，缺自動採購建議　**✅ 2026-09-07 已實作**
`parts.safety_stock`（v66）已經做到「低於安全庫存」篩選。**複查更正**：原始建議寫「資料已齊備」其實不完全正確——系統從未追蹤供應商前置時間（`suppliers`/`parts` 都沒有對應欄位），所以實作時刻意不做 ETA 預估，只算「該補多少、上次跟誰買、大概多少錢」：新端點 `GET /api/inventory/purchase-suggestions`（見 `MOTRIX-ERP-QUICK.md` §7.19），建議量補到黃燈門檻（安全庫存 × 1.5），供應商/單價取自該料號最近一筆 `stock_batches` 進貨紀錄，查無紀錄退回 `parts.cost`。`inventory.html` 新增「採購建議」按鈕＋唯讀 Modal，v1 刻意不含下單/已處理狀態追蹤。
