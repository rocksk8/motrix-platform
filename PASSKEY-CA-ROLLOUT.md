...
> | 2 部署最新程式碼 | ⬜ **未執行** | 用 `deploy_packages\` 底下 **commit 為 `11ca7ea` 的那一包**（資料夾名形如 `20260910_<時間>_11ca7ea`，`deploy_manifest.json` 裡的 `commit_short` 才是權威）。**刻意不寫死資料夾名**——名字含建置時間戳，而這份文件自己也在部署包裡，寫死就會變成「包內文件指向上一個包」的先有雞先有蛋。該包涵蓋正式機 `2471747` 之後的**全部** commit（營運報表期別與季範圍、全系統稽核三項、稽核後續、報價單單號競態、`???` 假單號與後端單號守門、flaky e2e 根因、慢請求記錄），563 非 e2e + 8 e2e 全過、System32 tar、exit 0。**比它舊的包一律不要再用。** 正式機仍是 `2471747` |
...
> | 9 端對端實測 | ⬜ 未執行 | |
>
> **下一步就是第 2 步**：把上表指定的那一包（commit `11ca7ea`）用部署儀表板套用上去。那次重啟會讓
