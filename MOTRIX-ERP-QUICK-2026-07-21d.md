...
> 文件版本：**2026-07-21d**（財務報表「圖表分析」Tab：Chart.js 4 新增 5 種互動圖表）
...
> **CDN 依賴**：
> - `quotation-form.html`：SortableJS v1.15.3（品項拖曳排序），非 defer，置於 Alpine CDN 之前
> - `reports.html`：Chart.js 4（`https://cdn.jsdelivr.net/npm/chart.js@4`），非 defer，置於 `reports.js` 之前；初始化時設定全域字型（Inter/Noto Sans TC）。
...
| `pages/reports.html` | **`js/reports.js`**（✅ 唯一外部載入之二）+ **Chart.js 4 CDN** | GET /api/reports/financial（JSON）, /excel, /pdf；Tab：本期收款/未收款/案件清單/業務員績效/毛利分析/保固預警/目標達成率/圖表分析 |
