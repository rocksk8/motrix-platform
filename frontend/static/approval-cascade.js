 ...
 *   - completesOnFirst（案件額外支出：當層任一人簽即過層）：自己是該層簽核人之一即可。
 *
 * 各頁面用法：
...
(function () {
  ...
  function selfCascadeTiers(tiers, currentTier, me, delegatedFor, opts) {
    const list = tiers || [];
    const completesOnFirst = !!(opts && opts.completesOnFirst);
    ...
    if (!completesOnFirst && !curPending.every(a => matches(a, me, delegatedFor))) return [];
    if (completesOnFirst && !curPending.some(a => matches(a, me, delegatedFor))) return [];

    const out = [];
    for (let i = cur + 1; i < list.length; i++) {
      const pending = ((list[i] || {}).approvers || []).filter(a => a.status !== 'approved');
      if (!pending.length) break;
      const ok = completesOnFirst
        ? pending.some(a => matches(a, me, delegatedFor))
        : pending.every(a => matches(a, me, delegatedFor));
      if (!ok) break;
      out.push(i + 1);
    }
    return out;
