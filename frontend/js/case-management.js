...
function app() {
  return {
    ...
    prPreviewFetching: false,
    prCreateModal: false,
    prRemaining: null,
    prRemainingLoading: false,
    prMode: 'amount',
    prRatioInput: 0,
    prAmountInput: 0,
    prItemSelections: {},
    prTerms: { paymentTerms: '', deliveryTerms: '', acceptanceTerms: '', warrantyTerms: '' },
    _prEditingTermsFor: null,
    prSubmitting: false,
    ...
    async selectCase(quoteNo) {
      ...
      try {
        ...
        this.closePaymentRequestPreview()
        // 背景查詢是否已有關聯專案
    ...
    async openPaymentRequestModal() {
      if (this.dirty) { alert('款項明細有未儲存的修改，請先儲存後再申請請款單'); return }
      this.prMode = 'amount'
      this.prRatioInput = 0
      this.prAmountInput = 0
      this.prItemSelections = {}
      this.prRemaining = null
      this.prCreateModal = true
      this.prRemainingLoading = true
      try {
        const r = await fetch(`/api/payment-requests/remaining?quote_no=${encodeURIComponent(this.selected.quote_no)}`, {
          headers: { Authorization: 'Bearer ' + this.session.token }
        })
        if (r.ok) this.prRemaining = await r.json()
        else { alert((await r.json()).detail || '載入額度失敗'); this.prCreateModal = false }
      } catch (e) { alert('網路錯誤：' + e.message); this.prCreateModal = false }
      this.prRemainingLoading = false
    },

    closePaymentRequestModal() {
      this.prCreateModal = false
    },

    prToggleItem(it) {
      if (this.prItemSelections[it.itemId]) {
        delete this.prItemSelections[it.itemId]
      } else {
        const qty = it.remainingQty
        this.prItemSelections[it.itemId] = { qty, ratioPct: 100, amount: Math.round(qty * (it.unitPrice || 0)) }
      }
    },

    prItemQtyChanged(it) {
      const sel = this.prItemSelections[it.itemId]
      if (!sel) return
      if (sel.qty > it.remainingQty) sel.qty = it.remainingQty
      if (sel.qty < 0) sel.qty = 0
    ...
    // 品項模式下逐項輸入比例（%）自動換算數量/金額——比照整單金額模式既有的
    // 比例換算捷徑，使用者不用每項都手動算數量。換算後的數量仍受剩餘可請款
    // 數量上限保護，金額欄位維持可再手動微調（比照既有 prItemQtyChanged 慣例）。
    prItemRatioChanged(it) {
      const sel = this.prItemSelections[it.itemId]
      if (!sel) return
    ...
    _prTaxRatio() {
      const p = this.prRemaining?.quotePretax || 0
      return p > 0 ? (this.prRemaining.quoteTotal / p) : 1
    },

    // 比例輸入 → 換算出對應的請款金額（含稅），僅供畫面即時顯示；實際換算/
    // 防超收檢查一律以後端為準
    prRatioToAmount() {
      const total = this.prRemaining?.quoteTotal || 0
      return Math.round(total * (this.prRatioInput || 0) / 100)
    },

    prSelectedTotal() {
      return Object.values(this.prItemSelections).reduce((sum, s) => sum + (Number(s.amount) || 0), 0)
    },
    prSelectedGrossTotal() {
      return Math.round(this.prSelectedTotal() * this._prTaxRatio())
    },
    prSelectedTax() {
      return this.prSelectedGrossTotal() - this.prSelectedTotal()
    },

    async submitPaymentRequestCreate() {
      if (!this.prRemaining) return
      let body
      if (this.prMode === 'amount') {
        if (this.prRatioInput > 0) {
          if (this.prRatioInput > (this.prRemaining.remainingRatioPct + 1e-6)) { alert('超過剩餘可請款比例'); return }
          body = { quote_no: this.selected.quote_no, scope: 'amount', ratio_pct: this.prRatioInput }
        } else {
          if (!this.prAmountInput || this.prAmountInput <= 0) { alert('請輸入請款金額或請款比例'); return }
          if (this.prAmountInput > this.prRemaining.remainingAmount) { alert('超過剩餘可請款金額'); return }
          body = { quote_no: this.selected.quote_no, scope: 'amount', amount: this.prAmountInput }
        }
      } else {
        const items = Object.entries(this.prItemSelections).map(([itemId, sel]) => ({
          itemId: Number(itemId), qty: sel.qty, amount: sel.amount
        }))
        if (items.length === 0) { alert('請至少選擇一項品項'); return }
        if (this.prSelectedGrossTotal() > this.prRemaining.remainingAmount) { alert('超過剩餘可請款金額'); return }
        body = { quote_no: this.selected.quote_no, scope: 'items', items }
      }
      if (!confirm('確定送出建立請款單？')) return
      this.prSubmitting = true
      try {
        ...
        if (!r.ok) { alert((await r.json()).detail || '建立失敗'); this.prSubmitting = false; return }
        this.prCreateModal = false
        await this.loadPaymentRequests(this.selected?.quote_no)
      } catch (e) { alert('網路錯誤：' + e.message) }
      this.prSubmitting = false
    },

    prOpenTermsEditor(v) {
      this.prTerms = { paymentTerms: '', deliveryTerms: '', acceptanceTerms: '', warrantyTerms: '', ...(v.terms || {}) }
      this._prEditingTermsFor = v.requestNo
    },

    async prSaveTerms() {
      if (!this._prEditingTermsFor) return
      try {
        const r = await fetch(`/api/payment-requests/${this._prEditingTermsFor}/terms`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + this.session.token },
          body: JSON.stringify(this.prTerms)
        })
        if (!r.ok) { alert((await r.json()).detail || '儲存條款失敗'); return }
        this._prEditingTermsFor = null
        await this.loadPaymentRequests(this.selected?.quote_no)
      } catch (e) { alert('網路錯誤：' + e.message) }
    },

    async deletePaymentRequest(v) {
      if (!confirm(`確定刪除請款單「${v.requestNo}」？`)) return
      try {
        const r = await fetch(`/api/payment-requests/${v.requestNo}`, {
          method: 'DELETE',
          headers: { Authorization: 'Bearer ' + this.session.token }
        })
        if (r.ok) await this.loadPaymentRequests(this.selected?.quote_no)
        else alert((await r.json()).detail || '刪除失敗')
      } catch (e) { alert('網路錯誤：' + e.message) }
    },

    async submitPaymentRequest(v) {
      if (!confirm(`確定送出請款單「${v.requestNo}」進行簽核？`)) return
      try {
        const r = await fetch(`/api/payment-requests/${v.requestNo}/submit`, {
          method: 'POST',
          headers: { Authorization: 'Bearer ' + this.session.token }
        })
        if (!r.ok) { alert((await r.json()).detail || '送出失敗'); return }
    ...
    async approvePaymentRequest(v) {
      if (!confirm(`確定簽核請款單「${v.requestNo}」？`)) return
      try {
        const r = await fetch(`/api/payment-requests/${v.requestNo}/approve`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + this.session.token },
          body: JSON.stringify({})
        })
    ...
    async rejectPaymentRequest(v) {
      const note = prompt(`退回請款單「${v.requestNo}」，可填寫退回原因（選填）：`)
      if (note === null) return
      try {
        const r = await fetch(`/api/payment-requests/${v.requestNo}/reject`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + this.session.token },
          body: JSON.stringify({ note })
        })
    ...
    async revokePaymentRequestApproval(v) {
      const note = prompt(`撤銷請款單「${v.requestNo}」的核准？將退回草稿。\n\n可填寫撤銷原因（選填）：`)
      if (note === null) return
      try {
        const r = await fetch(`/api/payment-requests/${v.requestNo}/revoke-approval`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + this.session.token },
          body: JSON.stringify({ note })
        })
    ...
    async downloadPaymentRequestPdf(v) {
      try {
        fetch(`/api/payment-requests/${v.requestNo}/export?mode=external`, {
          method: 'POST',
          headers: { Authorization: 'Bearer ' + this.session.token }
        }).catch(() => {})
        const r = await fetch(`/api/payment-requests/${v.requestNo}/pdf-download`, {
          headers: { Authorization: 'Bearer ' + this.session.token }
        })
        if (!r.ok) { alert((await r.json().catch(() => ({}))).detail || 'PDF 產生失敗'); return }
        const blob = await r.blob()
    ...
    async previewPaymentRequestPdf(v) {
      this.prPreviewFetching = true
      try {
        const r = await fetch(`/api/payment-requests/${v.requestNo}/pdf-download`, {
          headers: { Authorization: 'Bearer ' + this.session.token }
        })
        if (!r.ok) { alert((await r.json().catch(() => ({}))).detail || 'PDF 產生失敗'); this.prPreviewFetching = false; return }
        const blob = await r.blob()
        this.prPreviewBlobUrl = URL.createObjectURL(blob)
        this.prPreviewVoucher = v
        this.prPreviewModal = true
      } catch (e) { alert('預覽失敗：' + e.message) }
      this.prPreviewFetching = false
    },

    closePaymentRequestPreview() {
      if (this.prPreviewBlobUrl) URL.revokeObjectURL(this.prPreviewBlobUrl)
      this.prPreviewBlobUrl = ''
      this.prPreviewModal = false
      this.prPreviewVoucher = null
    },
